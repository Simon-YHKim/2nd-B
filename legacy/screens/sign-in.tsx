...
//
//   was:  src/app/(auth)/sign-in.tsx   (the EXPO_PUBLIC_UI=legacy renderer + its styles)
//   why:  every delivery path pins deep-space and ui-mode.ts defaults to it, so
//         this branch had been unreachable for months.
...
//           git show <sha-before-retirement>:src/app/(auth)/sign-in.tsx
//
// 인용은 먼저 옮겼다(#1777). 그때 손실처럼 보이던 셋이 전부 손실이 아니었다:
//
//   accessibilityState 의 disabled   공용 PixelPressable 이 합쳐서 넘긴다
//                                    (PixelPressable.tsx: accessibilityState 병합)
//   언어 전환                         가입 화면으로 옮겨갔다 — 한 탭 거리
//   안내서 링크                       같음
//
// 그리고 진짜로 뒤집힌 계약 둘은 지우지 않고 검사에 적어 뒀다:
//
//   홈 뒤로가기가 없다   **없는 것이 맞다.** src/app/index.tsx 가 로그아웃
//                       사용자를 /sign-in 으로 되돌리므로 이 버튼은 자기 화면으로
//                       오는 제자리 고리였다.
//   재설정이 라우트로     인라인 안내 -> /reset-password. resetPasswordHref 가
//                       완전한 주소일 때만 프리필해서 반쯤 친 값이 라우트 상태로
//                       새지 않는다.
//
// 바이트 핀이 이 아카이브를 따라왔다 — 아래 본문의 sha256 이 은퇴 전 라우트 파일과
// **같다**는 것이 "옮기면서 안 고쳤다"의 증거다
// (src/lib/auth/__tests__/sign-in-screen-contract.test.ts).
//
