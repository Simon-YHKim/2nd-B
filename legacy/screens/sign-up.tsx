...
//
//   was:  src/app/(auth)/sign-up.tsx   (the EXPO_PUBLIC_UI=legacy renderer + its styles)
...
//           git show <sha-before-retirement>:src/app/(auth)/sign-up.tsx
//
// The gate moved with it: the live screen carries the same
// `userId && !submitting && !judgeWelcome && !toast -> /` redirect itself, so
// nothing had to stay behind (unlike /account).
//
// ⚠ There are TWO DeepSpaceSignUpDesignScreen. The route imports the one in
// dds-sign-up-screen.tsx; dds-auth-screens.tsx holds a 224-line shadow copy no
// route renders. shadow-screens.test.ts (#1767) now pins which is which.
//
