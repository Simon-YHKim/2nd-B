...
//
//   was:  src/app/index.tsx   (the EXPO_PUBLIC_UI=legacy renderer + its styles)
//   why:  every delivery path pins deep-space and ui-mode.ts defaults to it, so
//         this branch had been unreachable for months. Simon approved retiring
//         the legacy home skin (Q-260905-02) on the condition "migrate the
//         guards first" — that migration landed in #1781.
...
//           git show <sha-before-retirement>:src/app/index.tsx
//
// ⚠ This one hid for longer than the others. The span parser looked for lines
// starting `function Name(` and this file declares `export function GraphScreen()`
// — one word, and 559 lines were invisible to every check built on that module
// (#1779 fixed it). The roster of unparsed shapes even carried a wrong reason
// for it: "위임 뒤에 bare return 이 없다", which was not true.
//
// What moved with the renderer, because only it used them:
//   useSkyDrift · pickInsight · featuredVillageForCards · FIRST_PIECE_INSIGHT ·
//   RECORDS_ONLY_INSIGHT · OFFLINE_INSIGHT · entryFlourishPlayed · GraphStarHost ·
//   styles · HomePressable(+Props) · logo · AnimatedPressable · HOME_GROUND ·
//   homeAlphaC · INSIGHTS · isVillageId
// Those sat at module scope, which the dead-span check cannot see (it covers
// function bodies), so they had to be found by reading rather than by counting.
//
// The live route kept exactly one thing besides the dispatch:
// `useImportPendingCaptures()`. It runs on every home mount regardless of skin.
//
