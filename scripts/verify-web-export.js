...
function run() {
  ...
  // graph. Expo's persistent Metro cache can otherwise replay an older
  // document shell after a security-policy edit, producing a false result.
  const exportResult = spawnSync(`npx expo export --clear --platform web --output-dir ${OUT}`, {
    stdio: "inherit",
    shell: true,
    env: {
      ...process.env,
