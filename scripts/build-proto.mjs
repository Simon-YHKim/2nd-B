...
const noticeSource = resolve(projectRoot, "public/THIRD_PARTY_NOTICES.txt");
...
async function prepareRoots() {
  ...
  return { distReal, sourceReal, outputReal };
}

async function writeThirdPartyNotices(distReal) {
  await assertNoSymlinkSegments(projectRoot, noticeSource, "third-party notice source");
  const sourceReal = await realpath(noticeSource);
  assertInside(await realpath(projectRoot), sourceReal, "third-party notice source");

  const noticePath = resolve(distRoot, "THIRD_PARTY_NOTICES.txt");
  assertInside(distReal, noticePath, "third-party notice output");
  await assertNoSymlinkSegments(distRoot, noticePath, "third-party notice output");
  await writeFile(noticePath, await readFile(sourceReal));
}
...
await writeThirdPartyNotices(roots.distReal);
