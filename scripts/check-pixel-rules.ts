...
const MIGRATED: readonly string[] = [
  ...
  "src/screens/deepspace/dds-profile-screen.tsx",
...
// 342 → 333 → 315 → 202 → 193 → 165 → **164** (2026-08-31).
//
...
// 158 → 152 (2026-09-08): 레거시 렌더러 여덟을 legacy/ 로 옮기면서 그 안의 위반
// 6건이 빌드 밖으로 나갔다. 고쳐서 줄어든 것이 아니라 **대상이 옮겨간 것**이라
// 남은 빚의 성격은 그대로다 — 이 수는 목표가 아니라 현재 사실이다.
// 152 → 148 (2026-09-08, 같은 날 2차): research·insights·import 렌더러가 나가면서
// 4건이 더 빌드 밖으로. 같은 이유이므로 같은 단서가 붙는다 — **고친 것이 아니다.**
// 은퇴가 계속되는 동안 이 수는 계속 내려가는데, 그것을 진척으로 읽지 말 것.
const RATCHET_BASELINE = 148;
