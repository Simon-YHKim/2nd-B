        ...
        <div style={{ display: 'flex', gap: 9, marginBottom: 20 }}>
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} onClick={() => setEnergy(n)} className="md-interactive" aria-label={`에너지 ${n}`}
              ...
              <span className="md-state" />
              {/* 시인성: 무라벨 레벨 바는 미선택 상태에서 빈 사각형으로 보임 — 단계 숫자를 노출 */}
              <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 11, fontWeight: 700,
                color: n <= energy ? C('on-tertiary') : C('on-surface-variant') }}>{n}</span>
            </button>
          ))}
        </div>
