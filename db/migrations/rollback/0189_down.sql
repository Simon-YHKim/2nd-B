...
--
--   잊으면 두 군데서 빨강이다. 이 문단은 원래 "CI 가 빨강이다" 한 줄이었고, 그
--   말은 절반만 사실이었다 (r50 생성물 게이트 B-NEW-01 · 실측 2026-09-21 00:28 KST:
--   그 대조는 사실 여섯 개만 읽어서 새 제약 · 인덱스 · 정책 · 트리거 · COMMENT ·
--   열 기본값 등 22종 중 12종에는 움직이지 않았다).
--     1) DB 없이, `npm run verify` 에서:
--        src/lib/privacy/__tests__/erasure-registry-migration.test.ts 가 0189 부터의
--        마이그레이션을 읽어 **실행되는 SQL** 에 두 객체 이름이 나오는 파일을
--        찾고 (주석 · 문자열 · 실행되지 않는 달러 본문은 가린다), 그 ledger name 이
--        아래 c_names 에 있기를 요구한다. 실행 시점에 조립하는 SQL 도 **상수이면**
--        복원해서 읽는다 (EXECUTE 뒤의 리터럴 · 리터럴 연결 · 상수 인자의 format()).
--        상수가 아니어서 복원하지 못한 SQL 이 든 파일은 조용히 통과하지 않는다:
--        여기 c_names 에 올리든지, 그 테스트의 "읽어 보았고 기대지 않는다" 목록에
--        이유와 함께 올리든지, 사람이 둘 중 하나로 답해야 초록이다.
--     2) 실제 DB 에서, supabase-dry-run.yml "Exercise the 0189 rollback round trip":
--        되돌리기 + 다시 밀기 뒤에 두 객체에 **매달린 것 전부** (소유자 · ACL · 함수
--        정의 · 등록부 행 · 열 · 제약 · 인덱스 · 정책 · 트리거 · 규칙 · 통계 객체 ·
--        COMMENT · 그 밖에 pg_depend 가 매단 것)가 되돌리기 전과 같은지 대조한다.
--        그 대조가 각 종류를 실제로 **보는지** 도 같은 단계가 매번 증명한다 (못 보면
--        BLIND = 빨강).
--   두 검사는 **보는 방식이 다르다.** 1) 은 어떤 종류의 변경이든 이름만 나오면 잡지만
--   철자(도우미 함수를 거친 호출 등)에 속을 수 있고, 2) 는 철자에 속지 않지만 읽는
--   카탈로그 사실의 목록만큼만 본다. 구멍은 둘이 **함께** 못 보는 곳에만 난다
--   (r52 게이트 둘: 1) 이 못 읽는 EXECUTE format(...) × 2) 가 안 모으던 열 통계 목표).
--
--   ⚠ c_names 는 1) 이 찾은 것보다 **많아도 된다.** 1) 은 하한이다. 2) 가 빨강이 되어
--     목록에 올린 마이그레이션을 1) 이 "그런 파일은 기대지 않는다" 며 거부하던 것이
--     r52 의 거짓 빨강이었다. 대신 목록은 아무 이름이나 받지 않는다: 0189 부터의 실제
--     파일이어야 하고, 그 **파일 번호와 이유**를 이름 옆 주석에 적어야 한다. 목록에 든
--     마이그레이션은 되돌린 뒤 **다시 적용되기** 때문이다 - 사고 대응 중에, 이유를
--     아무도 적어 두지 않은 마이그레이션이 한 번 더 도는 일은 없어야 한다.
--   두 번 적용해도 되는 파일이어야 하고, 그것을 실제로 보여 주는 것이 2) 다.
...
DO $rollback_ledger$
DECLARE
  -- 한 줄에 하나, 이름 옆에 `-- <파일 번호>: <두 객체에 무엇을 매다는가>`. 번호와 이유가
  -- 없으면 `npm run verify` 가 빨강이다 (머리말 "잊으면 두 군데서 빨강이다").
