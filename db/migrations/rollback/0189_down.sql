...
--        src/lib/privacy/__tests__/erasure-registry-migration.test.ts 가 모든
--        마이그레이션을 읽어 **실행되는 SQL** 에 두 객체 이름이 나오는 파일을
--        찾고 (주석 · 문자열 · 실행되지 않는 달러 본문은 가린다), 그 ledger name 이
--        아래 c_names 에 있기를 요구한다. 목록에만 있고 그런 파일이 없어도 빨강이다.
--     2) 실제 DB 에서, supabase-dry-run.yml "Exercise the 0189 rollback round trip":
--        되돌리기 + 다시 밀기 뒤에 두 객체에 **매달린 것 전부** (소유자 · ACL · 함수
--        정의 · 등록부 행 · 열 · 제약 · 인덱스 · 정책 · 트리거 · COMMENT · 그 밖에
--        pg_depend 가 매단 것)가 되돌리기 전과 같은지 대조한다. 문자열로 조립해
--        EXECUTE 하는 SQL 은 1) 이 읽지 못하므로 이쪽이 잡는다. 그 대조가 각
--        종류를 실제로 **보는지** 도 같은 단계가 매번 증명한다 (못 보면 BLIND = 빨강).
--   목록에 든 마이그레이션은 되돌린 뒤 **다시 적용된다.** 두 번 적용해도 되는
--   파일이어야 하고, 그것을 실제로 보여 주는 것이 2) 다.
