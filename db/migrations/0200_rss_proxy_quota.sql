-- UNNUMBERED DRAFT. The integration owner must allocate a fresh migration
-- number only after re-scanning the remote reservation set.
--
-- Deployment order: apply this migration successfully, verify the postconditions,
-- then deploy rss-proxy. The function fails closed while this RPC is absent.
-- Each successful claim spends both the global UTC-day allowance and the
-- caller's UTC-day allowance in one transaction. Every caller locks the global
-- row first, then its user row, so concurrent requests share one lock order.
