...
--
-- No top-level BEGIN/COMMIT: the Supabase migration runner owns the transaction.
...
BEGIN
  ...
  -- The global lock makes this read authoritative until an admitted request
  -- increments the shared counter below. Check before allocating a new keyed
  -- row so a distributed burst cannot grow ledger cardinality past the cap.
  SELECT CASE
           WHEN limits.window_start = v_window_start THEN limits.request_count
           ELSE 0
         END
    INTO v_global_count
    FROM public.peer_response_rate_limits AS limits
   WHERE limits.dimension = 'global'
     AND limits.action = p_action
  ...
  -- Globally denied traffic returns before this maintenance work.
  ...
  -- A peer already over its keyed ceiling must not burn the fleet-wide budget.
  IF v_key_count > v_key_cap THEN
    RETURN QUERY SELECT false AS allowed, v_retry AS retry_after_seconds;
    RETURN;
  END IF;
  ...
  DO UPDATE SET
    ...
    request_count = CASE
      WHEN limits.window_start = EXCLUDED.window_start
        THEN LEAST(limits.request_count + 1, v_global_cap)
      ELSE 1
    END,
    updated_at = EXCLUDED.updated_at
...
BEGIN
  IF pg_catalog.to_regclass('public.peer_response_rate_limits') IS NULL
    ...
    RAISE EXCEPTION 'peer response limiter postcondition failed';
  END IF;
END
$postconditions$;
