...
- **내 데이터 내려받기가 화면에 붙었다.** 계정에 저장된 것을 구조화된 파일 하나로 받는다.
  일부만 받아졌으면 받아졌다고만 말하지 않고 **무엇이 빠졌는지** 함께 말한다.
- **기기에 남는 민감한 값이 암호화 저장소로 옮겨간다.** 로그인 복구 상태, 임시 초안,
  가져오기 기록 같은 것들이다. 안드로이드에서는 백업으로도 새어 나가지 않는다.

### Changed

- **"다 받았다" 를 함부로 말하지 않는다.** 내보내기 문구에서 "everything"·"complete" 를
  걷어냈다. 이 기능은 열 가지 범주를 일부러 제외하고, 제외한 사실을 응답으로 돌려준다.
- **삭제 영수증이 "확인하지 못했다" 와 "서버가 못 끝냈다고 말했다" 를 구분한다.**
  둘은 사용자가 다음에 할 일이 다르다.

### Fixed

- **추천을 켤 때 동의 기록이 실패하면 설정이 켜지지 않는다.** 전에는 개인화만 켜지고
  동의한 기록은 남지 않을 수 있었다.
- **로그아웃 뒤 도착한 내보내기를 다음 사람에게 건네지 않는다.** 롤백 화면에서
  앞 사람의 계정 파일이 다음 로그인 사용자의 기기로 내려가거나 공유창이 열릴 수 있었다.

### Security

> ⚠ **엣지 함수 항목은 코드만 이 태그에 들어 있다.** 서버에 올리는 것은 별도 동작이고
> 아직 하지 않았다. (v0.7.2 에서 안내한 것과 같은 상태다.)

- **앱이 기기에 쓰는 민감한 값이 평문으로 남지 않는다.** 암호화 저장소가 들어왔고,
  안드로이드 백업에서 제외된다.
