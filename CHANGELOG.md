...
이번 판은 새 기능보다 **안전과 정직**에 관한 것이다. v0.8.0 이후 **172개**의 변경이
들어갔고(머지 커밋 제외), 그중 **69개(40%)** 가 보안 · 법무 · 인증 · 가드였다
(법무 28 · 보안 18 · 가드 12 · 인증 11).

> **이 업데이트는 새로 받아야 한다.** 네이티브 저장소와 권한 선언이 움직여 플랫폼
> fingerprint 가 바뀌었다. 이전 설치본은 무선으로 받지 못하고, 전달 수단은 새 APK · AAB · IPA 다.

### 고친 것 중 가장 아픈 것

- **안드로이드에서 앱이 "Loading" 에서 영영 넘어가지 않던 문제.** 봉인된 저장소를
  `expo-crypto` 에 넘기는 형식이 Android 에서만 달랐다. 재부팅해도 안 풀렸고, 앱을 완전히
  지웠다 깔아야 했다. 그 형식을 바로잡았다.
- **인용을 눌러도 그 문서가 열리지 않던 문제.** 최근 200개 밖의 글을 인용하면 화면이 그 글을
  조용히 버렸다. 이제 한 행을 더 당겨 온다.
- **동의한 초기화가 마지막 단계에서 멈추던 문제.** 이미 끝난 초기화가 "끝났다"고 말하지
  못해 다시 시도할 길이 사라졌다.

### 새로 할 수 있는 것

- **저장한 조각을 그 조각이 속한 생활 영역에서 연다.** 어디에 담겼는지 보인다.
- **가져온 자료를 읽을 자리**가 생겼다. 임포트한 글이 목록에만 있지 않다.
- **뮤지엄의 43개 이벤트 전부에 영어 짝**이 붙었다. 번역인지 원문인지 정직하게 표시한다.
- **웹에 공유 카드**가 생겼다. 링크를 붙여넣으면 캐논에서 만든 그림이 따라온다.
- 다섯 언어의 문구를 다시 썼다. 말투가 아니라 **무엇을 약속하는가**를 고쳤다.

### 보이지 않지만 중요한 것

- **공공데이터 키가 앱을 떠났다.** 이제 프록시가 대신 부른다.
- **콘텐츠 삭제를 서버가 소유한다.** 지울 표의 목록을 손으로 적지 않고 스키마에서 뽑고,
  그 목록이 실제로 지워지는지를 실DB 회귀가 확인한다.
- **한 계정의 글이 다음 계정 화면에 남지 않도록** 위키 화면에 계정 울타리를 쳤다.
- **가입 연령을 나라별로 본다.** 63개국의 자기동의 연령을 1차 법령으로 확인해 표로 넣었다.
  나라를 모를 때는 보수적인 값을 쓴다. 가입이 막히는 화면도 **그 사람에게 실제로 적용된 나이**를
  말한다 — 전에는 어느 나라 사람이든 "만 14세"라고만 했다.
- 보안 마이그레이션 **17개(0171-0187)** 와 그 뒤의 삭제 등록부 · 잠금(0189 · 0190)이
  들어갔다.
- 문서와 가드가 **"배송되는 화면"** 을 읽도록 고쳤다. 예전에는 이름만 보고 판정해서
  없는 기능을 살아 있다고 적는 일이 있었다.

### 아직 아닌 것

- 스토어에는 아직 올라가지 않았다. 이 릴리스는 APK · AAB · IPA 를 GitHub Release 로 전달한다.
- 지역을 읽지 못하는 환경(웹이 흔하다)에서는 보수적인 연령이 적용된다. 거주국을 묻는
  화면은 다음 판에 붙는다.
