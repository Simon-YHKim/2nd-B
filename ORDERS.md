...
### [O-15 / 2026-06-15] 허브 자가점검 — 발견 에러 수정 + 규칙화(재발방지 학습)
...
### [O-14 / 2026-06-15] landing-clone — 반응형(좌우 10%) + 메뉴 2ndb 정식 IA 재구성 + 진입화면 스타일 일관성
[2026-06-15 / 07:42 KST] Simon 디렉터(모바일) — O-13 라이브 확인 후 피드백. 대상 동일(`landing-clone`, 8777). 검증 = 다양한 뷰포트 스크린샷.

**A. 반응형 — 사용자 화면 크기에 적응 (현 `charHeight: 4.9` 고정이 문제)**
- 캐릭터(머리) hero/nav 크기 + 말풍선 + 메뉴가 **뷰포트 폭/높이에 맞게 스케일**. 작은 폰(360) ~ 큰 데스크톱(1280+) 모두 정상.
- **좌우 여백 각 10%만** — 콘텐츠가 화면 폭 약 80%를 채운다. hero 머리도 이 80% 폭 기준 크기, nav의 좌상단 머리·말풍선·메뉴도 그 80% 안에 배치.
- 세로(폰) 화면에서도 좌상단 머리·말풍선·메뉴가 서로 겹치지 않게(좁으면 메뉴를 머리 아래로 등 적응).

**B. 메뉴 = 2ndb 정식 IA로 재구성 (현 당기/세컨비/기록/위키는 IA와 불일치)**
- 정본 = `src/components/premium/tab-bar.tsx` 의 4 primary 탭: **그래프(`/`) · 담기(`/capture`) · 세컨비(`/secondb`) · 나(`/profile`)** (VISION 3축 IA, menu-restructure Phase 3-5).
- 교정: `당기`→**담기**(앱 표기 일치) · **그래프·나 추가**(현재 누락) · 라벨은 앱과 일치(ko: 그래프/담기/세컨비/나), `data-go`는 라우트.
- `기록(records)`·`위키(wiki)`는 **별도 탭이 아니라 그래프 마을 경유 2차 진입** → primary 메뉴에서 제거하지 말고 **2차 그룹으로 시각 분리**(primary 4 + 보조). 완전 제거 여부는 Simon 라이브 확인.
- ⚠️ landing이 곧 홈이면 "그래프"가 자기 자신일 수 있음 — 그래프 항목을 마을/그래프 뷰로 보낼지 실행자 판단 후 `## DONE`에 기재.

**C. 진입 화면 스타일 일관성**
- 메뉴 항목이 실제 화면/서브뷰로 연결될 때 전부 현 landing 언어(deep-space · Space Mono · 모노톤 3색 이내 · crisp, bounce/elastic 금지)로 일관.
- 2ndb 본체는 gameboy 트랙이라 다름 — 이 프로토타입은 **deep-space 독립 트랙** 유지, 본체와 섞지 말 것. 통일 방향이 모호하면 `## DONE`에 질문.

**수용 기준**: 360/768/1280px 뷰포트에서 캐릭터·메뉴가 좌우 10% 여백으로 정상 배치 / 메뉴 primary = 그래프·담기·세컨비·나(앱 라벨) + records/wiki 2차 분리 / 모노톤·crisp 유지. before/after 스크린샷(폰 세로 + 데스크톱 2뷰포트).
