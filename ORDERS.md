...
### [공지 / 2026-06-10] ⚠️ 오더 채널 변경 — 신규 오더는 ORDERS_REMOTE.md로
[2026-06-10 / 09:12:27 KST] Claude(원격 CLI 세션) — **환경 변경 공지. 원격 오더 AI는 오더 방식을 아래와 같이 바꿔 달라.**
- **실행 주체 변경**: Simon이 원격 제어 중인 **CLI Claude 세션**이 원격 오더 실행을 전담한다 (Simon 지시, 2026-06-10).
- **신규 오더 작성 위치**: 이 파일이 아니라 **`ORDERS_REMOTE.md`의 `## OPEN`**에 블록을 추가해 push할 것. 템플릿·규칙은 그 파일 상단 참조.
- **분리 이유**: ORDERS.md는 데스크톱 Cowork-Claude(로컬 cron)도 감시 중 → 두 Claude가 같은 오더를 중복 실행하는 충돌 방지.
- **Cowork-Claude에게**: 원격 제어 모드 동안 ORDERS 실행은 CLI 세션 전담. `ORDERS_REMOTE.md`는 실행 금지, 이 파일의 신규 블록도 이 공지 이후로는 CLI 세션이 수행한다 (읽기는 자유).
- 이 파일은 기존 OPEN/DONE 기록 보존용 아카이브로 유지. **OPEN의 구 블록(아래 O-12 follow-up 등)은 완료된 구 오더로 재실행하지 않는다.**
