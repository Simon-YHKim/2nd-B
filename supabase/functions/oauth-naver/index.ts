...
// login. Keep ENABLE_NAVER_OAUTH off until migration 0183 and all secrets exist.
...
async function readRequestObject(req: Request): Promise<Record<string, unknown>> {
  ...
  if (mediaType !== 'application/json') {
    await cancelUnreadBody(req.body);
  ...
  if (contentLength !== null) {
    ...
    if (declaredLength > REQUEST_BODY_LIMIT_BYTES) {
      await cancelUnreadBody(req.body);
      throw new BoundaryError(413, 'request_body_too_large');
    }
  }

  const bytes = await readBoundedBytes(req.body, {
    ...
    declaredLength,
    lengthMismatchCode: 'request_content_length_mismatch',
  });
  return decodeJsonObject(bytes, 'invalid_json', REQUEST_JSON_MAX_DEPTH);
}
