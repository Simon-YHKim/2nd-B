...
function PersonaCard({ onClose, onRoute }) {
  ...
  const records = window.SB_DATA.persona.card.records; // mock corpus size → data/screens/persona.json

  // archetype = blend of dominant trait + dominant domain leaning
  const archetype = window.SB_DATA.persona.card.archetype; // → data/screens/persona.json
  const essence = window.SB_DATA.persona.card.essence; // → data/screens/persona.json
  const summary = window.SB_DATA.persona.card.summary; // → data/screens/persona.json
  ...
  function Label({ children, tone }) {
    return (
      <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 11, letterSpacing: '.14em',
        color: tone, opacity: .95, margin: '18px 0 10px' }}>{children}</div>
    );
  }
}
...
/* ============================================================
   ...
   ============================================================ */
const SB_PERSONAS = window.SB_DATA.persona.personas; // → data/screens/persona.json
...
function PersonaDeck({ go }) {
  ...
  return (
    <div style={{ padding: '14px 0 6px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '0 18px 10px' }}>
        <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 11, letterSpacing: '.16em', color: PUR, whiteSpace: 'nowrap' }}>
          내 역할
        </span>
  ...
  function PCard({ p, go, PUR, PURSOFT }) {
    const lbl = (txt, mt) => (
      <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 11, letterSpacing: '.16em',
        color: PUR, opacity: .95, margin: `${mt}px 0 8px` }}>{txt}</div>
    );
    return (
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 9999, whiteSpace: 'nowrap',
                background: PURSOFT, border: '1px solid rgba(167,139,250,.3)', fontSize: 11, color: '#D9CBF7' }}>
                {p.scope}
              </span>
