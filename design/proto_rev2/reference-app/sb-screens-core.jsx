              ...
              <div key={c.id} onClick={() => { if (!confirming) onSelect(c.id); }} className="md-interactive"
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    ...
                    <span style={{ flex: '0 0 auto', fontSize: 11, color: C('on-surface-variant'), opacity: .9 }}>{relTime(c.ts)}</span>
                  </div>
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '8px 10px 8px 4px', background: m.soft, borderBottom: `1px solid ${m.soft}` }}>
        ...
        <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10.5, fontWeight: 700, color: m.onSoft, whiteSpace: 'nowrap', flex: '0 0 auto' }}>{m.tag}</span>
        ...
        {MODES.map((x) => {
          ...
          return (
            <button key={x.id} onClick={() => setModeId(x.id)} className="md-interactive"
              ...
              <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10.5, color: on ? x.accent : C('on-surface-variant'), marginTop: 1 }}>{x.tag}</div>
            </button>
          );
        })}
      </div>
