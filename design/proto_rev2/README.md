...
> **[개선 작업본 · 2026-07-04]** 이 폴더는 `2ndB proto_rev2 (Copy).zip` 원본의 **비율·시인성 개선 클론**이다.
> 전 60화면에 ① 390×820 균일 비율 보장(`text-size-adjust` 가드 + 스펙 캡처 37장 균일 재생성) ② 한국어 `word-break: keep-all` 전역화 ③ 폰트 바닥(한글 ≥11px · 모노 ≥10px) ④ WCAG 대비 보정을 적용했다.
> 상세: **`IMPROVEMENTS.html`** · 비포/애프터 전체 캡처: `docs/improve-audit/{before,after}/` · 원본 픽셀은 zip에 그대로 보존.
...
- 데이터는 전부 **`reference-app/data/*.json`**(정본)에서 온다 — `sb-boot.js`가 모든 스크립트 실행 전에 동기 로드해 `window.SB_DATA`로 노출하고, `sb-data.jsx`가 기존 `window.SB` 형태로 재구성한다(소비 코드 무변경). 서버 없음, 분석·AI 응답은 시뮬레이션. 상세: `reference-app/data/README.md`.
...
- `data/` — **앱 구동 데이터 정본** (화면 레지스트리 `app/screens.json`, nav, 별하늘, 아이콘, 화면팩 `screens/*.json`; 매니페스트 `data/index.json`) + `sb-boot.js` 동기 로더
- `m3-theme.css` 토큰 / `sb-data.jsx` (JSON→`window.SB` 재구성)+프리미티브 / `sb-app.jsx` 셸·라우터(JSON 레지스트리 구동)
