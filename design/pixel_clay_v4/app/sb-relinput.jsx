...
function RelAvatar({ name, cat, size = 42 }) {
  ...
  return (
    <div style={{ width: size, height: size, flex: '0 0 auto', borderRadius: 0, display: 'grid', placeItems: 'center',
      background: col ? `color-mix(in srgb, ${col} 26%, rgba(12,19,38,.6))` : 'rgba(127,178,255,.14)',
      border: `1.5px solid ${col || 'rgba(127,178,255,.3)'}`, color: col || 'var(--c06)',
  ...
  const Row = (c) => (
    <button key={c.id} type="button" onClick={() => pick(c)} className="md-interactive"
      style={{ position: 'relative', width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px',
        border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', borderRadius: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          {c.cat &&
            <span style={{ flex: '0 0 auto', fontSize: 12, fontWeight: 700, color: REL_CCOLOR[c.cat], whiteSpace: 'nowrap',
              background: `color-mix(in srgb, ${REL_CCOLOR[c.cat]} 18%, transparent)`, borderRadius: 0, padding: '2px 8px' }}>
              {REL_CNAME[c.cat]}
            </span>}
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 12px 20px' }}>
        ...
        {/* 검색 */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', margin: '0 4px 14px', borderRadius: 0,
...
/* ===================== 한 사람에 대해 입력 ===================== */
const relInput = (C) => ({
  width: '100%', boxSizing: 'border-box', border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: '11px 13px',
    ...
    return (
      <ScreenPad>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 440, gap: 18, textAlign: 'center' }}>
          <div style={{ width: 88, height: 88, borderRadius: 0, display: 'grid', placeItems: 'center', background: C('primary-container'), color: C('on-primary-container') }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 14px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 3, padding: 3, borderRadius: 0, background: C('surface-container-highest'), border: `1px solid ${C('outline-variant')}` }}>
              {REL_TIERS.map(([k, lab]) => {
                const on = tier === k;
                return (
                  <button key={k} type="button" onClick={() => setTier(k)} className="md-interactive"
                    style={{ flex: 1, position: 'relative', border: 'none', cursor: 'pointer', borderRadius: 0, padding: '8px 0',
                      fontSize: 15, fontWeight: on ? 700 : 500, fontFamily: 'var(--md-ref-typeface-plain)',
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: C('on-primary-container'), background: C('primary-container'), borderRadius: 0, padding: '2px 8px', whiteSpace: 'nowrap', flex: '0 0 auto' }}>★ 가장 중요</span>
            </div>
