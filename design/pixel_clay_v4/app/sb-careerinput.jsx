    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={hint}
          ...
          style={{ flex: 1, minWidth: 0, border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: '11px 13px',
      ...
      {tags.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
          {tags.map((tg) => (
            <span key={tg} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 6px 6px 12px', borderRadius: 0,
              border: `1px solid ${C('outline-variant')}`, background: C('surface-container-highest'), color: C('on-surface'), fontSize: 12, fontWeight: 700 }}>
              {tg}
              <button type="button" onClick={() => remove(tg)} aria-label="삭제"
                style={{ display: 'grid', placeItems: 'center', width: 18, height: 18, borderRadius: 0, border: 'none', cursor: 'pointer',
...
function CareerInputScreen({ t, go, param }) {
  ...
  const inputStyle = { width: '100%', border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: '11px 13px',
    background: C('surface-container-highest'), color: C('on-surface'), fontFamily: 'var(--md-ref-typeface-plain)', fontSize: 15, outline: 'none' };
    ...
    return (
      <ScreenPad>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 440, gap: 18, textAlign: 'center' }}>
          <div style={{ width: 88, height: 88, borderRadius: 0, display: 'grid', placeItems: 'center', background: C('tertiary-container'), color: C('on-tertiary-container') }}>
        ...
        <CiSection C={C} icon="insights" title="KPI · 성과 지표" hint="직접 입력 · 추천에서 담기">
          ...
          {kpis.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
              {kpis.map((k) => (
                <div key={k.id} style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: '8px 8px 8px 12px', background: C('surface-container') }}>
                  ...
                  <input value={k.value} onChange={(e) => setKpiVal(k.id, e.target.value)} placeholder="값" inputMode="decimal"
                    style={{ width: 76, textAlign: 'right', border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: '7px 9px',
                      background: C('surface-container-highest'), color: C('on-surface'), fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 15, outline: 'none' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
            ...
            <span className="md-label-small" style={{ color: C('on-tertiary-container'), background: C('tertiary-container'), padding: '2px 8px', borderRadius: 0, whiteSpace: 'nowrap' }}>직무 기반</span>
          </div>
            ...
            {SUGGEST.map((s) => {
              ...
              return (
                <button key={s.name} type="button" onClick={() => addKpi(s.name, s.unit)} disabled={on} className="md-interactive"
                  style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 12px', borderRadius: 0, cursor: on ? 'default' : 'pointer',
                    border: `1px solid ${on ? C('tertiary') : C('outline-variant')}`, background: on ? C('tertiary-container') : C('surface-container-highest'),
                    color: on ? C('on-tertiary-container') : C('on-surface'), fontFamily: 'inherit', fontSize: 12, fontWeight: 700, opacity: on ? .9 : 1 }}>
