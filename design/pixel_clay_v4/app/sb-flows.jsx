...
function OnboardingScreen({ onDone }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 40, display: 'flex', flexDirection: 'column',
      background: 'var(--ds-space)', padding: '0 24px' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <div style={{ animation: 'sb-bob 2.4s steps(2,end) infinite' }}>
          <window.SbHead size={160} expression="positive" track />
        </div>
        <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, letterSpacing: '.2em', color: 'var(--ds-nebula)', marginTop: 22 }}>2NDB</div>
        <div className="md-headline-small" style={{ color: 'var(--c07)', marginTop: 8 }}>나를 알아가는 AI</div>
        {/* 인트로 화면은 별도 제작 예정 — 지금은 자리만 잡아 둔다 */}
        <div className="px-frame" style={{ marginTop: 22, padding: '10px 14px', background: 'var(--panel)' }}>
          <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, color: 'var(--fg-muted)' }}>인트로 화면 · 제작 예정</span>
        </div>
      </div>
      <div style={{ paddingBottom: 26 }}>
        <MdButton variant="filled" full trailingIcon="arrow_forward" onClick={onDone}>시작하기</MdButton>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 50 }}>
      ...
      <div style={{ position: 'absolute',
        ...
        borderRadius: r.h > 120 ? 24 : '50%', border: '2px solid var(--c11)', boxShadow: 'none', pointerEvents: 'none' }} />
      {/* tip card */}
      <div style={{ position: 'absolute', left: 20, right: 20, ...(s.arrow === 'up' ? { top: '54%' } : { bottom: 110 }) }}>
        <window.DialogBox compact kindLabel={`가이드 ${i + 1}/${steps.length}`} title={s.title} line={s.body}
        head={<window.SbHead size={32} track={false} />}
        choices={[{ label: '다시 안 보기', icon: 'close', on: onDone },
        { label: last ? '시작하기' : '다음', icon: last ? 'check' : 'arrow_forward', on: () => (last ? onDone() : setI(i + 1)) }]} />
      </div>
    </div>
  );
}
      ...
      <div style={{ overflowY: 'auto', height: '100%' }}>
        <ScreenPad>
          ...
          <MdCard variant="filled" style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {transcript.map((l, i) => (
              <div key={i} style={{ display: 'flex', gap: 8 }}>
                <span style={{ flex: '0 0 auto', fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 0, height: 'fit-content',
          ...
          <div style={{ marginTop: 12 }}>
            <window.DialogBox compact kindLabel="세컨비" line="관계·건강 별과 이어지는 통화예요. 사람을 만나면 충전되는 결이 한 번 더 보였어요."
            head={<window.SbHead size={32} track={false} />} />
          </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '0 28px', textAlign: 'center' }}>
        <div style={{ position: 'relative', width: 150, height: 150, display: 'grid', placeItems: 'center' }}>
          {phase === 'rec' && <div style={{ position: 'absolute', inset: 0, borderRadius: 0, border: '2px solid rgba(255,90,90,.4)', animation: 'sb-ping 1.6s ease-out infinite' }} />}
          <div style={{ width: 120, height: 120, borderRadius: 0, display: 'grid', placeItems: 'center',
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 4, alignItems: 'center', height: 30 }}>
              {[14, 24, 32, 20, 28, 16, 26, 18, 30, 14].map((h, i) => (
                <span key={i} style={{ width: 3, height: h, borderRadius: 0, background: C('error'), animation: `sb-pulse 0.8s ${i * 0.08}s ease-in-out infinite` }} />
              ))}
            </div>
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%', maxWidth: 290, marginTop: 4, textAlign: 'left' }}>
              {[
                ...
                <div key={ic} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 12px', borderRadius: 0, background: C('surface-container-highest') }}>
  ...
  return (
    <ScreenPad>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, margin: '8px 0 4px' }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 0, background: C('tertiary-container'), color: C('on-tertiary-container') }}>L3</span>
      ...
      <MdCard variant="outlined" style={{ padding: 16 }}>
        <div style={{ position: 'relative', width: '100%', aspectRatio: '1 / 1', borderRadius: 0, overflow: 'hidden',
          ...
          <div style={{ position: 'absolute', left: `${x}%`, top: `${y}%`, transform: 'translate(-50%,-50%)' }}>
            <div style={{ width: 18, height: 18, borderRadius: 0, background: C('primary'), boxShadow: 'none' }} />
          </div>
          <span style={{ position: 'absolute', left: '50%', bottom: 4, transform: 'translateX(-50%)', fontSize: 10, color: C('on-surface-variant') }}>← 회피 낮음 · 높음 →</span>
        </div>
...
function NorthStarEditor({ t, go }) {
  ...
  return (
    <ScreenPad>
      ...
      {/* current */}
      <div style={{ position: 'relative', borderRadius: 0, padding: 18, overflow: 'hidden',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
          <span style={{ width: 12, height: 12, borderRadius: 0, background: 'radial-gradient(circle,var(--c08),var(--ds-nebula) 84%)', boxShadow: 'none' }} />
          <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, letterSpacing: '.12em', color: 'var(--ds-polaris)' }}>NORTH STAR</span>
        </div>
        <textarea value={val} onChange={(e) => setVal(e.target.value)} rows={2}
          style={{ width: '100%', resize: 'none', border: 'none', background: 'transparent', outline: 'none',
            color: 'var(--c07)', fontFamily: 'var(--md-ref-typeface-plain)', fontSize: 24, fontWeight: 700, lineHeight: 1.4 }} />
      </div>
        ...
        {items.map((it, i) => (
          <MdCard key={i} variant="filled" onClick={() => go(it.route)} style={{ padding: 14 }}>
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 0, flex: '0 0 auto', display: 'grid', placeItems: 'center',
