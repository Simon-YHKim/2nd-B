  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {targets.map((tg) => {
          ...
          return (
            ...
            style={{ position: 'relative', textAlign: 'left', padding: 12, borderRadius: 0, cursor: 'pointer',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 14, padding: '12px 14px',
        borderRadius: 0, background: C('surface-container-highest') }}>
      ...
      {/* ops_push consent */}
      <div style={{ display: 'flex', gap: 10, marginTop: 14, padding: '11px 13px', borderRadius: 0,
    ...
    <ScreenPad>
      ...
      <MdCard variant="elevated" style={{ padding: 16, marginTop: 4,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <window.PxChart type="ringProgress" data={Math.round(doneN / routines.length * 100)} label={`오늘의 루틴 ${doneN}/${routines.length}`}
          options={{ w: 24, h: 24, scale: 2 }} style={{ minHeight: 0, width: 58, height: 58, flex: '0 0 auto' }} />
          <div style={{ flex: 1 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: 'var(--ds-ember)' }}>
              <Icon name="local_fire_department" fill size={22} />
              <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 24, fontWeight: 700 }}>12</span>
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
        ...
        style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left',
          padding: '12px 14px', borderRadius: 0, cursor: 'pointer', border: 'none',
      ...
      <MdCard variant="elevated" style={{ padding: 16,
        ...
        <div style={{ marginTop: -22 }}>
          <window.DialogBox compact kindLabel={`${adv.star} 별 · 오늘 하나`} title={adv.headline} line={`${adv.read} ${adv.detail}`}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginTop: 14 }}>
          ...
          {adv.evidence.map((e) =>
          <span key={e} className="md-label-small" style={{ padding: '3px 9px', borderRadius: 0, whiteSpace: 'nowrap',
...
function FirstInsight({ onDone }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 60, overflow: 'hidden',
      ...
      <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, letterSpacing: '.2em', color: 'var(--c11)', marginBottom: 6 }}>FIRST LIGHT · 첫 빛</div>
      <div className="md-title-medium" style={{ color: 'var(--c07)', marginBottom: 18 }}>당신의 첫 별이 켜졌어요</div>
      ...
      <div style={{ position: 'relative', width: 220, height: 92, marginBottom: 8, display: 'grid', placeItems: 'center' }}>
        <div style={{ position: 'relative' }}>
          <span style={{ display: 'block', width: 15, height: 15, borderRadius: 0, background: 'var(--c07)',
            boxShadow: 'none', animation: 'sb-pulse 2.4s ease-in-out infinite' }} />
          <span style={{ position: 'absolute', top: 22, left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap',
            fontSize: 12, color: 'var(--c05)', fontFamily: 'var(--md-ref-typeface-mono)', letterSpacing: '.06em' }}>북극성</span>
        </div>
      </div>
      ...
      <React.Fragment>
          ...
          <div className="md-title-medium" style={{ color: 'var(--c07)', maxWidth: 280, wordBreak: 'keep-all', lineHeight: 1.5 }}>
            당신은 <span style={{ color: 'var(--ds-core)' }}>‘먼저 다가가는’</span> 사람일지도 몰라요.
          </div>
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '0 auto', background: 'none', border: 'none', cursor: 'pointer',
            color: 'rgba(190,225,255,.8)', fontSize: 12, fontWeight: 700 }}>
              <Icon name="target" size={15} />이렇게 본 이유 2가지
          ...
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left' }}>
            ...
            map((e, i) =>
            <div key={i} style={{ padding: '10px 12px', borderRadius: 0, background: 'rgba(127,182,255,.1)', border: '1px solid rgba(127,182,255,.2)' }}>
                    <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, letterSpacing: '.04em', color: 'var(--c11)', marginBottom: 3 }}>{e.q}</div>
                    <div className="md-body-small" style={{ color: 'var(--c06)', wordBreak: 'keep-all' }}>{e.a}</div>
                  </div>
            )}
                <div className="md-label-small" style={{ color: 'rgba(190,225,255,.55)', textAlign: 'center', marginTop: 2, wordBreak: 'keep-all' }}>
                  아직 가입 답변뿐이라 ‘첫 신호’예요. 담을수록 이유가 늘어요.
                </div>
              </div>
          }
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 22, width: '100%', maxWidth: 320 }}>
            ...
            <MdButton variant="outlined" style={{ flex: 1, color: 'var(--c05)', borderColor: 'rgba(180,200,255,.4)' }} onClick={ratify}>조금 달라요</MdButton>
          </div>
      ...
      <React.Fragment>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 16px', borderRadius: 0,
          background: 'rgba(130,216,246,.16)', color: 'var(--c11)', marginBottom: 14 }}>
            <Icon name="trending_up" size={16} /><span style={{ fontWeight: 700, fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12 }}>관계 · L1 → L2</span>
          </div>
          <div className="md-title-medium" style={{ color: 'var(--c07)', maxWidth: 280, wordBreak: 'keep-all', lineHeight: 1.5 }}>
            관계 별이 한 단계 밝아졌어요.
          </div>
