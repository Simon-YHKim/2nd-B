  ...
  return (
    <div style={{ flex: 1, height: '100%', minHeight: 320, display: 'flex', flexDirection: 'column', alignItems: 'center',
      ...
      <div style={{ position: 'relative', width: 132, height: 132, display: 'grid', placeItems: 'center' }}>
        {/* breathing halo */}
        <div style={{ position: 'absolute', width: 104, height: 104, borderRadius: 0, filter: 'none',
        ...
        <div style={{ position: 'absolute', inset: 9, borderRadius: 0, border: '1px dashed rgba(126,180,255,.18)' }} />
        ...
        <div style={{ position: 'absolute', inset: 0, animation: 'sb-spin 2.4s linear infinite' }}>
          ...
          <div key={i} style={{ position: 'absolute', inset: 0, transform: `rotate(${i * 60}deg)` }}>
              ...
              width: 7 - i * 0.5, height: 7 - i * 0.5, borderRadius: 0, background: 'var(--ds-star)',
              opacity: 1 - i * 0.15, boxShadow: 'none' }} />
            </div>
          )}
        </div>
        {/* head */}
        <div style={{ animation: 'sb-bob 3.4s ease-in-out infinite' }}>
          <img src={window.SB_HEAD['head-front']} alt="세컨비" style={{ width: 72, height: 72, display: 'block',
            filter: 'none' }} />
        </div>
      </div>
...
function EmptyState({ icon = 'inbox', title, body, cta, onCta }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      ...
      <div style={{ width: 88, height: 88, borderRadius: 0, display: 'grid', placeItems: 'center',
...
const STAR_BG = 'radial-gradient(120% 62% at 50% 1%, rgba(40,86,150,.40), transparent 56%), radial-gradient(86% 56% at 85% 13%, rgba(120,96,210,.22), transparent 58%), var(--ds-space)';
  ...
  return (
    <div aria-hidden="true" style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}>
      ...
      <span key={i} className="sc-tw" style={{ position: 'absolute', left: `${st.x}%`, top: `${st.y}%`, width: st.r * 2, height: st.r * 2,
        borderRadius: 0, background: 'var(--c06)', opacity: st.o, boxShadow: 'none', '--d': `${st.d}s`, animationDelay: `${st.delay}s` }} />
      )}
    </div>);
});
...
function FinanceLens({ C }) {
  ...
  { k: '여가/취미', amt: 238000, pct: 19, color: 'var(--warn)' },
  ...
  return (
    <React.Fragment>
      ...
      <MdCard variant="outlined" style={{ padding: 16 }}>
        ...
        {/* 카테고리별 */}
        <div style={{ display: 'flex', height: 14, borderRadius: 0, overflow: 'hidden', gap: 2, marginTop: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
          ...
          <div key={c.k} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 9, height: 9, borderRadius: 0, background: c.color, flex: '0 0 auto' }} />
      ...
      <MdCard variant="outlined" style={{ padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 24, fontWeight: 700, color: C('primary') }}>{saveRate}%</div>
          </div>
          <div style={{ flex: '0 0 auto', textAlign: 'right' }}>
            <div className="md-label-small" style={{ color: C('on-surface-variant') }}>목표</div>
            <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 15, color: C('on-surface-variant') }}>25%</div>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {flow.map((f, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 12px', borderRadius: 0, background: C('surface-container-highest') }}>
                ...
                <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, fontWeight: 700, color: col, flex: '0 0 auto' }}>
      ...
      <MdCard variant="filled" style={{ background: C('surface-container-high'), padding: 14 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {[{ k: '불안', pct: 40, c: 'var(--warn)' }, { k: '충동', pct: 25, c: 'var(--ds-ember)' }, { k: '즐거움', pct: 20, c: C('primary') }, { k: '계획', pct: 15, c: C('tertiary') }].map((s) =>
          <span key={s.k} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap',
            padding: '4px 10px', borderRadius: 0, background: `${s.c}1f`, color: s.c }}>
              <span style={{ width: 7, height: 7, borderRadius: 0, background: s.c }} />{s.k} {s.pct}%
            </span>
          )}
        </div>
      </MdCard>
...
function CareerLens({ C }) {
  ...
  const KIND = {
    ...
    mil: { label: '병역', icon: 'shield', color: 'var(--ok)' },
    award: { label: '수상', icon: 'emoji_events', color: 'var(--warn)' },
    cert: { label: '자격', icon: 'verified', color: 'var(--ds-nebula)' },
    job: { label: '경력', icon: 'badge', color: C('tertiary') } };

  // 트랙: 경력 하위 프로젝트와 비-경력 항목을 '메인(회사 업무)·개인(사이드)'으로 나눈다.
  const TRACK = {
    main: { label: '메인', color: C('tertiary') },
    personal: { label: '사이드', color: 'var(--sb-mood-negative)' } };
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ display: 'flex', gap: 4, padding: 4, borderRadius: 0, background: C('surface-container-high'), marginBottom: 10 }}>
        {TABS.map((t) => {
          const on = tab === t.id;
          return (
            <button key={t.id} type="button" onClick={() => setTab(t.id)} className="md-interactive"
              style={{ position: 'relative', flex: 1, padding: '8px 0', borderRadius: 0, border: 'none', cursor: 'pointer',
                background: on ? C('secondary-container') : 'transparent', color: on ? C('on-secondary-container') : C('on-surface-variant'),
                fontWeight: on ? 700 : 600, fontSize: 12, fontFamily: 'inherit' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
        {Object.keys(KIND).map((kk) => {
          const k = KIND[kk];
          return (
            <span key={kk} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 700, padding: '3px 8px 3px 6px', borderRadius: 0, whiteSpace: 'nowrap',
          ...
          return (
            <div key={it.id} style={{ display: 'flex', gap: 12 }}>
              ...
              <div style={{ flex: '0 0 auto', width: 24, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ marginTop: 2, width: 24, height: 24, borderRadius: 0, flex: '0 0 auto', zIndex: 1,
                  ...
                  <Icon name={k.icon} size={14} style={{ color: it.now ? 'var(--ds-space-2)' : k.color }} />
                </div>
              ...
              <div style={{ flex: 1, minWidth: 0, paddingBottom: last ? 0 : 14 }}>
                <RowTag type={hasDetail ? 'button' : undefined} onClick={hasDetail ? () => setOpen(isOpen ? null : it.id) : undefined}
                  ...
                  style={{ position: 'relative', width: '100%', textAlign: 'left', border: 'none', background: 'transparent', cursor: hasDetail ? 'pointer' : 'default', padding: '2px 2px 2px 0', borderRadius: 0, display: 'block' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 0, whiteSpace: 'nowrap',
                ...
                {hasDetail && isOpen && (isJob
                        ...
                        return (
                          <div key={tk}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                              <span style={{ width: 7, height: 7, borderRadius: 0, background: t.color, flex: '0 0 auto' }} />
                              <span style={{ fontSize: 12, fontWeight: 700, color: t.color }}>{t.label}</span>
                              <span style={{ fontSize: 10, color: C('on-surface-variant') }}>{ps.length}</span>
                            </div>
                            <div style={{ marginLeft: 3, paddingLeft: 12, borderLeft: `1.5px solid ${t.color}40`, display: 'flex', flexDirection: 'column', gap: 8 }}>
                              {ps.map((p, di) =>
                                <div key={di} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                                  <span style={{ width: 6, height: 6, borderRadius: 0, flex: '0 0 auto', marginTop: 6, background: t.color }} />
                  ...
                  : (/* 비-경력: 단순 목록 */
                    <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8, paddingLeft: 2 }}>
                      {it.details.map((p, di) =>
                        <div key={di} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                          <span style={{ width: 6, height: 6, borderRadius: 0, flex: '0 0 auto', marginTop: 6, background: k.color }} />
                          <div style={{ minWidth: 0 }}>
                            <div className="md-body-medium" style={{ color: C('on-surface'), wordBreak: 'keep-all' }}>{p.name}</div>
                            {p.note && <div className="md-body-small" style={{ color: C('on-surface-variant'), wordBreak: 'keep-all' }}>{p.note}</div>}
                          </div>
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
        ...
        <span style={{ width: 3, height: 3, borderRadius: 0, background: C('outline') }} />
          ...
          return (
            <div key={d} style={{ display: 'flex', gap: 12 }}>
              ...
              <div style={{ flex: '0 0 auto', width: 16, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ marginTop: 5, width: isCur ? 13 : 10, height: isCur ? 13 : 10, borderRadius: 0, flex: '0 0 auto', zIndex: 1,
                ...
                <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {subs.map((s, si) => {
                    ...
                    return (
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 11px', borderRadius: 0, width: '100%', textAlign: 'left',
                        ...
                        <span style={{ flex: '0 0 auto', width: 6, height: 6, borderRadius: 0, background: future ? C('outline') : C('primary') }} />
...
function LeisureSpark({ hist, color, dot }) {
  return <window.PxChart type="area" data={hist} options={{ w: 96, h: 22, scale: 3 }} style={{ minHeight: 0, width: '100%' }} />;
}

function LeisureLens({ C }) {
  const LEAK = C('error'),ANCHOR = 'var(--warn)';
  ...
  const VP_H = 250;
  const ctrlBtn = { width: 32, height: 32, borderRadius: 0, border: `1px solid ${C('outline-variant')}`, background: C('surface-container'), color: C('on-surface'), display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 15, lineHeight: 1, fontWeight: 400, padding: 0 };
  const segBtn = (active) => ({ border: 'none', background: active ? C('primary') : 'transparent', color: active ? C('on-primary') : C('on-surface-variant'), fontFamily: 'inherit', fontSize: 12, fontWeight: 700, padding: '5px 10px', borderRadius: 0, cursor: 'pointer' });
  ...
  return (
    <MdCard variant="outlined" style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        ...
        <span style={{ flex: 1 }} />
        <div style={{ display: 'flex', background: C('surface-container'), border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: 2, gap: 2 }}>
      ...
      style={{ position: 'relative', width: '100%', height: VP_H, borderRadius: 0, background: C('surface-container-highest'), overflow: 'hidden', touchAction: 'none', cursor: drag.current && moved.current ? 'grabbing' : 'grab' }}>
          ...
          {acts.map((a) => {
            ...
            return (
              ...
              style={{ position: 'absolute', left: `${st.x * 100}%`, top: `${(1 - st.y) * 100}%`, transform: `translate(-50%,-50%) scale(${1 / z})`, transformOrigin: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, justifyContent: 'center', minWidth: 44, minHeight: 44, padding: 6, border: 'none', background: 'transparent', cursor: 'pointer' }}>
                {ring &&
                <span style={{ position: 'absolute', bottom: `calc(50% + ${dia / 2 + 8}px)`, left: '50%', transform: 'translateX(-50%)', fontSize: 10, fontWeight: 700, whiteSpace: 'nowrap', color: ring, textShadow: `0 1px 3px ${C('surface-container-highest')}` }}>{isLeak ? '⚠ 누수' : '★ 앵커'}</span>}
                <span style={{ position: 'relative', width: dia, height: dia, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {ring && <span style={{ position: 'absolute', width: dia + 12, height: dia + 12, borderRadius: 0, border: `2px solid ${ring}`, boxSizing: 'border-box' }} />}
                  <span style={{ width: dia, height: dia, borderRadius: 0, boxSizing: 'border-box', background: filled ? C('primary') : 'transparent', border: filled ? 'none' : `2px solid ${C('primary')}`, boxShadow: on ? `0 0 0 4px color-mix(in srgb, ${C('primary')} 28%, transparent), 0 0 12px ${C('primary')}` : filled ? `0 0 8px ${C('primary')}` : 'none' }} />
                </span>
                <span style={{ fontSize: 12, fontWeight: on ? 800 : 600, whiteSpace: 'nowrap', color: on ? C('primary') : C('on-surface'), textShadow: `0 1px 3px ${C('surface-container-highest')}` }}>{a.nm}</span>
              </button>);
        ...
        <span key={tx} style={{ position: 'absolute', fontSize: 10, fontWeight: 700, color: C('on-surface-variant'), pointerEvents: 'none', ...s }}>{tx}</span>)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 }}>
        ...
        style={{ width: 34, height: 34, flex: 'none', borderRadius: 0, border: 'none', background: C('primary'), color: C('on-primary'), cursor: 'pointer', fontSize: 12, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingLeft: playRef.current ? 0 : 2 }}>{playRef.current ? '⏸' : '▶'}</button>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: C('primary'), whiteSpace: 'nowrap', minWidth: 58, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmtD(dateAt(t))}</span>
      </div>

      {/* legend */}
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '4px 14px', marginTop: 8, fontSize: 12, color: C('on-surface-variant') }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>크기 = 빈도
          <span style={{ display: 'inline-flex', alignItems: 'flex-end', gap: 3 }}>
            <span style={{ width: 7, height: 7, borderRadius: 0, background: C('primary') }} />
            <span style={{ width: 11, height: 11, borderRadius: 0, background: C('primary') }} />
            <span style={{ width: 15, height: 15, borderRadius: 0, background: C('primary') }} /></span></span>
        <span style={{ color: LEAK, fontWeight: 700 }}>⚠ 누수</span>
        <span style={{ color: ANCHOR, fontWeight: 700 }}>★ 앵커</span>
        <span>· 자동 표시</span>
      </div>
      <div style={{ fontSize: 12, color: C('outline'), marginTop: 6 }}>기록 시작 · {fmtD(dates.current.start)} → 오늘 {fmtD(dates.current.today)}</div>
      ...
      <div style={{ overflow: 'hidden', transition: 'max-height .28s ease, opacity .2s ease, margin-top .2s ease', maxHeight: selAct ? 380 : 0, opacity: selAct ? 1 : 0, marginTop: selAct ? 12 : 0 }}>
        {selAct &&
        <div style={{ borderRadius: 0, background: C('surface-container'), padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 11, height: 11, borderRadius: 0, background: selAct.id === anchorId ? ANCHOR : selAct.id === leakId ? LEAK : C('primary') }} />
              <span style={{ fontSize: 15, fontWeight: 700, color: C('on-surface') }}>{selAct.nm}</span>
              <span style={{ flex: 1 }} />
              <button type="button" onClick={() => setSel(null)} style={{ border: 'none', background: 'transparent', color: C('on-surface-variant'), cursor: 'pointer', display: 'flex', padding: 2, fontSize: 15, lineHeight: 1 }}>×</button>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 9 }}>
              {[selState.y >= .5 ? '채움' : '비움', selState.x >= .5 ? '함께' : '혼자', selAct.freq].map((c, i) =>
            <span key={i} style={{ fontSize: 12, fontWeight: 700, padding: '3px 9px', borderRadius: 0, background: i === 0 ? C('secondary-container') : C('surface-container-highest'), color: i === 0 ? C('on-secondary-container') : C('on-surface-variant') }}>{c}</span>)}
            </div>
            <div className="md-body-small" style={{ color: C('on-surface-variant'), marginTop: 10, wordBreak: 'keep-all' }}>{selAct.note}</div>
            {/* 기쁨 추세 */}
            <div style={{ background: C('surface-container-highest'), borderRadius: 0, padding: '11px 12px', marginTop: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: C('on-surface-variant') }}>기쁨 추세 · 최근 6회</span>
                <span style={{ fontSize: 15, fontWeight: 700, color: C('primary') }}>{Math.round(selState.joy)}</span>
              </div>
              <LeisureSpark hist={selAct.hist} color={C('primary')} dot={C('surface-container-highest')} />
            </div>
            {/* 지금 기쁨 기록 */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: C('on-surface-variant'), whiteSpace: 'nowrap' }}>지금 기쁨 기록</span>
              ...
              <span style={{ fontSize: 15, fontWeight: 700, color: C('primary'), width: 28, textAlign: 'right' }}>{logJoy}</span>
            </div>
          </div>}
      </div>
      ...
      <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C('outline-variant')}` }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          ...
          <span key={h} style={{ fontSize: 12, fontWeight: 700, color: C('on-secondary-container'), padding: '6px 12px', borderRadius: 0, background: C('secondary-container') }}>{h} <span style={{ color: C('on-surface-variant'), fontWeight: 400 }}>· {why}</span></span>)}
        </div>
      </div>
    </MdCard>);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <MdCard key={i} variant="outlined" style={{ padding: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 0, background: C('secondary-container'), color: C('on-secondary-container') }}>
...
function starInsight(id, C, meta) {
  const W = { color: 'var(--c08)' },P = { color: C('primary') };
  switch (id) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
        {[1, 2, 3, 4, 5].map((i) =>
        <span key={i} style={{ width: 11, height: 4, borderRadius: 0, background: i <= level ? 'var(--ds-core)' : 'rgba(127,178,255,.2)', boxShadow: i <= level ? '0 0 6px rgba(95,212,255,.7)' : 'none' }} />
        )}
      </span>
  ...
  return shell(
    <div style={{ padding: '0 16px 24px' }}>
      ...
      <div style={{ paddingTop: 12 }}>
        <window.DialogBox compact kindLabel="세컨비" line={starInsight(star.id, C, meta)}
...
function IdenScreen({ t, go }) {
  /* 파일명: 닉네임_페르소나.iden (예: 홍길동_최선을 다하는 의사.iden) */
  const idenFileName = () => {
    const p = window.SBProfile ? window.SBProfile.get() : null;
    const nick = (p && (p.handle || p.name)) || '나';
    const list = (window.SB_PERSONAS_PUBLIC || []);
    const role = (list[0] && list[0].role) || (p && p.role) || '나를 쌓는 사람';
    return String(nick).replace(/^@/, '') + '_' + role + '.iden';
  };
  const C = window.SB.C;
  ...
  const targets = [
  { k: 'ChatGPT', c: '#10A37F' }, { k: 'Claude', c: 'var(--accent)' },
    ...
    <ScreenPad>
      ...
      <MdCard variant="elevated" style={{ padding: 0, overflow: 'hidden', marginTop: 4 }}>
        <div style={{ padding: 18, background: C('tertiary-container'), textAlign: 'center' }}>
          {/* 아바타 + 파일명(닉네임_페르소나.iden) */}
          <div style={{ width: 56, height: 56, borderRadius: 0, margin: '0 auto 10px', display: 'grid', placeItems: 'center',
            background: C('surface'), overflow: 'hidden' }}>
            {window.SbAvatar
              ? <window.SbAvatar spec={(window.SBProfile && window.SBProfile.get().avatar) || null} size={56} crop />
              : <Icon name="badge" fill size={30} style={{ color: C('on-tertiary-container') }} />}
          </div>
          <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 15, color: C('on-tertiary-container'), wordBreak: 'break-all' }}>{idenFileName()}</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginTop: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 0, background: C('surface'), color: C('on-surface-variant') }}>v2.1</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 0, background: C('primary'), color: C('on-primary') }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        ...
        <MdCard key={tg.k} variant="outlined" onClick={() => go('connect')} style={{ padding: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 30, height: 30, borderRadius: 0, flex: '0 0 auto', display: 'grid', placeItems: 'center',
              background: tg.c, color: 'var(--c08)', fontWeight: 700, fontSize: 15 }}>{tg.k[0]}</span>
          ...
          return (
            <MdCard key={s.id} variant="outlined" style={{ padding: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 42, height: 42, borderRadius: 0, display: 'grid', placeItems: 'center', flex: '0 0 auto',
  ...
  return (
    <ScreenPad>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        style={{ padding: 16, border: tr.cur ? `2px solid ${C('primary')}` : undefined }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
              ...
              {tr.cur && <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 0, background: C('primary'), color: C('on-primary') }}>이용 중</span>}
              <span style={{ flex: 1 }} />
      ...
      style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', textAlign: 'left', cursor: 'pointer',
        padding: '14px 16px', borderRadius: 0, background: C('surface-container'),
        ...
        <div style={{ flex: 1 }}>
          <div className="md-title-small" style={{ fontWeight: 700 }}>광고로 담기 가속 충전</div>
  ...
  return (
    <div style={{ overflowY: 'auto', height: '100%' }}>
      <div style={{ padding: '4px 16px 18px' }}>
        ...
        <MdCard variant="filled" onClick={() => go('profilesetup')} style={{ padding: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ width: 48, height: 48, display: 'grid', placeItems: 'center', flex: '0 0 auto',
              background: C('surface-container-highest'), boxShadow: 'var(--ds-edge)' }}>
              <window.SbAvatar spec={window.SBProfile.get().avatar} size={40} />
            </span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="md-body-large" style={{ color: C('on-surface') }}>{window.SBProfile.get().name}</div>
              <div className="md-body-small" style={{ color: C('on-surface-variant') }}>
                @{window.SBProfile.get().handle}{window.SBProfile.job() ? ' · ' + window.SBProfile.job().ko : ''}
              </div>
            </div>
        ...
        <MdCard variant="filled" style={{ padding: 4 }}>
          ...
          <div style={{ borderTop: `1px solid ${C('outline-variant')}` }} />
          <div className="md-interactive" onClick={() => go('applock')}
          style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px 12px', cursor: 'pointer', minHeight: 48 }}>
            <span className="md-state" />
            <Icon name="lock" size={22} style={{ color: C('on-surface-variant') }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="md-body-large" style={{ color: C('on-surface') }}>사용자 인증</div>
              <div className="md-body-small" style={{ color: C('on-surface-variant') }}>
                {f.authMethod === 'pin' ? 'PIN 번호' : f.authMethod === 'pattern' ? '패턴' : f.authMethod === 'password' ? '비밀번호' : f.authMethod === 'bio' ? '생체 인증' : '사용 안 함'}
              </div>
            </div>
            <Icon name="chevron_right" size={20} style={{ color: C('on-surface-variant') }} />
          </div>
          <div style={{ borderTop: `1px solid ${C('outline-variant')}` }} />
          <ToggleRow icon="badge" label="기기에서 먼저 처리" sub="원문은 기기에서만 분석" checked={f.ondevice !== false} onChange={(v) => setF('ondevice', v)} />
          <div style={{ borderTop: `1px solid ${C('outline-variant')}` }} />
        ...
        {/* 연동 */}
        <SectionLabel action={<MdButton variant="text" size="s" trailingIcon="chevron_right" onClick={() => go('connect')}>전체</MdButton>}>데이터 연동</SectionLabel>
        <MdCard variant="filled" style={{ padding: 4 }}>
          {sources.map((s, i) =>
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 12px',
            borderTop: i ? `1px solid ${C('outline-variant')}` : 'none' }}>
              <div style={{ width: 32, height: 32, borderRadius: 0, flex: '0 0 auto', display: 'grid', placeItems: 'center',
              background: conn[s.id] ? C('primary') : C('surface-container-highest'), color: conn[s.id] ? C('on-primary') : C('on-surface-variant') }}>
                <Icon name={s.icon} fill={!!conn[s.id]} size={18} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <MdButton variant={conn[s.id] ? 'tonal' : 'outlined'} size="s" icon={conn[s.id] ? 'check' : undefined}
              style={{ minWidth: 78, flex: '0 0 auto' }} onClick={() => setConn(s.id, !conn[s.id])}>
                {conn[s.id] ? '연결됨' : '연결'}
              </MdButton>
            </div>
          )}
        ...
        <MdCard variant="filled" style={{ padding: 4 }}>
          ...
          { icon: 'shield_person', label: '내 데이터 리뷰', sub: '저장된 것 보기·지우기', route: 'datareview' }].
          map((row, i) =>
          <div key={row.label} className="md-interactive" onClick={() => go(row.route)}
          style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px 12px', borderRadius: 0,
        ...
        <MdCard variant="filled" style={{ padding: 4 }}>
          ...
          { icon: 'person', label: '북극성 · 나의 종합', sub: '7별 종합 · 검사', route: 'me' },
        ...
        {/* 소식 · 기능 (신기능) */}
        <SectionLabel>소식 · 기능</SectionLabel>
        <MdCard variant="filled" style={{ padding: 4 }}>
          <div className="md-interactive" onClick={() => go('notices')}
          style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px 12px', cursor: 'pointer' }}>
            <span className="md-state" />
            <Icon name="campaign" size={22} style={{ color: C('on-surface-variant') }} />
            <div style={{ flex: 1 }}>
              <div className="md-body-large" style={{ color: C('on-surface') }}>공지사항</div>
              <div className="md-body-small" style={{ color: C('on-surface-variant') }}>패치노트 · 개발자 소식</div>
            </div>
            {window.SBNotices && window.SBNotices.unread() > 0 &&
            <span style={{ minWidth: 18, height: 18, padding: '0 5px', display: 'grid', placeItems: 'center', background: C('primary'), color: C('on-primary'),
              fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, boxShadow: 'var(--ds-edge)' }}>{window.SBNotices.unread()}</span>}
            <Icon name="chevron_right" size={20} style={{ color: C('on-surface-variant') }} />
          </div>
          <div className="md-interactive" onClick={() => go('reasoning')}
          style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px 12px', cursor: 'pointer', borderTop: `1px solid ${C('outline-variant')}` }}>
            <span className="md-state" />
            <Icon name="bolt" size={22} style={{ color: C('on-surface-variant') }} />
            <div style={{ flex: 1 }}>
              <div className="md-body-large" style={{ color: C('on-surface') }}>리즈닝</div>
              <div className="md-body-small" style={{ color: C('on-surface-variant') }}>자동 실행 · 선택 실행 · 주 2회</div>
            </div>
            <window.QuotaPips left={window.SBReasoning ? window.SBReasoning.state.left : 0} small />
            <Icon name="chevron_right" size={20} style={{ color: C('on-surface-variant') }} />
          </div>
        </MdCard>
        ...
        <MdCard variant="filled" style={{ padding: 4 }}>
          {[
          { icon: 'person', label: '계정', sub: '프로필 · 데이터 · IDEN', route: 'account' },
          { icon: 'workspace_premium', label: '구독 관리', sub: '결제 상태 · 해지 · 환불 요청', route: 'subscription' },
          { icon: 'palette', label: '테마 · 글꼴', sub: '보기 편한 테마와 글꼴', route: 'theme' },
        ...
        <MdCard variant="filled" style={{ padding: 4 }}>
          ...
          onClick={() => {
            if (row.danger) setConfirm({ title: '정말 계정을 삭제할까요?', danger: true, confirmLabel: '영구 삭제',
              ...
              body: '계정과 모든 별·기록·분석 결과가 완전히 지워져요. 이 작업은 되돌릴 수 없어요.',
              onConfirm: () => {e.showToast && e.showToast({ msg: '삭제 요청을 받았어요' });e.logout && e.logout();} });else
            setConfirm({ title: '로그아웃할까요?', confirmLabel: '로그아웃',
              body: '다시 로그인하면 별과 기록은 그대로 이어져요.',
              onConfirm: () => e.logout && e.logout() });
          }}
          style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px 12px', borderRadius: 0,
            borderTop: i ? `1px solid ${C('outline-variant')}` : 'none', cursor: 'pointer' }}>
              <span className="md-state" />
...
/* ===================== REASONING 리즈닝 (신기능 A · PIXEL-CLAY) ===================== */
function ReasoningScreen({ t, go, env, onBack }) {
  const C = window.SB.C;
  const RS = window.SBReasoning;
  const [, force] = useSt(0);
  React.useEffect(() => RS.subscribe(() => force((x) => x + 1)), []);
  const [sel, setSel] = useSt([]);
  const st = RS.state;
  const mats = RS.MATERIALS;
  const running = st.running;
  const depleted = st.left <= 0 && !running;
  const selMode = sel.length > 0 && !running && !depleted;
  const toggleSel = (id) => {if (running || depleted) return;setSel((s) => s.includes(id) ? s.filter((x) => x !== id) : [...s, id]);};
  const runSel = () => {if (RS.run(sel, env)) setSel([]);};
  const quotaSub = running ? '분석 중이에요' : st.left === 0 ? '0회 남음 · 일요일 초기화' : st.left === 1 ? '1회 사용됨 · 1회 남음' : selMode ? `실행하면 1회 사용 · 2회 중` : '일요일에 2회로 채워져요';
  const rows = running ? running.list : mats;
  const rowState = (m, i) => {
    if (running) return i < running.done ? 'done' : i === running.done ? 'now' : 'wait';
    return st.results[m.id] === 'done' ? 'done' : 'idle';
  };
  const QuotaRow = () =>
  <div className="px-frame" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 16px', background: C('surface-container-low'), marginTop: 12 }}>
      <Icon name="bolt" fill size={18} style={{ color: depleted ? C('error') : 'var(--ds-core)' }} />
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: C('on-surface') }}>이번 주 리즈닝</div>
        <div style={{ fontSize: 10, fontFamily: 'var(--font-micro)', color: C('on-surface-variant'), marginTop: 1 }}>{quotaSub}</div>
      </div>
      <window.QuotaPips left={st.left} />
    </div>;
  const Note = ({ icon, children }) =>
  <div className="px-frame" style={{ display: 'flex', gap: 9, alignItems: 'flex-start', padding: '12px 15px', background: C('surface-container-low'), marginTop: 12 }}>
      <Icon name={icon} size={16} style={{ color: 'var(--ds-core)', marginTop: 1 }} />
      <span style={{ fontSize: 12, color: C('on-surface-variant'), lineHeight: 1.5, wordBreak: 'keep-all' }}>{children}</span>
    </div>;
  const AutoCard = () =>
  <MdCard variant="filled" style={{ padding: 4, background: C('surface-container'), marginTop: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px' }}>
        <Icon name="auto_awesome" fill={st.auto} size={20} style={{ color: st.auto ? C('primary') : C('on-surface-variant') }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="md-body-large" style={{ color: C('on-surface') }}>자동 리즈닝</div>
          <div className="md-body-small" style={{ color: depleted && st.auto ? C('error') : C('on-surface-variant') }}>
            {depleted && st.auto ? '다 썼어요 — 다음 주부터 다시 실행돼요' : '자료를 담을 때마다 세컨비가 바로 별을 이어요'}</div>
        </div>
        <MdSwitch checked={st.auto} onChange={(v) => RS.setAuto(v)} />
      </div>
    </MdCard>;
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: 56, padding: '6px 8px 6px 6px', flex: '0 0 auto' }}>
        <MdIconButton name={selMode ? 'close' : 'arrow_back'} title={selMode ? '선택 해제' : '뒤로'} onClick={selMode ? () => setSel([]) : onBack} />
        <span className="md-title-large" style={{ color: C('on-surface') }}>{selMode ? `${sel.length}개 선택됨` : '리즈닝'}</span>
      </div>
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 18px 10px' }}>
        {depleted &&
        <div className="px-bevel" style={{ background: C('surface-container'), boxShadow: 'var(--ds-edge), 0 0 0 6px var(--sunken), 0 0 0 8px var(--danger)', padding: 18, marginTop: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <Icon name="bolt" fill size={20} style={{ color: C('error') }} />
            <span style={{ fontSize: 15, fontWeight: 700, color: C('on-surface') }}>이번 주 리즈닝을 다 썼어요</span>
          </div>
          <p style={{ fontSize: 12, color: C('on-surface-variant'), lineHeight: 1.5, wordBreak: 'keep-all', margin: '0 0 16px' }}><b style={{ color: 'var(--c11)' }}>일요일</b>에 2회가 다시 채워져요. 지금 더 잇고 싶다면 아래에서 채울 수 있어요.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            <MdButton variant="filled" full icon="play_circle" onClick={() => RS.grant(1)}>광고 보고 1회 받기</MdButton>
            <MdButton variant="tonal" full icon="rocket_launch" onClick={() => go('plans')}>업그레이드 · 무제한</MdButton>
          </div>
        </div>}
        {running &&
        <MdCard variant="filled" style={{ padding: 16, marginTop: 12, background: C('surface-container') }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <SbHead size={48} expression="positive" track={false} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: C('on-surface') }}>별을 잇는 중이에요</div>
              <div style={{ fontSize: 12, color: C('on-surface-variant'), marginTop: 2 }}>선택한 {running.total}건을 읽고 있어요 · {Math.min(running.done + 1, running.total)} / {running.total}</div>
            </div>
          </div>
          <div style={{ marginTop: 14 }}><ProgressLinear value={Math.min(100, Math.round(running.pct))} /></div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 14 }}>
            {running.list.map((m, i) => {
              const rs = rowState(m, i);
              return (
                <span key={m.id} style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, padding: '5px 10px', display: 'inline-flex', alignItems: 'center', gap: 5,
                  background: rs === 'now' ? C('primary-container') : C('surface-container-high'),
                  boxShadow: rs === 'done' ? '0 0 0 2px var(--ok)' : 'var(--ds-edge)',
                  color: rs === 'done' ? 'var(--ok)' : rs === 'now' ? C('on-primary-container') : C('outline') }}>
                  {rs !== 'wait' && <Icon name={rs === 'done' ? 'check' : 'bolt'} size={16} />}{m.short}
                </span>);
            })}
          </div>
        </MdCard>}
        {!depleted && !running && <AutoCard />}
        {st.auto && !running && !depleted && <Note icon="bolt">자동 실행도 주간 횟수를 1회씩 써요. 횟수를 아끼려면 필요한 자료만 직접 실행하세요.</Note>}
        {running && <Note icon="info">지금 다른 화면을 봐도 돼요. 다 되면 알림으로 알려드릴게요.</Note>}
        {!running && <QuotaRow />}
        {depleted && <AutoCard />}
        <SectionLabel action={<span className="md-label-small" style={{ color: C('on-surface-variant') }}>{running ? '분석 중' : selMode ? `${sel.length} / ${mats.length} 선택` : '최근 · 미분석'}</span>}>담은 자료</SectionLabel>
        <MdCard variant="filled" style={{ padding: 4, background: C('surface-container') }}>
          {rows.map((m, i) => {
            const rs = rowState(m, i);
            const on = sel.includes(m.id);
            return (
              <div key={m.id} className={!running && !depleted ? 'md-interactive' : ''} onClick={() => toggleSel(m.id)}
              style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', cursor: !running && !depleted ? 'pointer' : 'default',
                background: on ? 'var(--c02)' : 'transparent', borderTop: i ? `1px solid ${C('outline-variant')}` : 'none' }}>
                {!running && !depleted && <span className="md-state" />}
                {!running && <span style={{ pointerEvents: 'none', display: 'inline-flex' }}><MdCheckbox checked={on} disabled={depleted} onChange={() => {}} /></span>}
                <span style={{ width: 32, height: 32, display: 'grid', placeItems: 'center', flex: '0 0 auto',
                  background: C('surface-container-high'), boxShadow: rs === 'done' ? '0 0 0 2px var(--ok)' : 'var(--ds-edge)' }}>
                  <Icon name={rs === 'done' ? 'check' : rs === 'now' ? 'bolt' : m.icon} size={16} style={{ color: rs === 'done' ? 'var(--ok)' : rs === 'now' ? C('primary') : 'var(--c11)' }} />
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, color: C('on-surface'), whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.title}</div>
                  <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 10, marginTop: 1, color: rs === 'done' ? 'var(--ok)' : rs === 'now' ? C('primary') : C('on-surface-variant') }}>
                    {rs === 'done' ? m.done || '별에 연결됨' : rs === 'now' ? '읽는 중…' : m.sub}</div>
                </div>
              </div>);
          })}
        </MdCard>
        {!st.auto && !running && !depleted && !selMode && <Note icon="info">자료를 선택하면 아래 실행 버튼이 켜져요.</Note>}
      </div>
      <div style={{ padding: '10px 18px 14px', flex: '0 0 auto' }}>
        {running ?
        <MdButton variant="tonal" full icon="stop_circle" onClick={() => RS.cancel()}>실행 취소</MdButton> :
        depleted ?
        <MdButton variant="filled" full disabled icon="bolt">이번 주 다 썼어요</MdButton> :
        selMode ?
        <MdButton variant="filled" full icon="bolt" onClick={runSel}>선택한 {sel.length}건 리즈닝</MdButton> :
        <MdButton variant="filled" full disabled icon="bolt">선택한 자료 리즈닝</MdButton>}
      </div>
    </div>);
}
window.ReasoningScreen = ReasoningScreen;
