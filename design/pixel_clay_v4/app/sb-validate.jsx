/* ============================================================
   ...
   ValuesScreen   : 가치관 검사 (Schwartz 기반, 핵심 가치 랭킹)
    ...
    <ScreenPad>
      ...
      <div style={{ position: 'relative', borderRadius: 0, padding: 18, overflow: 'hidden',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {top.map((v, i) => (
            <div key={v.k} style={{ flex: 1, textAlign: 'center', padding: '12px 6px', borderRadius: 0,
              background: i === 0 ? C('primary') : C('surface'), border: `1px solid ${i === 0 ? C('primary') : C('outline-variant')}` }}>
              <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, color: i === 0 ? C('on-primary') : C('on-surface-variant') }}>{i + 1}</div>
        ...
        {VALUES.map((v) => (
          <div key={v.k}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 5 }}>
              <span className="md-body-medium" style={{ color: C('on-surface'), fontWeight: 700 }}>{v.k}</span>
      ...
      <div style={{ marginTop: 26 }}>
        <window.DialogBox compact kindLabel="세컨비" line="자율성이 가장 높아요. 일·관계에서 ‘스스로 정하는’ 선택을 반복해 온 기록이 이 짐작을 받쳐요."
...
const RATIFY = [
  { id: 1, src: '통화 녹음', icon: 'mic', target: '관계 별', from: 'L2', to: 'L3', decision: '맞아요', when: '방금', note: '“사람 만나면 충전” 신호 확인' },
  { id: 2, src: '심층 인터뷰', icon: 'forum', target: '외향성', from: '41', to: '47', decision: '맞아요', when: '1시간 전', note: '먼저 다가가는 패턴 4건' },
  { id: 3, src: '기록 분석', icon: 'inventory_2', target: '리듬 별', from: 'L2', to: 'L3', decision: '나중에', when: '오늘', note: '이유가 더 필요해요' },
  { id: 4, src: '캘린더 연동', icon: 'workspaces', target: '건강 별', from: 'L1', to: 'L2', decision: '아니에요', when: '어제', note: '내가 동의하지 않은 추측' },
  { id: 5, src: '심층 인터뷰', icon: 'forum', target: '애착 유형', from: '—', to: '안정형', decision: '맞아요', when: '2일 전', note: 'ECR 12문항 응답' },
];
const DEC = {
  '맞아요': { c: 'var(--md-sys-color-primary)', bg: 'var(--md-sys-color-primary-container)', on: 'var(--md-sys-color-on-primary-container)', icon: 'check_circle' },
  '나중에': { c: 'var(--warn)', bg: 'rgba(247,185,85,.18)', on: 'var(--warn)', icon: 'replay' },
  '아니에요': { c: 'var(--md-sys-color-error)', bg: 'var(--md-sys-color-error-container)', on: 'var(--md-sys-color-on-error-container)', icon: 'close' },
};

function RatifyScreen({ t, go }) {
  const C = window.SB.C;
  const [filter, setFilter] = useVS('전체');
  const filters = ['전체', '맞아요', '나중에', '아니에요'];
  const list = RATIFY.filter((r) => filter === '전체' || r.decision === filter);
  const counts = { ok: RATIFY.filter((r) => r.decision === '맞아요').length, later: RATIFY.filter((r) => r.decision === '나중에').length, no: RATIFY.filter((r) => r.decision === '아니에요').length };

  return (
    <ScreenPad>
      <div className="md-headline-small" style={{ color: C('on-surface'), margin: '8px 0 4px' }}>확인 이력</div>
      ...
      <MdCard variant="filled" style={{ padding: 14, marginBottom: 14 }}>
        <div style={{ display: 'flex', textAlign: 'center' }}>
          {[['제안', RATIFY.length, C('on-surface')], ['맞아요', counts.ok, C('primary')], ['나중에', counts.later, 'var(--warn)'], ['아니에요', counts.no, C('error')]].map(([lb, n, col], i) => (
            <div key={lb} style={{ flex: 1, borderLeft: i ? `1px solid ${C('outline-variant')}` : 'none' }}>
              <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 24, fontWeight: 700, color: col }}>{n}</div>
          ...
          return (
            <MdCard key={r.id} variant="outlined" style={{ padding: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 0, background: d.bg, color: d.on }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, color: C('on-surface-variant') }}>
                  <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12 }}>{r.from}</span>
                  <Icon name="arrow_forward" size={14} />
                  <span style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, color: r.decision === '승인' ? d.c : C('on-surface-variant') }}>{r.to}</span>
                </span>
              ...
              {r.decision === '나중에' && (
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <MdButton variant="tonal" size="s" icon="check" onClick={() => {}}>지금 확인</MdButton>
