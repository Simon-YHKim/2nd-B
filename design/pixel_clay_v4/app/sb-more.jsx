    ...
    <ScreenPad>
      ...
      <div style={{ position: 'relative', width: 280, height: 280, margin: '0 auto' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 45, fontWeight: 700, color: C('on-surface'), letterSpacing: '.02em' }}>{mm}:{ss}</div>
      ...
      <MdCard variant="filled" style={{ padding: 16, marginTop: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{ display: 'flex', gap: 5 }}>
          {[0, 1, 2, 3].map((i) => (
            <span key={i} style={{ width: 12, height: 12, borderRadius: 0,
        ...
        <Icon name="local_fire_department" fill size={22} style={{ color: 'var(--ds-ember)' }} />
      </MdCard>
    </ScreenPad>
  );
}
          ...
          <MdCard key={x.id} variant="outlined" style={{ padding: 14, opacity: x.on ? 1 : .55,
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                  <button onClick={() => setTimeEdit(x.id)} className="md-interactive"
                    style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 4, border: 'none', cursor: 'pointer',
                      borderRadius: 0, padding: '3px 9px 3px 8px', background: C('secondary-container') }}>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 8, padding: '2px 8px', borderRadius: 0,
    ...
    <ScreenPad>
      {phase === 'pick' && (
        <React.Fragment>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[['file', 'upload_file', '파일로'], ['account', 'link', '계정 연동']].map(([id, ic, lb]) => (
              <button key={id} onClick={() => setMode(id)} className="md-interactive"
                style={{ position: 'relative', flex: 1, padding: '14px 10px', borderRadius: 0, cursor: 'pointer',
          ...
          {mode === 'file' ? (
            <div style={{ border: `1.5px dashed ${C('outline')}`, borderRadius: 0, padding: '28px 16px', textAlign: 'center', background: C('surface-container') }}>
      ...
      {phase === 'parsing' && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 420, textAlign: 'center' }}>
          <div style={{ position: 'relative', width: 64, height: 64, marginBottom: 20 }}>
            <span style={{ position: 'absolute', inset: 0, borderRadius: 0, border: `3px solid ${C('primary')}`,
              borderTopColor: 'transparent', animation: 'none' }} />
            <img src={window.SB_HEAD['head-front']} alt="" style={{ position: 'absolute', inset: 12, width: 40, height: 40 }} />
          </div>
          ...
          <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, color: C('on-surface-variant'), marginTop: 8 }}>{Math.round(pct)}%</div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        {stores.map((s) => (
          <MdCard key={s.label} variant="filled" style={{ padding: 12, textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 15, fontWeight: 700, color: C('on-surface'), marginTop: 6 }}>{s.n}</div>
        ...
        {signals.map((sg, i) => (
          <MdCard key={i} variant="outlined" style={{ padding: 13 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <span className="md-body-medium" style={{ color: C('on-surface'), fontWeight: 700, wordBreak: 'keep-all' }}>{sg.to}</span>
            </div>
          ...
          <div key={row.label} className="md-interactive"
            onClick={() => {
              ...
              if (row.danger) setConfirm({ title: '정말 영구 삭제할까요?', danger: true, confirmLabel: '영구 삭제',
                ...
                onConfirm: () => env.showToast({ msg: '영구 삭제 요청을 받았어요' }) });
            ...
            style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px', borderRadius: 0,
...
function ShareCardScreen({ t, go, env }) {
  ...
  return (
    <ScreenPad>
      ...
      {/* preview */}
      <div style={{ width: CARD, height: CARD, margin: '0 auto', borderRadius: 0, overflow: 'hidden', position: 'relative',
        background: 'radial-gradient(130% 100% at 50% 0%, var(--c02), var(--ds-space) 75%)', boxShadow: 'none',
        ...
        {variant === 'insight' ? (
          <div style={{ position: 'absolute', inset: 0, padding: 30, display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, letterSpacing: '.18em', color: 'var(--c11)' }}>2ND-BRAIN · 이번 주</div>
            <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
              <div style={{ fontSize: 30, fontWeight: 700, color: 'var(--c07)', lineHeight: 1.4, wordBreak: 'keep-all' }}>
                나는 <span style={{ color: 'var(--ds-core)' }}>먼저 다가가는</span> 사람.<br />이번 주, 관계 별이 가장 밝았어요.
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <img src={window.SB_HEAD['head-front']} alt="" style={{ width: 34, height: 34 }} />
              <div style={{ fontSize: 12, color: 'rgba(220,230,255,.7)' }}>세컨비가 함께 본 한 주</div>
            </div>
          </div>
        ) : (
          <div style={{ position: 'absolute', inset: 0, padding: 26 }}>
            ...
            <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, letterSpacing: '.18em', color: 'var(--c11)', textAlign: 'center' }}>MY CONSTELLATION</div>
            <div style={{ position: 'relative', width: '100%', height: 220, marginTop: 6 }}>
              {stars.map((s) => (
                <div key={s.k} style={{ position: 'absolute', left: `${s.x}%`, top: `${s.y}%`, transform: 'translate(-50%,-50%)' }}>
                  <span style={{ display: 'block', width: s.on ? 11 : 6, height: s.on ? 11 : 6, borderRadius: 0,
                    background: s.on ? 'var(--c11)' : 'rgba(127,208,255,.4)',
              ...
              <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 16, height: 16,
                borderRadius: 0, background: 'var(--c07)', boxShadow: 'none' }} />
            </div>
            <div style={{ textAlign: 'center', marginTop: 6 }}>
              <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--c07)' }}>5개 별이 빛나는 중</div>
              <div style={{ fontSize: 12, color: 'rgba(220,230,255,.65)', marginTop: 2 }}>2nd-Brain · 124개 별가루</div>
            </div>
            </div>
          </div>
        )}
        ...
        <React.Fragment>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {seed.steps.map((st, i) => (
              <MdCard key={i} variant="outlined" style={{ padding: 13 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ width: 24, height: 24, borderRadius: 0, flex: '0 0 auto', display: 'grid', placeItems: 'center',
