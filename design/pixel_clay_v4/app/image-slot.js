...
(() => {
  ...
  const stylesheet =
    ...
    '  font:13px/1.3 var(--font-ui,system-ui),system-ui,-apple-system,sans-serif;' +
    '  color:var(--fg,rgba(0,0,0,.55));width:240px;height:160px}' +
    '.frame{position:absolute;inset:0;overflow:hidden;background:var(--panel,rgba(0,0,0,.04))}' +
    ...
    '.empty svg{opacity:.9}' +
    '.empty .cap{max-width:90%;font-weight:700;letter-spacing:.01em}' +
    '.empty .sub{font-size:11px;opacity:.75}' +
    '.empty .sub u{text-underline-offset:2px;text-decoration-color:currentColor}' +
    '.empty:hover .sub u{color:var(--accent-soft,inherit);text-decoration-color:currentColor}' +
    ':host([data-over]) .frame{outline:2px solid #c96442;outline-offset:-2px;' +
    '  background:rgba(201,100,66,.10)}' +
    '.ring{position:absolute;inset:0;pointer-events:none;border:1.5px dashed var(--edge-soft,rgba(0,0,0,.25));' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap"></div>' +
        '    <div class="sub">또는 <u>파일 선택</u></div></div>' +
