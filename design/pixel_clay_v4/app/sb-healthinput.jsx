...
function VoiceField({ value, onChange, rec, setRec, placeholder, C }) {
  return (
    <div style={{ border: `1px solid ${rec ? C('error') : C('outline-variant')}`, borderRadius: 0, background: C('surface-container-highest'), padding: 12, transition: 'border-color .2s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
        <button onClick={() => setRec(!rec)} className="md-interactive" aria-label={rec ? '녹음 멈추기' : '음성으로 받아 적기'}
          style={{ position: 'relative', width: 38, height: 38, borderRadius: 0, border: 'none', cursor: 'pointer', flex: '0 0 auto',
        ...
        {rec ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ display: 'flex', gap: 3, alignItems: 'center', height: 22 }}>
              {[0, 1, 2, 3, 4, 5, 6].map((i) => (
                <span key={i} style={{ width: 3, height: 18, borderRadius: 0, background: C('error'),
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 16px 14px' }}>
        ...
        <div className="md-title-medium" style={{ color: C('on-surface'), marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <span className="md-label-small" style={{ whiteSpace: 'nowrap', color: C('on-tertiary-container'), background: C('tertiary-container'), padding: '2px 8px', borderRadius: 0 }}>초저마찰 · 10초</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 7, marginBottom: 20 }}>
          {MOODS.map((m, i) => (
            <button key={m.ic} onClick={() => setMood(i)} className="md-interactive" aria-pressed={mood === i} title={m.l}
              style={{ position: 'relative', flex: 1, border: `1px solid ${mood === i ? C('tertiary') : C('outline-variant')}`, borderRadius: 0,
        ...
        <div style={{ display: 'flex', gap: 9, marginBottom: 20 }}>
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} onClick={() => setEnergy(n)} className="md-interactive" aria-label={`에너지 ${n}`}
              style={{ position: 'relative', flex: 1, height: 34, borderRadius: 0, cursor: 'pointer',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {SHORTCUTS.map((s) => (
            <MdCard key={s.id} variant="outlined" onClick={() => go('lifeinput', { tplId: s.id })}
              style={{ padding: '14px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>              <div style={{ width: 40, height: 40, borderRadius: 0, display: 'grid', placeItems: 'center', background: C('tertiary-container'), color: C('on-tertiary-container') }}>
      ...
      <div style={{ padding: '10px 16px calc(10px + env(safe-area-inset-bottom))', borderTop: `1px solid ${C('outline-variant')}`, background: C('surface') }}>
        <div style={{ marginBottom: 8 }}>
          <MdButton variant="outlined" size="s" icon="cardiology" style={{ width: '100%' }} onClick={() => go('healthdata')}>건강 데이터 항목</MdButton>
        </div>
