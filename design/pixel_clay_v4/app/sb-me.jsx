...
function MeScreen({ t, go }) {
  ...
  return (
    <div style={{ overflowY: 'auto', height: '100%' }}>
      {/* 프로필 — 아바타·이름·직업 */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px 4px' }}>
        <button onClick={() => go('profilesetup')} aria-label="프로필 수정" className="md-interactive"
        style={{ position: 'relative', width: 52, height: 52, flex: '0 0 auto', display: 'grid', placeItems: 'center', border: 'none', cursor: 'pointer',
          background: C('surface-container-highest'), boxShadow: 'var(--ds-edge)' }}>
          <span className="md-state" /><window.SbAvatar spec={prof.avatar} size={44} />
        </button>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="md-title-medium" style={{ color: C('on-surface') }}>{prof.name}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, fontFamily: 'var(--font-micro)', color: C('on-surface-variant'), marginTop: 2 }}>
            {job && <Icon name="badge" size={12} />}{job ? job.ko : '@' + prof.handle}
          </div>
        </div>
      </div>
      {/* data-generated persona deck — swipe through facets (PRD §3.1) */}
      <window.PersonaDeck go={go} heading="내 역할" />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
          ...
          <MdCard variant="filled" onClick={() => go('ratify')} style={{ padding: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="md-body-medium" style={{ color: C('on-surface') }}>확인 이력</div>
        ...
        <MdCard variant="elevated" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: 16, background: C('tertiary-container') }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 15, color: C('on-tertiary-container') }}>simon.iden</div>
                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 7px', borderRadius: 0, background: C('surface'), color: C('on-surface-variant') }}>v2.1</span>
                  <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 7px', borderRadius: 0, background: C('primary'), color: C('on-primary') }}>서명됨</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); go('northstar'); }} className="md-interactive" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', color: C('tertiary'), fontSize: 12, fontWeight: 700, cursor: 'pointer', padding: '2px 6px', borderRadius: 0 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, color: C('on-surface'), marginTop: 3 }}>O72 C58 E41 A67 N39</div>
              </div>
            </div>
