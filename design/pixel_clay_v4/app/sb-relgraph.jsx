...
(function () {
  ...
  const CATS = [
    { id: 'love',   name: '연인', color: 'var(--sb-mood-negative)' },
    { id: 'family', name: '가족', color: 'var(--warn)' },
    { id: 'friend', name: '친구', color: 'var(--c11)' },
    { id: 'work',   name: '동료', color: 'var(--ok)' },
    { id: 'acq',    name: '지인', color: 'var(--ds-nebula)' },
  ];
        ...
        <MdCard variant="outlined" style={{ padding: 0, overflow: 'hidden' }}>
          <div ref={vpRef} onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerCancel={onUp}
            ...
            <div ref={worldRef} style={{ position: 'absolute', top: 0, left: 0, width: G.W, height: G.H, transformOrigin: '0 0', zIndex: 2 }}>
              <svg width={G.W} height={G.H} viewBox={`0 0 ${G.W} ${G.H}`} style={{ position: 'absolute', inset: 0, overflow: 'visible' }}>
                ...
                {G.edges.map((e, i) => {
                  ...
                  return (
                    <g key={i}>
                      <line ref={reg} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="var(--ds-core)" strokeWidth={1.1 * ls} strokeDasharray="3 4" opacity={showThis ? 0.42 : 0.05} />
                      {cfg.showBondLabels && showThis && labelsOn &&
                        <text ref={(el) => (bondTextEls.current[i] = el)} x={(a.x + b.x) / 2} y={(a.y + b.y) / 2 - 4} textAnchor="middle"
                          style={{ fontSize: 10, fontWeight: 700, fill: 'var(--c11)', paintOrder: 'stroke', stroke: 'rgba(7,10,19,.9)', strokeWidth: 3, fontFamily: 'var(--md-ref-typeface-plain)' }}>{e.label}</text>}
                    </g>
                  );
                })}
              </svg>
            ...
            <button onClick={() => { setTab('filter'); setFilterOpen(true); }} onPointerDown={(e) => e.stopPropagation()} className="md-interactive"
              style={{ position: 'absolute', left: 10, bottom: 10, zIndex: 6, display: 'inline-flex', alignItems: 'center', gap: 6, height: 36, padding: '0 12px 0 11px', borderRadius: 0, cursor: 'pointer', whiteSpace: 'nowrap',
              ...
              {filterCount > 0 && <span style={{ minWidth: 16, height: 16, padding: '0 4px', borderRadius: 0, background: CC('primary'), color: CC('on-primary'), fontSize: 10, fontWeight: 700, display: 'grid', placeItems: 'center' }}>{filterCount}</span>}
            </button>

            {/* count */}
            <span style={{ position: 'absolute', right: 10, bottom: 10, zIndex: 6, whiteSpace: 'nowrap', fontFamily: 'var(--md-ref-typeface-mono)', fontSize: 12, fontWeight: 700, color: 'var(--c11)',
            ...
            {filterOpen && (
              <div onPointerDown={(e) => e.stopPropagation()} style={{ position: 'absolute', inset: 0, zIndex: 20 }}>
                ...
                <div style={{ position: 'absolute', left: 8, right: 8, bottom: 8, borderRadius: 0, padding: 16, background: 'rgba(14,19,30,.98)',
                  border: `1px solid ${CC('outline-variant')}`, boxShadow: 'none', maxHeight: '94%', overflowY: 'auto' }}>
                      ...
                      {[0, 1, 2].map((tp) => {
                        ...
                        return (
                          <button key={tp} onClick={() => toggleTier(tp)} className="md-interactive"
                            ...
                            <span className="md-state" />
                            <span style={{ width: 10 - tp * 2, height: 10 - tp * 2, borderRadius: 0, background: 'var(--c11)', opacity: on ? 1 : 0.4 }} />
                            <span style={{ fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap', color: on ? 'var(--c07)' : 'rgba(200,220,255,.6)' }}>{TIER_LABEL[tp]}</span>
                          </button>
                        );
                      })}
                    </div>
                        ...
                        return (
                          <button key={id} onClick={() => setDateMode(id)} className="md-interactive"
                            style={{ position: 'relative', height: 34, padding: '0 14px', borderRadius: 0, cursor: 'pointer',
                              ...
                              color: on ? CC('on-primary') : 'rgba(200,220,255,.7)', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>
                  ...
                  {tab === 'force' && <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 12, padding: '10px 12px', borderRadius: 0, background: 'var(--panel-2)' }}>
                      ...
                      <span style={{ fontSize: 12, color: 'rgba(200,220,255,.65)', lineHeight: 1.5 }}>사람을 길게 끌어 옮길 수 있어요. 놓으면 관계망이 스스로 다시 자리를 잡아요.</span>
                    </div>
                  </div>}
                  ...
                  <button onClick={() => setFilterOpen(false)} className="md-interactive"
                    style={{ position: 'relative', width: '100%', height: 46, marginTop: 18, borderRadius: 0, border: 'none', cursor: 'pointer',
                      background: CC('primary'), color: CC('on-primary'), fontSize: 15, fontWeight: 700 }}>
  ...
  function MeNode({ node, onDown, reg, selected, showLabel, scale }) {
    ...
    return (
      <button ref={reg} onPointerDown={(e) => onDown(e, 'me')} style={nodeBtn(4, node.x, node.y)}>
        <span style={{ display: 'block', width: d, height: d, borderRadius: 0, background: 'radial-gradient(circle,var(--c08),var(--accent) 80%)',
          boxShadow: selected ? '0 0 0 4px rgba(90,166,255,.4), 0 0 22px rgba(90,166,255,.95)' : '0 0 20px rgba(90,166,255,.9)' }} />
        <Lbl on={showLabel} color="var(--c05)" size={11} weight={800} top={d / 2 + 9}>나</Lbl>
      </button>
    );
  }
  ...
  function RelDetail({ node, CC, go, onPick, onClose }) {
    ...
    const p = node.p;
    const accent = isMe ? 'var(--accent)' : CCOLOR[p.cat];
    ...
    return (
      <div onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: accent }}>{isMe ? '관계망 중심' : CNAME[p.cat]}</span>
              {!isMe && <span style={{ fontSize: 12, color: 'rgba(200,220,255,.6)' }}>{TIER_LABEL[p.tier]} · 마지막 연락 {lastText(p.last)}</span>}
            </div>
            <div className="md-title-medium" style={{ color: 'var(--c07)', fontWeight: 700, wordBreak: 'keep-all' }}>{isMe ? '나' : p.name}</div>
          </div>
        ...
        <div style={{ marginTop: 22 }}>
          <window.DialogBox compact kindLabel="세컨비가 본 관계"
          line={isMe ? `가까운 사람부터 느슨한 사이까지 ${PEOPLE.length}명과 이어져 있어요. 선이 밝을수록 자주 닿는 관계예요. 사람끼리 이어진 점선은 나를 거치지 않는 그들 사이의 연줄이고요.` : p.note}
          head={<window.SbHead size={32} track={false} />} />
        </div>

        {!isMe && <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
          <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 9px', borderRadius: 0, whiteSpace: 'nowrap', background: `color-mix(in srgb, ${accent} 22%, transparent)`, color: 'var(--c07)' }}># {CNAME[p.cat]}</span>
          <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 9px', borderRadius: 0, whiteSpace: 'nowrap', background: 'var(--panel-2)', color: 'var(--c05)' }}>함께한 기록 {p.recs}</span>
          <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 9px', borderRadius: 0, whiteSpace: 'nowrap', background: 'var(--panel-2)', color: 'var(--c05)' }}>마지막 {lastText(p.last)}</span>
        </div>}

        {/* 이 사람과 직접 이어진 사람들 (사람↔사람 연줄) */}
        {!isMe && links.length > 0 && <div style={{ marginTop: 12 }}>
          <div className="md-label-small" style={{ color: 'rgba(200,220,255,.6)', marginBottom: 6 }}>이 사람과 이어진 사람 {links.length}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {links.map(({ person: o, label }) => (
              <div key={o.id} onClick={() => onPick(o.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 0, background: 'var(--panel-2)', cursor: 'pointer' }}>
                <span style={{ width: 8, height: 8, borderRadius: 0, background: CCOLOR[o.cat], flex: '0 0 auto', boxShadow: 'none' }} />
                <span className="md-body-small" style={{ color: 'var(--c05)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o.name}</span>
                <span style={{ fontSize: 10, color: 'rgba(200,220,255,.55)', flex: '0 0 auto' }}>{label}</span>
        ...
        {!isMe && go && <button onClick={() => go('chat')} className="md-interactive"
          style={{ position: 'relative', width: '100%', height: 40, marginTop: 14, borderRadius: 0, border: 'none', cursor: 'pointer', background: accent, color: 'var(--ds-space-2)', fontSize: 12, fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, whiteSpace: 'nowrap' }}>
