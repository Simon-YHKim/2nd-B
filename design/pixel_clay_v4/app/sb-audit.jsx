      ...
      <ScreenPad>
        ...
        <MdCard variant="filled" style={{ padding: '12px 8px 4px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 4, marginBottom: 8 }}>
            <span className="md-display-small" style={{ color: C('primary'), fontWeight: 700 }}>{avg}%</span>
        ...
        <div style={{ marginTop: 22 }}>
          <window.DialogBox compact kindLabel="세컨비"
          line={`${darkest.name} 영역이 가장 비어 있어요. 한 영역만 또렷해져도 옆 영역까지 같이 밝아지곤 해요.`}
            ...
            return (
              <MdCard key={d.id} variant="outlined" onClick={() => { const s = starOf(d.id); if (s) go('star', s); }} style={{ padding: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 0, display: 'grid', placeItems: 'center', flex: '0 0 auto',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span className="md-label-small" style={{ color: C(st.c), background: C('surface-container-highest'), padding: '1px 8px', borderRadius: 0, whiteSpace: 'nowrap' }}>{st.k}</span>
                    </div>
...
function MiniBar({ label, value, color, C }) {
  return (
    <div style={{ flex: 1 }}>
      ...
      <div style={{ height: 6, borderRadius: 0, background: C('surface-container-highest'), overflow: 'hidden' }}>
        <div style={{ width: value + '%', height: '100%', borderRadius: 0, background: color }} />
      </div>
    </div>
  );
}
  ...
  return (
    <ScreenPad>
      ...
      <MdCard variant="filled" style={{ padding: 16, marginTop: 4, display: 'flex', alignItems: 'center', gap: 16 }}>
        <div>
          <div className="md-display-small" style={{ color: C('on-surface'), fontWeight: 700, lineHeight: 1 }}>{total}</div>
        ...
        {sorted.map((d) => (
          <MdCard key={d.id} variant="outlined" onClick={() => { const s = starOf(d.id); if (s) go('star', s); }} style={{ padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ width: 34, height: 34, borderRadius: 0, display: 'grid', placeItems: 'center',
                background: C('secondary-container'), color: C('on-secondary-container') }}>
                <Icon name={d.icon} size={18} />
              </div>
              <span style={{ width: 8, height: 8, borderRadius: 0, background: C('primary'),
                opacity: 0.3 + (d.level / 5) * 0.7, boxShadow: 'none' }} />
            </div>
            <div className="md-title-small" style={{ color: C('on-surface'), marginTop: 10 }}>{d.name}</div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 5, marginTop: 2 }}>
              <span className="md-title-medium" style={{ color: C('primary'), fontWeight: 700 }}>{d.count}</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 10 }}>
              {d.themes.map((th) => (
                <span key={th} className="md-label-small" style={{ color: C('on-surface-variant'),
                  background: C('surface-container-highest'), padding: '2px 8px', borderRadius: 0, whiteSpace: 'nowrap' }}>{th}</span>
              ))}
            </div>
          </MdCard>
        ))}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0 16px' }}>
          <div style={{ width: 44, height: 44, borderRadius: 0, display: 'grid', placeItems: 'center', flex: '0 0 auto',
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {MOODS.map(([ic, lb]) => (
                  <button key={lb} onClick={() => setMood(lb)} className="md-interactive"
                    style={{ flex: 1, border: `1px solid ${mood === lb ? C('primary') : C('outline-variant')}`, borderRadius: 0,
    ...
    <div>
      ...
      {multi ? (
        <textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder={hint} rows={3}
          style={{ width: '100%', resize: 'none', border: `1px solid ${C('outline-variant')}`, borderRadius: 0, padding: '11px 13px',
