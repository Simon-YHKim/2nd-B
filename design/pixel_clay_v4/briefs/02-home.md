...
- 세컨비 머리 또는 완성된 대화 패널의 포트레이트 제외 영역 전체 탭 → 말풍선 상태 순환: intro → 앱 사용 팁 3개 → reasoning → menu → intro. 사용 팁에는 다음/기능 메뉴 버튼을 항상 표시하고, 첫 기록 코칭 중에는 이 순환보다 실제 첫 기록 진입이 우선
- 세컨비 안내는 레트로 JRPG 대화 패널(좌측 포트레이트 + 화자 명패 + 본문)로 표시. 글자 단위 타입라이터와 짧은 블립을 사용하고, 출력 중 포트레이트/본문 탭은 전문 즉시 표시
- 대화 패널은 무대 배경보다 밝은 4방향 베벨 면으로 분리하고 홈 콘텐츠 최하단에서 독과 여백 없이 맞닿는다. 홈 독은 같은 표면 계열을 사용해 하나의 하단 조작부처럼 이어 보인다.
- 홈 상단의 알림·뮤지엄·커뮤니티·공지 버튼은 하나의 가로 바 안에 배치한다. 왼쪽 세 버튼은 묶고 공지는 오른쪽 끝에 두며, 성인 여부에 따른 커뮤니티 노출 규칙을 유지한다.
- 상단 바의 빈 가운데는 공지 센터의 실제 제목이 오른쪽에서 왼쪽으로 반복해서 흐르는 전광판으로 쓴다. 누르면 같은 공지를 열고, 모션 줄이기에서는 한 줄 정지 텍스트로 보여준다.
- 별 광채의 5단 디더 아래에는 불투명한 어두운 별 윤곽을 두어 L1에서도 흩어진 점처럼 보이지 않게 한다. 북극성의 크기·밝기 우위는 유지한다.
- 선택 버튼은 포트레이트와 같은 행의 오른쪽 열 하단에서 한 행의 동일한 가변 폭을 나눠 쓴다. 시각 높이는 32px로 압축하되 사방 6px hit slop으로 터치 영역은 44px 이상을 유지하며, 어떤 버튼도 패널의 실제 내부 폭을 넘지 않는다. 버튼은 전체 패널 탭 레이어보다 위에서 입력을 받는다.
- 별자리는 대화 패널의 최대 예약 높이를 먼저 제외한 나머지 영역에서 중앙 정렬하고, 짧은 화면에서는 높이 예산에 맞춰 비례 축소한다.
- 홈 독의 다섯 탭은 하나의 평평한 바가 아니라 간격을 둔 4방향 베벨 버튼으로 보이며, 선택된 탭은 눌린 면과 명도·글자 굵기로 함께 구분한다.
- 타입라이터가 끝난 뒤 기존 선택 버튼과 계속 표식을 표시. 모션 줄이기에서는 타입라이터·블립 없이 전문 즉시 표시
...
- 첫 기록 코칭: 실제 세컨비 머리 강조 → 메모 선택 → 한 줄 입력 → 담기 성공
- 배경은 보이는 디더 스크림, 강조된 실제 컨트롤을 눌러 진행 / 다시 보지 않기
- 독: 별자리 · 담기 · 세컨비 · 위키 · 설정
