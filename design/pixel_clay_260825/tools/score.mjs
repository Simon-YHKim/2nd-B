...
async function scoreOne(page, id, route) {
  ...
  // ⚠ **더 나은 이미지 눈금을 찾지 말 것. 이미 해봤고, 눈금이 문제가 아니다.**
  //
  // 격자 점유(칸별 밝기 표준편차) 눈금을 만들어 24개 설정으로 재봤다:
  //
  //     앱 캡처 6장 ↔ 레퍼런스 6장 : 자기 짝 찾기 **2/6** (최고)
  //     앱 캡처 6장 ↔ 앱 캡처 6장  : 자기 짝 찾기 **6/6** (24개 설정 **전부**, 벌어짐 +0.60~+0.92)
  //
  // 같은 데이터끼리는 완벽하고 레퍼런스와는 무너진다. 즉 **비교가 성립하지 않는다** —
  // 레퍼런스는 내용이 채워진 목업이고 우리 QA 계정은 비어 있다. 없는 말풍선과
  // 없는 목록의 자리를 이미지로 맞출 방법은 없다.
  //
  // 레퍼런스와 구조를 대조하고 싶으면 **먼저 비교 가능한 내용을 계정에 넣어야** 한다.
  // 그건 측정 설계 결정이지 도구가 혼자 정할 일이 아니다.
  //
  // 그 눈금은 버리지 않고 **회귀 감시**로 살려 뒀다 — `tools/structure.mjs` 와
  // `tools/structure-watch.mjs`. "이 화면이 지난번과 달라졌는가"는 아주 잘 잡는다
  // (변이 검증: 칩을 추가한 `interview` 만 0.8246 으로 걸리고 나머지 다섯은 통과).
  //
