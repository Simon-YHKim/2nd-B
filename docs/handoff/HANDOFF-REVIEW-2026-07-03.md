...
| hygiene | LOW | CONFIRMED | `C:/2ndB-dev` worktree 참조 | ⏳ B1 |
| accuracy | LOW | CONFIRMED | `15 PR` vs 나열된 14 PR | ⏳ B6 |
...
| aux-legacy | LOW | CONFIRMED | methodology-architecture.html `Brain Trinity`(외부출처) | ⏳ B6 |
| cross-repo | HIGH | CONFIRMED | satellite HANDOFF "착수 전" 상태 | ⏳ B3 |
| cross-repo | HIGH | CONFIRMED | SESSION_HANDOFF 존재X main HEAD | ⏳ B3 |
| cross-repo | MED | CONFIRMED | satellite 권위 포인터 부재 | ⏳ B3 |
| cross-repo | MED | CONFIRMED | Legacy-AI-Hub/comm-hub 중복 미러 drift | ⏳ B3 |
| cross-repo | LOW | CONFIRMED | comm-hub git mtime 오도 | ⏳ B6 |
| cross-repo | LOW | CONFIRMED | 세 갈래 체크아웃 경로 | ⏳ B6 |
| contradictions | HIGH | CONFIRMED | `C:/2ndB-dev` 를 "영구 정책"으로(CLAUDE.md 위반) | ⏳ B1 |
| contradictions | HIGH | CONFIRMED | "C:/2ndB 편집 금지" vs CLAUDE.md 정본 체크아웃 | ⏳ B1 |
| contradictions | MED | CONFIRMED | `--admin` 정책 모순(L727 vs 4곳) | ⏳ B4 |
| contradictions | MED | CONFIRMED | 31 `## Latest`(중복 발견) | ✅ A1 |
| contradictions | LOW | CONFIRMED | 머신별 절대경로 비포터블 | ⏳ B6 |
| contradictions | LOW | CONFIRMED | clinical `진단` 용어 경계선 | ⏳ B6 |
...
- **여전히 열린 항목(Simon 확인)**: 위 수정은 핸드오프가 CLAUDE.md 를 SoT 로 따르게 해 **문서 내부 모순**을
  없앴을 뿐이다. "CLAUDE.md 의 `.worktrees/` 규칙 자체가 현재 머신 환경과 맞는가"(진짜 머신-이전 질문)는
  그대로 Simon 결정. CLAUDE.md 를 바꾸면 핸드오프가 자동으로 따라옴.
