...
| accuracy | LOW | CONFIRMED | `15 PR` vs 나열된 14 PR | ⚪ 보류(§D) |
...
| aux-legacy | LOW | CONFIRMED | methodology-architecture.html `Brain Trinity`(외부출처) | ⚪ 보류(§D) |
...
- **§B6 잔여 처리(권장 방식)**: (a) Sprint 0 numbered H2 + 머신 절대경로 + `진단` 용어 → 전부 **아카이브로 이동**됐거나 헤더 안내가 커버(별도 편집 불요). (b) **픽셀-클로닝 `15 PR` 카운트 → 의도적 보류**: 나열된 `#`는 문맥 참조 포함 21개, "이번 세션 머지" 정밀 파싱은 14개로 해석 갈림 → 숫자 재작성이 새 오류를 낳을 위험 + resume agent 는 나열 목록에서 자기정정하므로 그대로 둠. (c) `methodology-architecture.html`/`design-legacy-timeline.html` LOW aux → finding 자체가 "배너 불요/범위 밖"이라 판단해 **미변경**.
