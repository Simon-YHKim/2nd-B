...
1. **AI capability is age-invariant.** `isMinor` (derived as `birth_date → age < 18`: the ceiling at `src/lib/auth/AuthContext.tsx:53`, the comparison at `src/lib/auth/AuthContext.tsx:153`) touches the LLM pipeline at exactly **one** point — selecting the youth crisis hotline 1388 over the adult line (`src/lib/safety/classifier.ts:70-90`; `src/lib/llm/safety.ts:436-466`). Model, prompt, RAG depth and persona are **byte-identical** for a 14-year-old and a 40-year-old. **All minor protections are data/egress rails, not capability limits.** **[COUNSEL TO CONFIRM]** whether identical AI capability for 14–17s is defensible given the mental-health context.
2. **Crisis handoff is deterministic and human-authored**, never LLM-generated: red-zone short-circuits before any network call and is re-checked on output, then replaced verbatim by a fixed human-written template (`fixedCrisisResponse`, `src/lib/llm/safety.ts:436-466`; `src/lib/llm/boundary.ts:560-576`, `src/lib/llm/boundary.ts:478-538`).
...
4. **Sensitive data + consent ledger.** Journaling = mental-health data. The immutable consent ledger captures `sensitive_data_ack` / `overseas_transfer_ack` / `llm_processing_ack` (`db/migrations/0031_consent_records.sql:26-28`), and ⚠ **[RE-READ 2026-09-08]** the notice/ack collection UI **is wired**: `src/lib/auth/consent-selections.ts:16-21`; both entry screens collect the acks - the shipped sign-up screen through `<ConsentBlock>` (`src/screens/deepspace/dds-sign-up-screen.tsx:346`) and complete-profile through `<ConsentNotice>` (`src/app/(auth)/complete-profile.tsx:333`); `recordConsentBestEffort(` at `src/lib/auth/useSignUpForm.ts:318` and `src/app/(auth)/complete-profile.tsx:169`; the three acks are written to the ledger at `src/lib/supabase/consent.ts:132-134`. The migration header still reads "NOT YET WIRED at sign-up" because a migration is immutable - it records the state at authoring time and cannot be corrected in place.
...
2nd-B captures free-text self-reflection (journal, notes, life-audit answers), runs it and a derived personal knowledge graph through a hosted LLM (Google Gemini) to return reflective prompts, persona inferences, and routine suggestions, and stores the results per-user in Supabase Postgres. A deterministic safety classifier inspects text on the way in and on the way out; crisis-zone text is intercepted and replaced with a fixed human-written hotline template rather than an AI reply (`src/lib/llm/boundary.ts:560-576`, `src/lib/llm/safety.ts:436-466`).
...
- **Geographic scope:** live age/consent logic is **hard-coded to KR (self-consent floor 14)**; non-KR jurisdictions are resolved but not yet legally reviewed (`src/lib/auth/consent-age-table.ts:61-125`, with the `TODO(legal)` at `src/lib/auth/consent-age.ts:67-70`). ⚠ **[RE-READ 2026-09-08]** The signal LANDED on 2026-08-16 (Simon, J1): `resolveJurisdiction()` reads the device region via `src/lib/auth/device-region.ts` and maps it to a bucket, and the result is consumed at `src/app/_layout.tsx:714`. What remains is narrower and is the thing to assess: an **unreadable or unrecognised region deliberately stays on KR** rather than raising the floor on a failed signal, so a user in an EU country whose region cannot be read is still governed by KR=14. The same comment also said `LEXICON_LAST_LEGAL_REVIEW` was null; it is `"2026-06-10"` (`src/lib/safety/lexicon.ts:460`). **The comment was the stale thing**, and this document quoted it five times - which is exactly how a stale comment does damage. EU/UK exposure is therefore a *planned* scope, explicitly fenced behind this DPIA in D-20's minority fallback (`DECISIONS.md` D-20). ⚠ **[RE-READ 2026-09-21]** r53 (2026-09-21) replaced the four buckets with a **63-country table** (`src/lib/auth/consent-age-table.ts`, generated from the r51 primary-source research; the file records the source JSON's sha256). Values run **13 to 20**, so there is no `DIGITAL_CONSENT_AGE` matrix and no `EU` bucket any more: each member state carries its own national figure (AT 14, FR 15, DE 16, IE 16). The unknown-country fallback is **18**, not KR 14 (`src/lib/auth/consent-age.ts:97`), because 23 of the 63 rows sit above 16. **The residual risk therefore inverts.** An unreadable region no longer under-protects a non-KR minor; it **over-blocks** - a 14-17 year old whose region the platform does not report cannot sign up at all, Korean users included. Nobody is blocked in practice today (the app has never shipped to a store and web usage is effectively nil), and the fix is residence self-report before launch, not a lower number. The three cases are now distinguishable in code so that round can tell who to ask (`FloorSource` at `src/lib/auth/consent-age.ts:131`).
...
### 2.5 Data subjects
- **14-17 self-consent minors (KR)** — primary target population. `isMinor` is derived as `birth_date → age < 18` (`src/lib/auth/AuthContext.tsx:153`); server `minor_tier='minor_self'` for 14-17 (`db/migrations/0030_server_age_gate.sql`).
- **Adults 18+.**
...
| **Google AdMob** | Rewarded video ads + server-side reward verification | Device ad identifiers plus an opaque, single-use SSV ticket. The account UUID is not sent to AdMob. **Minors never** — fail-closed at `src/lib/ads/policy.ts:58` | **Not launched**: production stays closed unless `EXPO_PUBLIC_REWARD_SSV=true` and a validated non-test `EXPO_PUBLIC_ADMOB_REWARDED_UNIT_ID` are both present. The SDK ships in the native build and the AdMob app ids are in `app.json` | n/a | `src/lib/ads/rewarded.native.ts`; `supabase/functions/rewarded-ssv/index.ts` |
...
- **AI:** identical model, prompt, RAG depth; the **only** minor branch is hotline selection — `crisisHotlines(locale, minor=true) → [KR_1388, KR_109]` and template version `red-ko-minor-v2` (`src/lib/safety/classifier.ts:73-75`; `src/lib/llm/safety.ts:436-466`).
...
- **Primary user data** (`records`, `personas`, `memorized_patterns`, `self_contexts`, `esm_responses`, `chat_usage`, `wiki_*`, `consent_records`, `gemini_spend_daily`): retained for the life of the account; erased through the `delete-account` Edge Function. The client binds the operation to the user who confirmed it, requires a durable owner-scoped local fence acknowledgement before the first Edge invocation, then obtains a new session; the function revalidates that token and its live `auth.sessions` row. The current source candidate first commits a durable database fence, then advances Storage cleanup in bounded flat-list batches, and deletes `auth.users` only after a fresh empty-bucket observation. Its permanent-erasure receipt is truthful only when the still-unnumbered completion-fence migration has been reserved, rehearsed, and applied (`src/lib/records/delete-bulk.ts:315-422`; `supabase/functions/delete-account/index.ts:182-277`; `supabase/functions/delete-account/storage-erasure.ts:48-177`; `db/migration-drafts/UNNUMBERED_account_deletion_completion_fence.sql:24-99,129-210,290-293`).
...
- **Retention-exception #2 — `crisis_events`:** keyed by `user_id_hash` with no user FK, so rows **are not cascade-erased**. The effective writers use deterministic unsalted MD5 (`db/migrations/0181_client_audit_ingest_hardening.sql:419-425,462-470`), while the original column comment still describes the superseded client djb2 form as 32-bit and re-identifiable (`db/migrations/0012_crisis_events.sql:12`). Neither is anonymisation. **[COUNSEL TO CONFIRM** retention justification and whether this remains personal data post-deletion.**]**
- **Retention-exception #3 — `revenue_events`:** `user_id` is **`ON DELETE SET NULL`**, while provider `external_id` and transaction facts remain (`db/migrations/0005_revenue_events.sql:19-30`). **[COUNSEL TO CONFIRM** the statutory retention period, access controls, and post-nulling identifiability.**]**
- **Retention-exception #4 — `credit_ledger`:** deliberately mirrors that `SET NULL` retention and keeps provider/event/payment attribution for the stated civil-claim window (`db/migrations/0134_credit_ledger.sql:63-70,100-105,137-142`). **[COUNSEL TO CONFIRM** the period and minimisation basis for every ledger kind, including non-purchase credits.**]**
...
1. **Data portability (GDPR Art. 20):** `exportUserWiki` produces a markdown bundle ("your second brain travels") of pages/sources (+optionally records) for pasting into another LLM (`src/lib/wiki/export.ts:1-12`), and a separate **structured, machine-readable, code-scoped export candidate** is implemented by `export-account` (`supabase/functions/export-account/index.ts:303-389,661-669`). Its cooldown migration and Edge deployment remain operational prerequisites. **[COUNSEL TO CONFIRM** whether its disclosed scope satisfies Art. 20; prior analysis treated portability as absent.**]**
...
4. **Jurisdiction signal:** the floor is resolved per device region, defaulting to KR when the region is unreadable or unrecognised; the non-KR values remain legally unreviewed (`src/lib/auth/consent-age.ts:57-66`; resolver `resolveJurisdiction` at `src/lib/auth/consent-age.ts:234-241`, which calls `deviceRegionCode`). ⚠ **[RE-READ 2026-09-08]** The signal LANDED on 2026-08-16 (Simon, J1): `resolveJurisdiction()` reads the device region via `src/lib/auth/device-region.ts` and maps it to a bucket, and the result is consumed at `src/app/_layout.tsx:714`. What remains is narrower and is the thing to assess: an **unreadable or unrecognised region deliberately stays on KR** rather than raising the floor on a failed signal, so a user in an EU country whose region cannot be read is still governed by KR=14. ⚠ **[RE-READ 2026-09-21]** r53 (2026-09-21) replaced the four buckets with a **63-country table** (`src/lib/auth/consent-age-table.ts`, generated from the r51 primary-source research; the file records the source JSON's sha256). Values run **13 to 20**, so there is no `DIGITAL_CONSENT_AGE` matrix and no `EU` bucket any more: each member state carries its own national figure (AT 14, FR 15, DE 16, IE 16). The unknown-country fallback is **18**, not KR 14 (`src/lib/auth/consent-age.ts:97`), because 23 of the 63 rows sit above 16. **The residual risk therefore inverts.** An unreadable region no longer under-protects a non-KR minor; it **over-blocks** - a 14-17 year old whose region the platform does not report cannot sign up at all, Korean users included. Nobody is blocked in practice today (the app has never shipped to a store and web usage is effectively nil), and the fix is residence self-report before launch, not a lower number. The three cases are now distinguishable in code so that round can tell who to ask (`FloorSource` at `src/lib/auth/consent-age.ts:131`).
...
**Drafting note for counsel:** "[IMPLEMENTED]" claims in Sections 2–3 retain their original 2026-06-14 baseline; account-deletion controls and citations were re-read on 2026-09-13, while the remaining document-wide re-baseline is still in progress. Every legal characterisation is flagged **[COUNSEL TO CONFIRM]** and intentionally left open. These sections assert *system behaviour*, not law.
...
| P4 | **Crisis detection & human-handoff routing** | message text (transient, classified), categorical crisis event | **Yes — health/safety** | `src/lib/llm/safety.ts:436-466`; `db/migrations/0012_crisis_events.sql` |
...
**Key invariant for the whole table:** AI *capability* (model, prompt, RAG depth, persona) is **byte-identical for minors and adults** (see §1.2 fact 1); the sole `isMinor` use in the LLM path is crisis-hotline selection — `src/lib/llm/types.ts:81-84, 114-117`; `src/lib/llm/safety.ts:436-466`; `src/lib/llm/boundary.ts:560-576`; `src/lib/auth/AuthContext.tsx:12-15,55`. The minor-specific controls are all on **P5/P7/P8/P9/P10 (data)**, not on **P2/P3 (capability)** — the proportionality argument in 4.6.
...
**Art.8 GDPR (child's consent for information-society services):** the digital-consent age is **jurisdiction-dependent** and the code encodes the matrix `KR=14 / US=13 / EU=16 / DEFAULT=16` as `DIGITAL_CONSENT_AGE` (`src/lib/auth/consent-age-table.ts:61-125`). **The live gate resolves the jurisdiction from the device region and falls back to KR=14** when that region is unreadable or unrecognised (`src/lib/auth/consent-age.ts:234-241`; signal at `src/lib/auth/device-region.ts`). ⚠ **[RE-READ 2026-09-08]** This said the gate "hard-assumes KR=14 because no reliable jurisdiction signal is collected" and that `LEXICON_LAST_LEGAL_REVIEW` is null. The signal landed 2026-08-16 and the constant is `"2026-06-10"`. The remaining exposure is the **fallback**, not the absence: an EU user whose region cannot be read is still governed by KR=14. **[COUNSEL TO CONFIRM]** that KR-only operation is the actual launch scope; any EU/UK exposure makes the 14-floor non-conforming (Art.8 default 16) and triggers the D-20 UK/EU OFF-fallback (DECISIONS.md D-20 소수의견). ⚠ **[RE-READ 2026-09-21]** r53 (2026-09-21) replaced the four buckets with a **63-country table** (`src/lib/auth/consent-age-table.ts`, generated from the r51 primary-source research; the file records the source JSON's sha256). Values run **13 to 20**, so there is no `DIGITAL_CONSENT_AGE` matrix and no `EU` bucket any more: each member state carries its own national figure (AT 14, FR 15, DE 16, IE 16). The unknown-country fallback is **18**, not KR 14 (`src/lib/auth/consent-age.ts:97`), because 23 of the 63 rows sit above 16. Art.8's 13-16 range is therefore modelled per member state rather than pinned at the ceiling, and the two figures this document called unknowable on 2026-08-16 (France, Ireland) are now fixed from the national statutes at 15 and 16. **The residual risk therefore inverts.** An unreadable region no longer under-protects a non-KR minor; it **over-blocks** - a 14-17 year old whose region the platform does not report cannot sign up at all, Korean users included. Nobody is blocked in practice today (the app has never shipped to a store and web usage is effectively nil), and the fix is residence self-report before launch, not a lower number. The three cases are now distinguishable in code so that round can tell who to ask (`FloorSource` at `src/lib/auth/consent-age.ts:131`).
...
- **Crisis path uses deterministic, human-written fixed templates — the LLM is never called in red-zone** (`src/lib/llm/safety.ts:436-466`; test asserts `mockGenerateContent` not called, `src/lib/llm/__tests__/advisor-output-swap.test.ts:230`). This keeps the most sensitive moment off the model entirely.
...
1. **Capability is constant across age.** No model/prompt/RAG/persona branch keys on `isMinor`; the sole `isMinor` use in the LLM path is crisis-hotline selection (`src/lib/llm/types.ts:81-84,114-117`; `src/lib/llm/safety.ts:440-452`; `src/lib/llm/boundary.ts:560-576`). A 14-year-old and a 40-year-old receive identical AI utility. *(This is the "bike" — it never slows.)*
...
- **Hashing / categorical-only at rest:** AI audit stores `prompt_hash`/`output_hash`, never raw text (`0004`); crisis events store categorical trigger info + a deterministic unsalted MD5 `user_id_hash` in the effective writers (`0181`), and "never raw user text" (`0012`); consent ledger stores hashed IP/UA (`0031`). *(Counsel note: the base schema's older djb2 comment and the effective MD5 writers disagree, but both forms are **obfuscation, not anonymization**, and re-identifiable; flag whether this meets pseudonymization expectations.)*
...
| Crisis hard-rail (deterministic, human templates, minor→1388, LLM not called) | **Implemented** | `src/lib/llm/safety.ts:436-466`, `0012` |
...
| **Data export / portability (GDPR Art.20)** | **Export implemented; hardened erasure pending** | both client surfaces are wired. Portability uses `export-account` (versioned JSON, user-owned tables + the `raw-clippings` bucket); the hardened erasure source is `delete-account`, but its forward Storage fence and deployment remain pending. *(A user-facing wiki/records markdown bundle exists as "your second brain travels" `export.ts`, but [COUNSEL TO CONFIRM] whether it qualifies as Art.20 portability of **all** personal data — it omits consent/persona/audit data.)* |
| **Jurisdiction signal (Art.8 age by country)** | **Signal shipped 2026-08-16; legal sign-off pending** — `resolveJurisdiction()` reads the device region and `DIGITAL_CONSENT_AGE.EU` applies; an unreadable region still answers KR by design | `src/lib/auth/consent-age.ts:57-66`, `src/lib/auth/consent-age.ts:234-241` | ⚠ **[RE-READ 2026-09-21]** the buckets are gone; see the 63-country table row above and `src/lib/auth/consent-age-table.ts:61-125`.
...
2. **Art.8 / PIPA §22-2 scope** — is launch genuinely KR-only? If any EU/UK exposure, the hardcoded 14-floor (`consent-age.ts`) is non-conforming and the D-20 EU/UK-OFF fallback must apply before launch. ⚠ **[RE-READ 2026-09-21]** there is no hardcoded 14-floor any more - the floor is per country (`src/lib/auth/consent-age-table.ts:61-125`) and unknown countries get 18. The scope question is unchanged and still needs answering; what changed is that EU/UK exposure no longer implies a 14-floor.
...
**Compounding issue — jurisdiction resolves, but fails safe to KR.** The self-consent floor resolves to KR=14 whenever the device region is unreadable or unrecognised (`src/lib/auth/consent-age.ts:97`, `DEFAULT: 16`; resolver at `src/lib/auth/consent-age.ts:234-241`); ⚠ **[RE-READ 2026-09-08]** this passage quoted the module comment as saying "the app does not yet collect a reliable jurisdiction signal (locale en/ko is not a country)" and that "`LEXICON_LAST_LEGAL_REVIEW` is still null". **Neither half is true any more, and the lines cited now say the opposite**: they open "2026-08-16: the country signal landed" (`src/lib/auth/consent-age.ts:57-66`), and the constant reads `"2026-06-10"` (`src/lib/safety/lexicon.ts:460`). The fail-safe below is real and is the actual residual risk; the absence of a signal is not. So a non-KR minor may be governed by the wrong consent age. ⚠ **[RE-READ 2026-09-21]** the fail-safe is no longer KR 14. r53 (2026-09-21) replaced the four buckets with a **63-country table** (`src/lib/auth/consent-age-table.ts`, generated from the r51 primary-source research; the file records the source JSON's sha256). Values run **13 to 20**, so there is no `DIGITAL_CONSENT_AGE` matrix and no `EU` bucket any more: each member state carries its own national figure (AT 14, FR 15, DE 16, IE 16). The unknown-country fallback is **18**, not KR 14 (`src/lib/auth/consent-age.ts:97`), because 23 of the 63 rows sit above 16. **The residual risk therefore inverts.** An unreadable region no longer under-protects a non-KR minor; it **over-blocks** - a 14-17 year old whose region the platform does not report cannot sign up at all, Korean users included. Nobody is blocked in practice today (the app has never shipped to a store and web usage is effectively nil), and the fix is residence self-report before launch, not a lower number. The three cases are now distinguishable in code so that round can tell who to ask (`FloorSource` at `src/lib/auth/consent-age.ts:131`).
...
**What / where.** Crisis handling is the **strongest** rail and is deterministic: a CRISIS_TERMS lexicon hit short-circuits the model and returns a **fixed, human-written template** — never LLM-generated text (`src/lib/llm/safety.ts:436-466` `fixedCrisisResponse`, versioned `red-ko-minor-v2` for 14-17). A **server-authoritative backstop** re-checks at the proxy (`src/lib/llm/boundary.ts:370-403` `inspectProxyCrisisRejection` / `proxyCrisisSafetyResult`) so a phrase that slips the client lexicon is still caught. Every crisis routing is written to two ledgers — `ai_audit_log` + restricted `crisis_events` (`src/lib/llm/boundary.ts:478-538`; `src/lib/llm/audit-write-outbox.ts:193-204`). Minors route to 1388 (youth line) + 109. A race guard ensures `isMinor` is resolved **before** any crisis-capable persona build, so a minor never gets adult routing mid-resolve (`src/app/persona.tsx:196-202`).
...
- **Data export/portability (GDPR Art.20).** Both code paths are wired locally: delete via `delete-account`, export via `export-account` (`supabase/functions/export-account/index.ts:569-685`). Neither is an operational control until its required migration and Edge deployment complete. ⚠ **[RE-READ 2026-09-13]** The former claim that export did not exist predated the function; the remaining questions are rollout and the scope described in 6.1.3.
...
|---|---|---|---|
| **C-AGE** | Server age gate, <14 hard-reject, server-derived `minor_tier` | `db/migrations/0030_server_age_gate.sql:18-67`; search_path-hardened `db/migrations/0033_minor_privacy_enforcement.sql:22-61`; client UX fail `src/screens/deepspace/dds-sign-up-screen.tsx:117`, `src/app/(auth)/complete-profile.tsx:80` (⚠ **[RE-READ 2026-09-08]** the sign-up half cited dds-auth-screens.tsx:373 - quoted without backticks: a record of a fixed citation, not a citation. `DeepSpaceSignUpDesignScreen` is defined in **two** files with the same line in both; the route imports the other one, so that coordinate was a dead duplicate); `MIN_SELF_CONSENT_AGE = digitalConsentAge("KR")` `src/lib/supabase/auth.ts:22` | Implemented |
| **C-CRISIS** | Crisis→deterministic human-written hotline handoff (minor→1388) | `crisisHotlines()` `src/lib/safety/classifier.ts:70-79`; `fixedCrisisResponse()` `src/lib/llm/safety.ts:436-466`; gates `src/lib/llm/boundary.ts:560-576` (input), `src/lib/llm/boundary.ts:478-538` (output swap), `callAdvisor` `src/lib/llm/boundary.ts:1335-1631` (its own input gate `src/lib/llm/boundary.ts:1349`, output swap `src/lib/llm/boundary.ts:1559`) | Implemented |
...
| **C-CONSENT** | Immutable, append-only consent ledger | `consent_records` `db/migrations/0031_consent_records.sql:15-55` (INSERT/SELECT only, no UPDATE/DELETE); guardian ledger `db/migrations/0028_minor_consent.sql:36-71` | Implemented (schema **and** sign-up wiring - ⚠ **[RE-READ 2026-09-08]** this said "wiring pending" on the strength of the frozen migration header; the acks are collected and written, `src/lib/supabase/consent.ts:132-134`) |
| **C-SENS** | Sensitive-data + LLM + overseas-transfer acknowledgement | `consent_records.sensitive_data_ack / llm_processing_ack / overseas_transfer_ack` `db/migrations/0031_consent_records.sql:26-28` | Implemented (schema **and** collection) - ⚠ **[RE-READ 2026-09-08]** this cell said *“collection UI pending”*; the three acks are discrete fields on the sign-up consent selection (`src/lib/auth/consent-selections.ts:16-21`), both entry screens collect the acks - the shipped sign-up screen through `<ConsentBlock>` (`src/screens/deepspace/dds-sign-up-screen.tsx:346`) and complete-profile through `<ConsentNotice>` (`src/app/(auth)/complete-profile.tsx:333`), and `recordConsentBestEffort()` writes them to the append-only ledger (`src/lib/supabase/consent.ts:132-134`). Rounds 64 and 67 corrected the same claim in nine places in the body; this cell outlived both of them because a table cell is not prose. |
...
| **C-DEL** | Terminal account erasure (Art.17/PIPA) | `requestAccountDeletion()` `src/lib/records/delete-bulk.ts:315`; server identity binding `supabase/functions/delete-account/index.ts:182-220`; UI `src/screens/deepspace/DeepSpaceDesignScreens.tsx:647-654` | Code candidate; deployment requires migration `0186`, a forward Storage completion fence, and Edge deployment |
...
- **Defence in depth, three layers**: (1) synchronous lexicon backstop `classifyInput()` `src/lib/safety/classifier.ts:91-116` (KO Suicide CARE 2.0 + EN C-SSRS markers `src/lib/llm/safety.ts:56-68`); (2) semantic Gemini Flash union classifier `classifySafety()` `src/lib/llm/safety.ts:305-418` (conservative RED-wins merge `src/lib/llm/safety.ts:182-204`, fail-closed on unknown zone `src/lib/llm/safety.ts:185-197`); (3) server-authoritative proxy 422 gate caught by `inspectProxyCrisisRejection()` `src/lib/llm/boundary.ts:370-392`.
...
**Residual risk.** (a) On the **keyless public web build and on live non-Vertex builds**, the Flash semantic layer is deliberately disabled to avoid uncapped egress (`getFlashClient` returns null `src/lib/llm/safety.ts:94`), degrading crisis detection to **lexicon-only** — novel phrasings without a lexicon term are a false-negative exposure. (b) Lexicon coverage is KO/EN only; other languages fall through. (c) Crisis routing is **information/handoff, not active intervention** — no human is actually contacted; the app steps back (`src/lib/llm/safety.ts:440-463`). **[COUNSEL TO CONFIRM]** whether passive hotline display satisfies duty-of-care / Raine v. OpenAI-class expectations for a minor mental-health product.
...
*(Crosswalk: see Section 6 for full treatment.)* **Control candidate: C-DEL (erasure only).** The client durably fences the confirmed owner's local writes, binds that user to a newly refreshed token, and keeps both a 12-attempt and 90-second absolute bound; the server verifies the live session, commits a deletion fence, bounds flat Storage progress, and deletes the auth row that drives the database cascade only after observing a fresh empty listing (`src/lib/records/delete-bulk.ts:315-422`; `supabase/functions/delete-account/index.ts:182-277`; `supabase/functions/delete-account/storage-erasure.ts:48-177`). The client deletion-pending/Auth-CAS integration is present in source. This is not an end-to-end operational control until the unnumbered forward fence is reserved and applied, the Edge is deployed, and this client build ships.
**Export/portability: IMPLEMENTED, with stated limits.** `export-account` returns a versioned, machine-readable JSON bundle (`kind: "2nd-b-account-export"`, `schema_version: 1`) gathered with a service-role client from a fixed, code-owned table inventory plus the `raw-clippings` bucket (table list `supabase/functions/export-account/index.ts:303-357`, bounded Storage reader `supabase/functions/export-account/index.ts:517-565`), reachable from the account screen (`src/screens/deepspace/dds-account-screen.tsx:210`). It runs service-role because several owned tables - `personas`, `memorized_patterns`, the append-only `consent_records` ledger, `xp_events` - have no client SELECT path at all (`supabase/functions/export-account/index.ts:276-286`). IDOR safety does not trust a body-selected identity: the function accepts only an empty JSON object, derives the subject from the gateway JWT, revalidates the token with `auth.getUser`, and requires an exact subject match before any personal read (`supabase/functions/export-account/index.ts:147-204,216-235,584-614`). The module that consumes it keeps a conservative compatibility contract for v1 responses rather than claiming the right is discharged: "A v1 response can contain **partial read failures and intentional exclusions**. It is neither a complete backup nor proof that a statutory access/portability duty is met" (`src/lib/account/export.ts:1-3`). Security and operational stores are deliberately excluded and reported in the response's `excluded` field; examples include `ai_audit_log`, `gemini_spend_daily`, and `revenue_events` (`supabase/functions/export-account/index.ts:359-389`). **[COUNSEL TO CONFIRM]** whether the explicit exclusions and compatibility with legacy partial-error envelopes leave Art.20 satisfied. ⚠ **[RE-READ 2026-09-13]** The current candidate fails closed on any source error, enforces finite row/byte/object/time budgets, and discloses the code-owned exclusion inventory; deployment still depends on the atomic export-cooldown migration.

**Residual risk.** Art.20 portability has a structured implementation candidate; what remains is the legal scope question above for its disclosed security/operational exclusions and backward-compatible v1 envelope, plus operational rollout of the cooldown migration and Edge function. Erasure note: effective writers derive `crisis_events.user_id_hash` as deterministic unsalted MD5 (`db/migrations/0181_client_audit_ingest_hardening.sql:419-425,462-470`), although the base schema still documents an older 32-bit djb2 form (`db/migrations/0012_crisis_events.sql:12`). It has no user FK and survives cascade; **[COUNSEL TO CONFIRM]** whether retaining the re-identifiable rows post-erasure is lawful and reconcile the stale schema description.
...
*(Crosswalk: compounds 5A-R6.)* **Control: partial.** `MIN_SELF_CONSENT_AGE = digitalConsentAge("KR")` = 14 `src/lib/supabase/auth.ts:22`; `MINOR_AGE_CEILING = 18` hardcoded `src/lib/auth/AuthContext.tsx:53`, applied at `src/lib/auth/AuthContext.tsx:153`. The analysis-lexicon now carries `LEXICON_LAST_LEGAL_REVIEW = "2026-06-10"` `src/lib/safety/lexicon.ts:460` (note: this **updates** the prior analysis which recorded `null`), but the **jurisdiction-specific** forbidden lists are **not CI-wired** `src/lib/safety/lexicon.ts:372-375` and the legal-review guard is non-blocking (`scripts/check-legal-review.ts`, warns only).

**Residual risk.** ⚠ **[RE-READ 2026-09-08]** this said non-KR ages "are not modelled" and that "a single KR=14 floor is applied globally". Both are out of date: the matrix is modelled as `DIGITAL_CONSENT_AGE` (`src/lib/auth/consent-age-table.ts:61-125`) and the live gate resolves the jurisdiction from the device region (`src/lib/auth/consent-age.ts:234-241`). What remains is narrower and is the real residual risk: **per-EU-member values are still flat 16**, and an unreadable or unrecognised region deliberately falls back to KR=14 rather than raising the floor, so a minor in an EU-16 country whose region cannot be read is still governed by 14. Per D-20 preserved minority view, **UK/EU minor `recommendations` should fall back to forced-OFF until this DPIA + counsel approve**. **[COUNSEL TO CONFIRM]** per-jurisdiction consent age and lexicon mapping before any non-KR launch. ⚠ **[RE-READ 2026-09-21]** "per-EU-member values are still flat 16" is no longer true either. r53 (2026-09-21) replaced the four buckets with a **63-country table** (`src/lib/auth/consent-age-table.ts`, generated from the r51 primary-source research; the file records the source JSON's sha256). Values run **13 to 20**, so there is no `DIGITAL_CONSENT_AGE` matrix and no `EU` bucket any more: each member state carries its own national figure (AT 14, FR 15, DE 16, IE 16). The unknown-country fallback is **18**, not KR 14 (`src/lib/auth/consent-age.ts:97`), because 23 of the 63 rows sit above 16. **The residual risk therefore inverts.** An unreadable region no longer under-protects a non-KR minor; it **over-blocks** - a 14-17 year old whose region the platform does not report cannot sign up at all, Korean users included. Nobody is blocked in practice today (the app has never shipped to a store and web usage is effectively nil), and the fix is residence self-report before launch, not a lower number. The three cases are now distinguishable in code so that round can tell who to ask (`FloorSource` at `src/lib/auth/consent-age.ts:131`).
...
| Pre-consent sensitive processing (5B-R2 / 5A-R2) | ⚠ **[RE-READ 2026-09-08]** rated *“High until sign-up consent UI ships”*. **That UI shipped** (§5B-R2 residual risk below measures it). The surviving exposure is narrower: the ledger write is **best-effort and does not gate entry**, so a failed insert leaves consent given and **not demonstrable** (Art.7(1)); and there is still **no withdraw/re-consent surface**. | **[COUNSEL TO CONFIRM]** whether the narrowed gap is still launch-blocking. Not re-rated here - the fact was wrong, the severity is a legal call. |
...
**Three controls were listed as launch-blocking gaps, not residual-risk-acceptable**: 5B-R2 ⚠ **[RE-READ 2026-09-08]** listed as *“consent UI not wired”* - **it is wired**, and this line is the third site of that claim to outlive the rounds that corrected it (64, 67, 69 each fixed the prose; this list, the rating table and the gap table kept it). What remains under 5B-R2 is demonstrability of a best-effort write plus the absent withdraw flow, and **[COUNSEL TO CONFIRM]** whether that still blocks launch - 5B-R5 (K11 disclosure absent), and the **D-19 anti-anthropomorphism CI gate** that the entire "non-companion" defence rests on. The D-18 learner-scaffold, D-19 invariants, D-20 understanding-gated *activation* UX (beyond the gate already landed), portability, and age-estimation are all **planned and explicitly gated on completion of this DPIA + §11-5 counsel sign-off** — none should be built ahead of counsel approval per the adopted debate verdicts.
...
- **Code candidate, not yet a completed deployed control:** terminal self-service erasure is implemented in the `delete-account` Edge Function source, but requires its database dependencies, the forward Storage completion fence, and Edge deployment before this DPIA can call the hardened flow operational (`supabase/functions/delete-account/index.ts`).
  - **IDOR-safe and session-current:** the client requires acknowledgement of the confirmed owner's durable local fence, refreshes the session, and rejects an account switch before sending the exact token; strict 409 progress may retry only inside one 12-attempt/90-second window (`src/lib/records/delete-bulk.ts:264-301,315-365`). The function independently calls Auth with that token (`supabase/functions/delete-account/index.ts:182-185`), binds its signed `sub` and role (`supabase/functions/delete-account/index.ts:188-198`), then binds `session_id` and `iat` to the recent live session through a service-only RPC (`supabase/functions/delete-account/index.ts:201-220`; `db/migrations/0186_account_deletion_hardening.sql`). The body is exactly `{}` and cannot select a target.
  - **Cascade and Storage scope:** the source candidate calls the service-only `begin_account_deletion` RPC first. Its durable tombstone and owner-keyed exclusive advisory lock pair with a `storage.objects` INSERT/UPDATE trigger that takes the shared lock and rechecks the tombstone after its user-row lock, serializing uploads against deletion (`db/migration-drafts/UNNUMBERED_account_deletion_completion_fence.sql:24-99,129-210`). The Edge then uses the exact-pinned experimental `listV2` flat view, restarts at page one after every removal, deletes up to two 1,000-object batches per four-call attempt, and returns retryable progress before the irreversible step when more remain. Only a fresh empty first page permits `auth.users` deletion; the response can then report both `raw_clippings_empty_at_check` and `raw_clippings_erased` as true behind that committed fence (`supabase/functions/delete-account/index.ts:207-277`; `supabase/functions/delete-account/storage-erasure.ts:73-177`). The schema cascade removes `public.users` and its user-owned children. This remains a release hold: the draft is unnumbered/unapplied, `listV2` is experimental, and both the managed-Storage trigger and two-connection race need deployment-environment rehearsal.
- **Residual data that deliberately survives erasure (flag):**
  - `ai_audit_log` rows are **retained with `user_id` set to NULL** by the FK (`db/migrations/0011_security_fixes.sql:20-27`). Content is hashes only (6.2).
  - `crisis_events` survives because it has **no `user_id` FK**. Effective writers use deterministic unsalted MD5 (`db/migrations/0181_client_audit_ingest_hardening.sql:419-425,462-470`); the older schema comment still describes djb2 (`db/migrations/0012_crisis_events.sql:12`).
  - `revenue_events` and `credit_ledger` retain transaction/ledger rows after setting `user_id` to NULL (`db/migrations/0005_revenue_events.sql:19-30`; `db/migrations/0134_credit_ledger.sql:63-70,100-105`). Provider identifiers may still permit linkage.
- **[COUNSEL TO CONFIRM]:** (1) whether nulled-user audit, revenue, and credit rows plus deterministic crisis hashes remain personal data requiring deletion or a documented exemption; (2) the exact retention/minimisation periods; (3) whether the crisis hash and stale schema description must be replaced before being relied on.
...
- **Why it does not yet satisfy Art.20:**
  - **Format:** Art.20 requires a "structured, commonly used and machine-readable format." The `export-account` response is versioned JSON (`supabase/functions/export-account/index.ts:661-669`). The markdown bundle (`src/lib/wiki/export.ts:1-3, 21-22`) still exists beside it as an *LLM-context bundle / pre-delete backup* and was never the portability path; it is no longer the only export.
  - **Coverage:** the eight categories once listed here as omitted - `consent_records`, `privacy_prefs`, `personas`, `memorized_patterns`, `xp_events`, `esm_responses`, `chat_usage`, `self_contexts` - are exactly what `EXPORT_TABLES` now gathers (`supabase/functions/export-account/index.ts:303-357`; `privacy_prefs` travels with the `users` profile row, `supabase/functions/export-account/index.ts:636-644`). ⚠ **[RE-READ 2026-09-08]** That list was the specification the function was written against, not a standing gap. The module that consumes it states its own limits rather than claiming the right is discharged: "A v1 response can contain **partial read failures and intentional exclusions**. It is neither a complete backup nor proof that a statutory access/portability duty is met" (`src/lib/account/export.ts:1-3`). Security and operational stores are deliberately excluded and reported in the response's `excluded` field; examples include `ai_audit_log`, `gemini_spend_daily`, and `revenue_events` (`supabase/functions/export-account/index.ts:359-389`). **[COUNSEL TO CONFIRM]** whether the explicit exclusions and compatibility with legacy partial-error envelopes leave Art.20 satisfied.
...
- **Constraints to note for counsel:**
  ...
  - **Withdraw-consent flow not wired:** ⚠ **[RE-READ 2026-09-08]** the *collection* half of this bullet is out of date - sign-up collection **is** wired ⚠ **[RE-READ 2026-09-08, third pass]** these five sites said *“both entry screens render `<ConsentNotice>`”*. Half of that was wrong: `<ConsentNotice>` at src/app/(auth)/sign-up.tsx:219 - quoted without backticks: a record of a fixed citation, not a citation - sits inside `SignUpLegacy`, which renders only under `EXPO_PUBLIC_UI=legacy` and which no delivery path sets. The shipped sign-up screen draws `<ConsentBlock>` instead, and it is that screen `signup-required-acks.test.ts` reads when it checks that every required ack is rendered. The control was never missing - rounds 64, 67 and 69 each passed this sentence without asking WHICH renderer the component lived in. (`src/lib/auth/consent-selections.ts:16-21`; both entry screens collect the acks - the shipped sign-up screen through `<ConsentBlock>` (`src/screens/deepspace/dds-sign-up-screen.tsx:346`) and complete-profile through `<ConsentNotice>` (`src/app/(auth)/complete-profile.tsx:333`); `recordConsentBestEffort(` at `src/lib/auth/useSignUpForm.ts:318` and `src/app/(auth)/complete-profile.tsx:169`; the three acks are written to the ledger at `src/lib/supabase/consent.ts:132-134`). What remains open is the withdrawal half, which is the actual subject of this bullet: No standing "withdraw / re-consent" UI is wired. **Planned (gated).**
...
| `revenue_events` | Transaction amount/currency/time/source and provider `external_id` (`db/migrations/0005_revenue_events.sql:19-30`) | **Indefinite in schema** | **No** — `user_id` set NULL | **[COUNSEL TO CONFIRM]** statutory period, deletion schedule after expiry, and whether provider identifiers keep rows linkable. |
| `credit_ledger` | Credit kind/units/lot, provider/event/payment attribution (`db/migrations/0134_credit_ledger.sql:63-70,72-110`) | **Indefinite in schema**; comment cites a civil-claim window | **No** — `user_id` set NULL | **[COUNSEL TO CONFIRM]** exact period and minimisation, especially promo/ad/spend rows that are not themselves payment records. |
| `crisis_events` | **Categorical only**, "never raw user text" (`src/lib/supabase/crisis-events.ts:2-3`; `db/migrations/0012_crisis_events.sql:4-8`): confidence, trigger categories, template version, locale, `resolved`, staff `notes` ("never user content"). Subject keyed by deterministic unsalted MD5 in effective writers (`db/migrations/0181_client_audit_ingest_hardening.sql:419-425,462-470`); the base schema's djb2 comment is stale. | **Indefinite** | **No** — no FK, survives deletion (`db/migrations/0012_crisis_events.sql:12`) | Retained safety record. **[COUNSEL TO CONFIRM]** lawful basis to retain post-erasure and hash strength (6.3-#7). |
...
| `raw-clippings` Storage | Raw clipped markdown | Indefinite | **Fenced, bounded pre-Auth erasure candidate** - each attempt makes at most four Storage calls and removes up to two 1,000-object batches from the flat all-depth `listV2` view, restarting at the first page after every removal and bounding UTF-8 path length (`supabase/functions/delete-account/storage-erasure.ts:1-3,73-177`); list failures fail closed (`supabase/functions/delete-account/storage-erasure.ts:95-116`). The exact returned-name set must equal the requested set before progress is credited (`supabase/functions/delete-account/storage-erasure.ts:149-173`). A service-only durable tombstone plus advisory-lock/row-lock trigger serializes raw-clipping INSERT/UPDATE against deletion (`db/migration-drafts/UNNUMBERED_account_deletion_completion_fence.sql:24-99,129-210`). Only a fresh empty listing permits Auth deletion, after which `raw_clippings_empty_at_check` and `raw_clippings_erased` are both true (`supabase/functions/delete-account/index.ts:207-277`). | Pre-Auth progress keeps the account intact and retryable. Operational completion is held until the migration gets a reserved number and is applied, the exact-pinned experimental `listV2` path and trigger are rehearsed on managed Storage, the Edge is deployed, and this client build ships. **[COUNSEL TO CONFIRM]** the retry/operator workflow and retention response. |
...
| 3 | **Portability export (Art.20)** | GDPR Art.20 / PIPA §35-2 | **Code candidate, rollout pending**: structured JSON via `export-account` from a fixed table inventory with disclosed security/operational exclusions; the markdown bundle remains an LLM-context/pre-delete artifact in the legacy shell only. The shipped wiki screen (`src/screens/deepspace/dds-wiki-records-screens.tsx`) has no markdown export path. Remaining: apply the cooldown migration, deploy the Edge function, and obtain counsel confirmation for the disclosed scope and backward-compatible v1 envelope | platform owner + counsel | **P2** |
| 4 | **Jurisdiction signal — not hardcoded KR=14** | GDPR Art.8 (13-16 by member state); PIPA §22-2; COPPA | Matrix exists (`src/lib/auth/consent-age-table.ts:61-125`, KR14/US13/EU16/DEFAULT16) and the **country signal shipped 2026-08-16** (`src/lib/auth/consent-age.ts:57-66`; resolver `src/lib/auth/consent-age.ts:234-241`). What is still open is **legal sign-off**, not the signal: the module's own TODO holds per-EU-member values and the signal itself as needing sign-off before non-KR reliance (`src/lib/auth/consent-age.ts:67-70`), and an unreadable or unrecognised region deliberately answers KR rather than raising the floor | counsel (per-EU-member values + sign-off) | **P1** (still blocks non-KR launch, for the sign-off rather than the signal) | ⚠ **[RE-READ 2026-09-21]** the table landed (63 countries, 13-20, fallback 18) and the fallback no longer answers KR. What is still open is unchanged: **legal sign-off**, and in particular the 15 rows whose value comes from civil-capacity rather than a data-protection statute (`basis: "civil-capacity"`), which counsel may be able to lower.
...
| 6 | **`LEXICON_LAST_LEGAL_REVIEW` review cadence** | Accountability (Art.5(2)); review of consent copy + jurisdiction age values | Partial: constant set to `"2026-06-10"` for the safety lexicon (`src/lib/safety/lexicon.ts:460`) with the 365-day cadence living in the checker rather than the lexicon - `REVIEW_CADENCE_DAYS = 365` (`scripts/check-legal-review.ts:14`) - and a **non-blocking CI warn** there, wired into `npm run verify` as `check:legal-review`. ⚠ **[RE-READ 2026-09-07]** This row previously said "no automated staleness warn"; that gate now exists and runs on every build - it warns and never fails, by design, because a review cadence is a process signal rather than a code defect. The same row said the `src/lib/auth/consent-age.ts:67-70` and `src/lib/supabase/consent.ts:14-21` comments "still read null (stale)"; neither does any more - the consent module now records that the acks are WIRED at sign-up. The genuinely-pending item is counsel's review of consent/policy/terms versions (placeholder `2026-06-02`) + overseas-transfer + per-EU age (`docs/HANDOFF.md:331,486`) | Eng (wire the warn + de-stale the comments) + counsel (perform & date the review) | **P2** | ⚠ **[RE-READ 2026-09-21]** the `src/lib/auth/consent-age.ts` comment this row cites was rewritten by r53; the `TODO(legal)` it refers to now sits at `src/lib/auth/consent-age.ts:67-70` and asks for sign-off on the table rather than on per-EU-member values. The per-jurisdiction age item is no longer pending research, only counsel.
| 7 | **Crisis-event subject identifier strength and schema drift** | Art.17/Art.32; bears on whether retained crisis rows are "anonymised" | Effective writers use deterministic unsalted MD5 (`db/migrations/0181_client_audit_ingest_hardening.sql:419-425,462-470`), while the base schema comment still describes the superseded 32-bit djb2 form (`db/migrations/0012_crisis_events.sql:12`). Both are re-identifiable without a secret salt. | Eng + counsel (does retention-post-erasure require true anonymisation?) | **P2** |
| 8 | **No withdraw-consent flow; consent-write failure is not demonstrable** ⚠ **[RE-READ 2026-09-08]** this gap was titled *“Consent capture not wired at sign-up / no withdraw-consent flow”* while its own Status cell already recorded that capture **is** wired - a row contradicting itself across two columns. Only the surviving half is named now. | GDPR Art.7 (demonstrable consent + Art.7(3) withdrawal); PIPA §22 | ⚠ **[RE-READ 2026-09-08]** immutable ledger built **and wired at sign-up** (`src/lib/supabase/consent.ts:132-134`); the remaining gap is the standing withdraw/re-consent UI | Eng + counsel (consent copy sign-off, D-03 external dependency) | **P1** |
...
| 10 | **Serialize account deletion against in-flight Storage completion** | Art.17 / PIPA §36 accuracy of erasure confirmation | **Forward migration reservation required; no number assigned.** A service-only begin-fence RPC must take a user-keyed advisory transaction lock and persist a deletion tombstone. A `storage.objects` `BEFORE INSERT OR UPDATE` trigger must take the same lock and reject a `raw-clippings` owner whose tombstone exists; this orders both “upload first” and “fence first” races even when Storage completes as superuser. The same forward migration must restore 0186's flat `.md` write bound that landed 0188 no longer preserves. Edge ordering is fence → bounded sweep → auth deletion; an incomplete attempt keeps the tombstone and retries. | DB + Edge owner (reserve one forward migration; do not mutate landed 0188) | **P1 / release-blocking for confirmed Storage erasure** |
...
- All rights/retention behaviours above are **age-invariant except** crisis-hotline routing (`src/lib/llm/safety.ts:436-466`; `src/lib/safety/classifier.ts:73-75`, minor → 1388 youth line first). The minor-specific protections are **data rails** (egress clamp `0032/0033/0038`, age floor `0030`), not capability limits — consistent with §1.2.
...
**Cross-cutting prerequisite (answer first):** Q-S1 (scope). Most questions below are *conditional* on whether EU/UK/CA/KR law applies to a given user. Today the product resolves jurisdiction from the device region and **falls back to KR** when it cannot (`src/lib/auth/consent-age.ts:234-241`); the non-KR values still lack legal sign-off (`src/lib/auth/consent-age.ts:67-70`). ⚠ **[RE-READ 2026-09-08]** The signal LANDED on 2026-08-16 (Simon, J1): `resolveJurisdiction()` reads the device region via `src/lib/auth/device-region.ts` and maps it to a bucket, and the result is consumed at `src/app/_layout.tsx:714`. What remains is narrower and is the thing to assess: an **unreadable or unrecognised region deliberately stays on KR** rather than raising the floor on a failed signal, so a user in an EU country whose region cannot be read is still governed by KR=14. Counsel's scope answer determines which of the framework blocks (A–F) are even reached. ⚠ **[RE-READ 2026-09-21]** the product no longer falls back to KR when it cannot read a region; it falls back to 18. r53 (2026-09-21) replaced the four buckets with a **63-country table** (`src/lib/auth/consent-age-table.ts`, generated from the r51 primary-source research; the file records the source JSON's sha256). Values run **13 to 20**, so there is no `DIGITAL_CONSENT_AGE` matrix and no `EU` bucket any more: each member state carries its own national figure (AT 14, FR 15, DE 16, IE 16). The unknown-country fallback is **18**, not KR 14 (`src/lib/auth/consent-age.ts:97`), because 23 of the 63 rows sit above 16. **The residual risk therefore inverts.** An unreadable region no longer under-protects a non-KR minor; it **over-blocks** - a 14-17 year old whose region the platform does not report cannot sign up at all, Korean users included. Nobody is blocked in practice today (the app has never shipped to a store and web usage is effectively nil), and the fix is residence self-report before launch, not a lower number. The three cases are now distinguishable in code so that round can tell who to ask (`FloorSource` at `src/lib/auth/consent-age.ts:131`).
...
1. **AI capability is age-invariant.** `isMinor` (`<18`, `MINOR_AGE_CEILING` at `src/lib/auth/AuthContext.tsx:53`, applied at `src/lib/auth/AuthContext.tsx:153`) touches the LLM pipeline at exactly one place: **crisis-hotline routing** to the youth line 1388 (`src/lib/safety/classifier.ts:70-79`; minor template `red-ko-minor-v2` at `src/lib/llm/safety.ts:440-452`; UI routing `src/app/capture.tsx:657,783,912`). Model, prompt, RAG, persona depth are byte-identical for a 14-year-old and a 40-year-old. The minor protections are **data rails, not capability limits.**
2. **Crisis handoff is deterministic + human-authored**, never LLM-generated (`fixedCrisisResponse`, `src/lib/llm/safety.ts:436`).
...
- **Q-D2 [COUNSEL TO CONFIRM]** — If SB243 *does* apply, do the design-enforced non-companion invariants (CI-binding anti-anthropomorphism + crisis-handoff threshold lowering on cadence-spike, D-19) satisfy SB243's safeguards for minors, or does SB243 mandate specific controls (e.g., break reminders, suicidal-ideation protocols, disclosure cadence) not yet implemented?
  - *Hinges on:* crisis handoff is implemented and deterministic (`src/lib/llm/safety.ts:436`); cadence-spike threshold-lowering is **planned**.
...
- **Q-E3 [COUNSEL TO CONFIRM] (Std 3 — age-assurance proportionality)** — Is **DOB self-report** a **proportionate** age-assurance measure under Standard 3 for a service processing **minors' mental-health data**, or does the sensitivity require a stronger method (e.g., third-party age estimation/verification)?
  - *Hinges on:* age is derived solely from a self-reported `birth_date` (`src/lib/auth/AuthContext.tsx:153`; server trigger `db/migrations/0030_server_age_gate.sql:18-49`), with a hard reject under 14 (`db/migrations/0030_server_age_gate.sql:31-34`) but **no verification**. This is the weakest age-assurance tier.
...
- **Q-G1 [COUNSEL TO CONFIRM]** — Is a **14-year-old's self-consent valid** as the lawful basis for processing under **GDPR Art.8**, given that Art.8 defaults to **16** (member states may lower to 13)? For which EU member states is the live KR-derived floor (`digitalConsentAge("KR") = 14`, `src/lib/auth/consent-age-table.ts:95`) **invalid**, requiring parental consent? ⚠ **[RE-READ 2026-09-21]** the question survives but its premise moved: there is no KR-derived global floor. KR 14 now applies only to KR devices (`src/lib/auth/consent-age-table.ts:95`), and each EU member state carries its own national figure. The narrower question for counsel is whether those national figures, read from the statutes by the r51 research and **not reviewed by a lawyer**, may be relied on.
  - *Hinges on:* the live gate hardcodes KR=14 and the matrix encodes `EU:16, DEFAULT:16` and **is reached** for a device whose region resolves to the EU bucket (`src/lib/auth/consent-age-table.ts:61-125`, resolver `src/lib/auth/consent-age.ts:234-241`); it is not reached when the region is unreadable, which lands on KR. ⚠ **[RE-READ 2026-09-21]** it does not hardcode KR=14 any more, and there is no EU bucket to reach. The **server floor is still a hard-coded `< 14`** and still reads no country, so every floor above 14 remains advisory and the 12 rows at 13 are inert until that changes; the table keeps the statutory value separately from the effective one for exactly that day (`src/lib/auth/consent-age.ts:151`).
...
- **Q-H1 [COUNSEL TO CONFIRM]** — ⚠ **[RE-READ 2026-09-08] this question's premise was two-ways stale and has been re-framed; the legal determination is untouched.** It asked whether the wiki markdown export satisfies Art.20 "or is it insufficient … does Art.20 require a structured, machine-readable export of *all* personal data". Both halves had moved:
  1. **The structured, code-scoped export candidate exists** and this document says so in four other places (2.3, 5B-R8, 6.1.3) - `export-account` returns versioned JSON gathered from its fixed user-owned table inventory (`supabase/functions/export-account/index.ts:303-357`). Round 52 corrected eleven statements that called it absent; this question kept the old premise because it is phrased as a question.
  ...
  **The question that is actually open** is the one section 6.1.3 already states: whether the deliberately excluded security/operational stores and backward-compatible v1 envelope are acceptable for Art.20. The markdown bundle is a convenience artifact alongside it, not the offering.
  - *Hinges on:* the structured export is a hardened code candidate (`export-account`, response contract `supabase/functions/export-account/index.ts:661-669`) pending its cooldown migration and Edge deployment; hardened erasure is also a code candidate (`src/lib/records/delete-bulk.ts:315-422`, `delete-account`) pending its forward Storage fence and deployment. The scope question remains for deliberately excluded stores.
...
- **Q-S1 [COUNSEL TO CONFIRM]** — **Which legal regimes does the product hold itself out to today?** The build reads a country/region signal from the device (`src/lib/auth/device-region.ts`) and applies the matrix, falling back to KR when the region is unreadable (`src/lib/auth/consent-age.ts:234-241`); the non-KR values still lack legal sign-off. Counsel to determine: (a) is the **launch scope KR-only** (in which case EU/UK Std 7/12/Art.8/Art.20 questions are deferred), or (b) does targeting/availability bring **EU/UK/CA users into scope** now? ⚠ **[RE-READ 2026-09-21]** the build applies a 63-country table and falls back to **18** when the region is unreadable (`src/lib/auth/consent-age.ts:234-241`). Launch scope was answered outside this document on 2026-09-20 (global), which is why the table exists; what counsel still owns is whether the per-country figures may be relied on.
...
*End of DPIA DRAFT. Prepared by the engineering/orchestration layer as a fact-grounded working draft for counsel completion. All `[COUNSEL TO CONFIRM]` flags remain open legal determinations. Account-deletion claims were re-read against source on 2026-09-13; the wider document retains its original 2026-06-14 baseline and is still being re-baselined. This document is not legal advice and is not effective until the Section 8 block is signed.*
