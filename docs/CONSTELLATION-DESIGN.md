> ## ⚠ 역사 기록 (2026-08-25 판정) — 새 작업의 참조 금지
>
> 이 문서의 골격("홈 별 = 6 생활 도메인 + 비가시 collect")은 **2026-08-24 별 구조
> 개편(#1376~#1379)으로 무효가 됐다.** 홈 일곱 별은 이제 나를 알아가는 자리
> (프로필·영유아기·학창시절·20대·30대 이후·직장·지금)이고, 정본은
> `src/lib/persona/seven-stars.ts` 와 `docs/PRD.md`(Draft v4)다. 아래 "상위 정본 =
> PRD Draft v2" 서술도 낡았다. 생활 도메인 여섯은 별이 아니라 세컨비 대시보드로
> 갔다 — 단, 이 문서의 도메인별 4-슬롯 계약·산출 로직 서술은 `/star/<domain>`
> 상세 화면들의 역사적 설계 근거로 여전히 읽을 가치가 있다.

# 별자리 모델 상세 설계 — Phase 1 (7 도메인 스펙) + Phase 2 (북극성 산출로직)

> **이 문서는 무엇인가.** 2026-06-25 방향 전환(`docs/PRD.md` §0, core 폐기 → 별자리 단일
> 모델)의 **설계 디벨롭 산출물**이다. 핸드오프(`docs/HANDOFF.md`) "다음 작업 큐 A" = *정본화
> 전에 7 도메인 스펙 + 북극성 산출로직을 설계로 단단히*. 정본화(Phase 0/B)가 이 문서를 PRD 본문으로
> 흡수한다. **코드 변경 없음** — 코드 이관은 Phase 4(별도 트랙).

| 항목 | 값 |
|---|---|
| Status | Design draft v1 + home brightness decision (2026-07-29) |
| Owner | Simon Kim |
| 상위 정본 | `docs/PRD.md` (Draft v2) · `docs/system-checkup.html` (v4 시각화) |
| 선행 결정 | core 폐기 / 별자리 단일 비유 / 7 데이터 도메인 / 홈 6별 + Museum / 북극성=출력 / 밝기=DIKW 한 사다리 |
...
```
 ┌─────────────────────────────────────────────────────────────┐
 │  레이어 A — 입력: 7 데이터 도메인                               │
 │  홈 별: 커리어·재정·성장·관계·건강·휴식 (6)                     │
 │  비가시 intake/routing: 담아내기(collect)                        │
 └───────────────┬─────────────────────────────────────────────┘
 ...
 │  변경은 propose→ratify 로만. 홈 밝기 = 보이는 6 도메인 종합      │
 └─────────────────────────────────────────────────────────────┘
```
...
| **북극성 헤드라인 밝기** | 홈의 6 도메인 별 밝기 평균 + 6별 전부 L2 이상 보너스 | 보이는 별과 일치하는 종합 readout. `collect`·Museum 제외 |
...
| 6 | **휴식** | 취미·독서·경험 기록 | `reading/books.ts` | 취미 폼 | 개방성, VIA 호기심·심미 |
...
**6. 휴식** — 입력: 취미·경험 기록(독서 = `reading/books.ts`). 출력: 취미 성장 조언·취미 추천.
리스트업: 취미/경험 항목. 검증 피드: 개방성(Big Five), VIA 호기심·심미.

**7. 담아내기** (§5.1) — catch-all 데이터 도메인. 홈에는 별로 그리지 않으며 북극성 헤드라인
밝기에도 참여하지 않는다. 장기적으로 **나머지 6별을 먹이는 메타 역할**을 검토한다.
...
| 개방성 (Big Five O) | BFI-44 | 휴식(취미 다양성)·담아내기(탐색 범위)·성장(서사 개방) |
| 외향성 (Big Five E) | BFI-44 | 관계(상호작용 빈도·폭)·휴식(사회적 취미) |
...
북극성은 **출력**이다. 직접 입력을 받지 않고, 7 도메인 데이터를 종합한다. 헤드라인 밝기만 홈에
그리는 6 도메인을 사용한다. 4단계 파이프라인:

```
[1] 도메인 요약        7 도메인 각각 → 결정론적 요약 (건수·트렌드·상위 태그). LLM-free.
        │
...
`src/lib/persona/north-star.ts::northStarBrightness(levels)`가 홈 헤드라인을 계산한다.

- 입력 집합: `career`, `finance`, `growth`, `relation`, `health`, `recreation`의 L1~L5.
- 제외: `collect`는 평균과 all-lit 판정 모두 제외. Museum은 도메인이 아니므로 제외.
- 공식: 6개 밝기 분율 평균 + 6개가 모두 L2 이상이면 `+0.05`, 최대 `1`.
- 구인 신뢰도는 *페르소나 주장 세기*만 통제하고 헤드라인 숫자는 건드리지 않는다.

> **Simon 결정, 2026-07-29 03:44 KST**: “북극성 밝기는 홈에 그려지는 6개 도메인만
> 평균한다.” 이 결정은 홈 숫자의 범위를 정한 것이며 7 도메인 데이터 모델이나 페르소나 종합 입력을
> 6개로 축소한 것이 아니다.
...
| fixture | 기대값 |
|---|---:|
| 홈 6별=L1, `collect`=L4 | `0.2` |
| 홈 6별=L2, `collect`=L1 | `0.45` |
| 홈 `[4,3,4,4,4,3]`, `collect`=L4 | `0.783333…` |
...
담아내기 = 비가시 intake/routing 데이터 도메인. 현재 확정된 일과 보류된 일을 분리한다:

1. **무차별 흡수 (intake, 현재)**: 어느 도메인에 속하는지 불명확한 모든 입력은 여기 raw로 떨어진다.
   "담기"(`/capture`)의 기본 착지점.
2. **이동 제안 (route, 보류)**: `collect` 잔량을 분류해 해당 도메인으로 이동 제안하는
   propose→ratify 라우터는 이번 라운드에서 구현하지 않는다. 폐기 결정은 아니다.
3. **보강 (supplement, 라우터 후)**: 비준된 항목만 목적지 도메인의 밝기 커버리지에 기여하고
   원본은 재태깅 이동해 이중계상을 막는 안을 다음 검토 입력으로 둔다.

`collect` 자체는 홈 별과 북극성 헤드라인 평균·all-lit 보너스에 참여하지 않는다.

> **재검토**: Play 12명 참여 체인 착수 확인 뒤 2026-08-04 decision-only 리뷰를 권장한다.
> 구현은 기본적으로 2026-08-17 XPRIZE 마감 뒤로 둔다. 그 전에 착수하려면 Simon의 별도
> 우선순위 결정이 필요하다. 입력은 실제 미분류 표본의 재분류 가능률, move-vs-copy 계약,
> 비준 뒤 catch-all 삭제의 정당성이다.
...
- `stars.ts`: 기존 `SELF_UNDERSTANDING_STARS`(구인 7축)는 **삭제하지 않고** "레이어 B 검증틀"로
  주석·역할 재분류. `DOMAIN_STARS`(7 도메인)는 입력·분류 레이어이며 홈은 그중 6개만 렌더.
- `northStarBrightness(levels)`: 홈 6 도메인만 평균하고 `collect`를 무시. 표시명 "북극성".
- 신규 `domainConfidence(entries)` 어댑터 → 기존 `brightness.ts` 체인 재사용.
- `import/detect.ts` → 담아내기 intake 유지. 잔량 propose→ratify 라우터는 §5.1대로 보류.
- 홈 자산 `src/lib/assets/constellation-home.ts`: 6 도메인 + Museum portal 기하 유지, "Soul Core"
  잔존 라벨 제거(PRD §15 M-migrate와 합류).
- 페르소나 종합 = `propose-self-model.ts` + `proposal.ts` 경로 재사용(신규 purpose 추가).

---

## 7. Simon 결정 — CONFIRMED 2026-06-25 + 2026-07-29 home brightness 보완

| # | 결정 | 권장 |
|---|---|---|
| a | 북극성 헤드라인 밝기 = 도메인-평균 vs 구인-가중 | **홈 6 도메인 평균** (`collect` 제외). 2026-07-29 03:44 KST Simon 보완 결정 |
| b | 페르소나(모자) 개수 상한 | 1 주(主) + 보조 N, 표시 캡 3 제안 |
| c | 담아내기 자동 라우팅 = propose-only vs 비민감 조용한 이동 | **보류, 폐기 아님**. 재검토 전까지 자동 이동 없음 |
| d | 도메인별 밝기 임계(항목수 밴드) 튜닝값 | brightness.ts 기존 밴드(1–4 / 5–14 / ≥15) 차용, 운영 보정 |
| e | 홈 별 ↔ Big Dipper 기하 | 6 도메인 + Museum portal 유지. `collect`를 8번째 별로 추가하지 않음 |
...
1. **별자리 맥락 익스포터 추가**: `exportConstellationContext(userId, locale)` = ① 7 도메인 데이터
