...
|---|---|---|---|---|
| v0.2.0 APK (GitHub Releases) | 25 (08-23) | `preview` | `fffe35e2…` | **닿을 수 있다** (아래 단서) |
...
2. **`preview` OTA 발행** — v0.2.0 APK 를 옮긴다. `eas-update.yml` 이 `eas.json` 의
   `EXPO_PUBLIC_*` 를 번들로 미러링한다(워크플로 164~176행).
   발행 로그의 rv 가 `fffe35e2…` 인지 확인 = 이 단계의 검증
3. **알파 트랙에 새 빌드 게시** — 1 이후 커밋에서. **OTA 로는 안 되는 유일한 대상.**
   14일 재테스트 창 중 업데이트 게시는 Play 정책상 허용
4. 그 다음에야 `EXPO_PUBLIC_FAILOVER_VENDOR` 이동 → gemini-proxy 폐기 (윗 절 순서)

2 와 3 은 서로를 대신하지 못한다. 채널이 다르고 지문이 다르다.
