...
**`xai` 가 없다** — xai 가 서빙한 행은 벤더 NULL 로 기록된다. → **`0165_log_ai_audit_xai_vendor.sql`
로 파일 생성(2026-08-31). 파일명은 원래 `0148_` 이었고, 2026-09-06 원격 예약 충돌(0148~0164 선점)으로 `0165_` 로 재번호했다. 적용된 역사 번호는 그대로 둔다. 적용은 콘솔(dry-run + tail 대조 규율).**
