...
- **답**: 기록을 한 곳에 담고(담기·인터뷰·임포트), 그 기록만을 근거로 나를 요약하며
  ...
  붙인다.
- 연령: **적용 하한 이상**이면 자기동의 가입, 만 14세 미만은 법정대리인 동의(C10). ⚠ **[갱신 2026-09-22]** 하한은 고정 14 가 아니라 **나라별**이다(63개국 표, 법정 13~20, 14 로 클램프). 기기 지역을 읽지 못한 등록 흐름에서만 거주 국가를 묻고, "목록에 없음"은 18을 적용한다. 미성년 개방 상태는
  CLAUDE.md 해당 절이 정본.
...
- 역할형 페르소나 카드는 일곱 별에 저장한 인터뷰를 북극성에서 **명시적으로** 종합해
  최대 3개까지 제안한다. 제안은 카드로 보이지만 자동 승인되지 않는다. 승인한 카드만
  `/records` 그래프의 2단계 별이 되고, 해당 카드가 인용한 실제 기록에 연결된다.
  재생성은 승인된 기존 카드를 임의로 지우거나 바꾸지 않는다.
