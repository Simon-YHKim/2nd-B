...
```
A 입력   7 도메인 별 (홈에 보이는 별) — 커리어·재정·성장·관계·건강·오락·담아내기
...
| 6 | **오락** | 취미·독서·경험 기록 | `reading/books.ts` | 취미 폼 |
...
| 개방성 (Big Five O) | BFI-44 | 오락·담아내기·성장 | `persona/bfi.ts` |
| 외향성 (Big Five E) | BFI-44 | 관계·오락 | `persona/bfi.ts` |
...
> **주의 — 레이어 A 도메인 ≠ 내부 키.** 새 7 도메인(커리어·재정·성장·관계·건강·오락·담아내기)은
...
| 오락 | 취미·독서 `reading/books.ts` | (개방성 → BFI) | shipped(독서) |
...
| 처분 | 라우트 |
|---|---|
| **KEEP — 도메인 별 진입** | 커리어/재정/성장(`/interview`·`/audit`)/관계/건강/오락/담아내기(`/capture`) |
...
- **레이어 A (7 도메인)** = 커리어·재정·성장·관계·건강·오락·담아내기 (신규 `DOMAIN_STARS`).
