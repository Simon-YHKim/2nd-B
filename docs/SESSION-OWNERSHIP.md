...
#### Paddle 환불 무결성 0189: 웹훅을 끈 상태에서만 교체

`0189`는 DB 함수와 `paddle-webhook`을 함께 바꾸므로 위의 일반적인 서버-first 규칙만
적용해서는 안 된다. **유일한 안전 순서**는 다음과 같다.

1. `PADDLE_WEBHOOK_ENABLED`를 `1`이 아닌 값으로 설정해 웹훅을 비활성화한다.
2. `db/migrations/0189_paddle_refund_consequence_integrity.sql` 적용을 완료한다.
3. `supabase/functions/paddle-webhook` 배포를 완료한다.
4. `PADDLE_WEBHOOK_ENABLED=1`로 재활성화한다. 단, 두 산출물과 503 재시도 동작을 먼저 확인한다.

DB-first만으로는 안전하지 않다. 배포 간격에 구 Edge 함수가 서명된 최상위 `partial`을
line item 하나의 `full`만 보고 전체 환불로 승격할 수 있다. Edge-first도 안전하지 않다.
새 Edge 함수가 호출하는 `set_paddle_refund_review` RPC는 `0189`가 만들기 때문에, 먼저
배포하면 결과 기록 단계가 실패하고 이미 확정됐는지 모르는 consequence를 남길 수 있다.

어느 단계에서든 실패하면 **웹훅을 비활성 상태로 유지한 채 roll-forward**한다. `0189`와
현재 Edge 함수가 모두 정상인 상태를 만든 뒤에만 다시 켠다. 구 Edge 함수를 재배포하지 않는다.
