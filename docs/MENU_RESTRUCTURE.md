...
## 6. Open Questions (구현 전 결정)

- **Q1. 통합 입력 저장 테이블**: journal은 `records`, capture는 `sources`. 통합 시 (a) 전부 `sources`로, (b) 전부 `records`로, (c) 모드별 유지하되 읽기는 이미 mergeEvidence로 통합됨 — 셋 중?
  - 잠정 권장: **(c) 모드별 유지 + 읽기 통합 유지.** 쓰기 마이그레이션 리스크 회피. 입력 UI만 하나로.
- **Q2. 마을 도착지**: 마을 탭 시 "전용 화면"을 새로 만들까, 아니면 "records를 도메인 필터로 연 뷰"로 재사용할까?
  - 잠정 권장: **records 도메인 필터 재사용** (신규 화면 최소화).
- **Q3. 탭 5개 확정**: 그래프/담기/세컨비/공상/나 — 이대로 OK? (공상을 탭으로 올릴지 vs 나 허브 안에 둘지)
