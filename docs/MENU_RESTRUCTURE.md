...
> ⚠️ **부분 SUPERSEDED (2026-06-05):** Phase 1~5 구현 완료. 단 본 문서의 **5탭(공상 포함)·공상 독립화면** 서술은 v-final(2026-06-02) 이후 폐기 — 현행은 **4탭(그래프·담기·세컨비·나), 공상 = 세컨비 Divergent 모드**(`src/lib/nav/tabs.ts`). Phase 6(죽은 라우트 정리)는 진행 중.
...
- ✅ **Phase 3 (탭 재정의)** — #79 머지. `tab-bar.tsx` 5탭 = 그래프·담기·세컨비·공상·나
  (explore/`core-brain`·records·store/`wiki` 탭 제거; records 는 그래프 마을·Phase 4 에서 접근).
  - `/journal` → `/capture` redirect: 라우트 파일·`_layout` Stack.Screen 유지 → 진입점 19곳과
    `characterForRoute("/journal")→momo` 보존. firstRun 파라미터는 어느 화면도 소비하지 않는
    no-op 라 redirect 시 무시(동선 유지).
  - capture 일기 모드에 Lv3 게이트(`checkGate`)+무료 한도(`checkUsage`) 이식 → redirect 가
    진행도 게이트를 우회하지 않음(온보딩 Lv3 funnel·과금 한도 보존).
  - ⚠️ 결정: 게이트는 capture 일기 모드 전체에 적용되므로, 담기 일기에도 무료 2회 한도가 생김
    (사용자 확정). 별로면 PR #79 단독 revert.
  - 🔧 #83 하드닝: 탭 재정의가 드러낸 시스템 이슈 수정 — BackArrow 라우트 정합
    (`src/lib/nav/tabs.ts` `PRIMARY_TAB_PATHS` 단일 소스로 탭바·BackArrow·shell 공유),
    탭바 하단 클리어런스 중앙화(PremiumAppShell), capture hooks 크래시(React #300) 수정,
    `react-hooks/rules-of-hooks` 린트 추가.
- ✅ **Phase 4 (마을 정합)** — #86 머지. 불일치 3개 마을이 records 도메인 필터 뷰로 진입:
  work→`/records?domain=work`(was /trinity), relation→`domain=relation`(was /interview),
  taste→`domain=taste`(was /insights). 기록보관소→`/records`(전체), 배움과지식→/wiki,
  공상→/imagine 유지, relation tier-3→/interview 유지(시기별 인터뷰). `OriginShard.domain`
  을 `domainForTags`로 계산(evidence.ts, 테스트됨), records.tsx 가 마을 칩으로 필터,
  `VILLAGE_LABEL` 단일 소스(relatedness.ts). ⚠️ /trinity 는 그래프 진입점 없어짐 → Phase 5.
- ✅ **Phase 5 (나 허브)** — #92 머지. `/profile`을 3축 '나' 허브로 확장, 묻힌 화면 18개를
  축별 칩으로 노출(나의 중심·평가·분석·계정설정). Phase 4에서 그래프 진입점 잃은 `/trinity`를
  분석 섹션에 재배치(인터림 해소). 라우팅 변경 없음(본문만 확장), 칩 route는 typed `Href`.
- ⬜ **Phase 6 (정리) — 다음 1순위**: 죽은 라우트/중복 제거, journal nav row 정리, i18n·테스트.
