...
- KO/EN/ES/PT/ID × 320/390/1440px × 기본/160% 소개 글자 크기: 잘림 16/30 → 0/30. 웹 배치 검사이며 네이티브 글자 확대 확인과 구분한다.
- 공통 문서 브랜치의 `docs/session-start/multilingual-copy-260906/text-clipping.patch`를 앞선 앱 패치 다음에 확인한다. 최신 전체 검증은 같은 폴더의 `text-clipping-checks.json`을 따른다.
- 이전 픽셀 기준선 차이와 연구자료 테스트는 다른 변경에서 정리됐으며, 이번에 재실행해 확인했다.
