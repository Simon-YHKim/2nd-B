...
## Latest — 2026-08-21 01:30 KST / 결정 5건 착지: 1차 플립 = OpenAI 확정 + 구독 UX 발주 (REQ-260821-03)

> 발행: **GUI(Cowork) 콘솔 세션**. 바로 아래 00:55 블록의 발주 4건(REQ-260820-03·04, REQ-260821-01·02)은
> **전부 그대로 유효**하다. 이 블록은 그 위에 얹히는 **확정값과 신규 발주 1건**만 담는다.
> 코딩 세션은 **이 블록 + 00:55 블록**을 한 세트로 읽을 것.

### Simon 결정 (2026-08-21 01:1x KST 회신)

| 질문 | 결정 |
|---|---|
| Q-260821-04 | **추론 1차 플립 = OpenAI.** purpose 별 세분 배치(Claude·Grok 투입)는 코딩 세션이 안을 내면 Simon 이 컨펌 |
| Q-260821-03 | **xAI 계정 개설 + Anthropic 크레딧 구매 둘 다 승인.** 실측: xAI 콘솔 계정은 이미 개설돼 API keys 화면까지 진입해 있음 |
| Q-260821-01 | 취소 시트에서 환불 자격자에게 환불 요청을 **함께 제안** - 발주 (아래 REQ-260821-03) |
| Q-260821-02 | 결제 직전 **자동 갱신 조건 명시 + 동의 단계** 추가 - 발주 (아래 REQ-260821-03) |
| Q-260821-05 | Play 프로덕션 **지금 신청.** 콘솔이 신청서 3단계 9문항을 입력 완료, 「적용」 클릭만 Simon 몫으로 남김 |

**REQ-260821-01 에 대한 확정 반영:** 완료조건 1의 "Gemini 아닌 벤더" = **OpenAI 로 확정.**
멀티모달(OCR·음성) 대체 경로도 openai-proxy 계열로 우선 검토하되, 더 나은 안이 있으면 근거와 함께 제시.

### REQ-260821-03 → CLI · 구독 UX 2건 (한 PR 권장)

**왜 하는가.** Simon 질문에서 드러난 기대와 실제 동작의 간극 2곳. 서버는 이미 양쪽을 다 지원하므로
(`subscription-manage` 의 cancel/refund_request 분리 + `refund_eligibility` 판정 노출) **클라 변경만**이다.

**(a) 취소 시트에 환불 동시 제안.**
`src/app/subscription.tsx` 의 cancel 시트에서, `canRequestRefund(eligibility)` 가 참인 사용자에게
"지금 취소하면 환불 대상입니다 - 환불도 함께 요청할까요?" 를 제안한다.
- 환불 선택 시: 기존 `refund_request` 액션을 그대로 호출 (서버 변경 0줄. 자격 재판정은 서버가 이미 한다).
- 환불 없이 취소만도 여전히 가능해야 한다.
- 문구는 기존 원칙 유지: "환불 접수/requested" 라고 쓰고 "환불 완료" 라고 쓰지 않는다 (Paddle 이 심사).

**(b) 결제 직전 자동 갱신 명시 동의.**
체크아웃 진입 직전(또는 Paddle 오버레이 열기 전) 화면에 **주기·금액·해지 방법**을 명시하고
동의를 받는 단계를 넣는다.
- 표기 값은 하드코딩하지 말고 기존 가격 소스(EXPO_PUBLIC_PADDLE_PRICE_* 로 렌더하는 경로)를 재사용.
- 이 단계는 Apple 심사 가이드라인 3.1.2(구독 조건 명시) 대응이기도 하다 - iOS 재제출 전에 들어가면 좋다.
- i18n ko/en 동시.

**완료조건:** ① 자격자 취소 흐름에서 환불 제안이 보이고, 수락 시 `billing_self_service_log` 에
cancel + refund_request 두 claim 이 남는다 ② 비자격자에게는 제안이 보이지 않는다 ③ 체크아웃 직전
동의 단계 없이는 결제 진입이 불가하다 ④ `npm run verify` 그린.

**하지 말 것:** 서버(`subscription-manage`·RPC) 변경 금지. 자격 판정을 클라에서 재구현하지 말 것
(화면은 서버 verdict 의 표시만). "환불 완료" 문구 금지.
...
```
1. REQ-260820-03 (effort 계층)  →  2. REQ-260821-01 (벤더 재편, 마감 08-31, 플립=OpenAI)
→  3. REQ-260820-04 (XPRIZE 제거)  →  4. REQ-260821-03 (구독 UX 2건)  →  5. REQ-260821-02 (RBAC 설계)
```

4번은 iOS 재제출 일정에 맞춰 3번보다 앞당겨도 좋다 - (b)가 Apple 3.1.2 대응이기 때문. 판단 맡김.

### 콘솔 상태 (2026-08-21 01:2x)

| 항목 | 상태 |
|---|---|
| Play 프로덕션 신청 | **9문항 입력 완료, 「적용」 대기** (백업: `Output\260821_0130_2ndB_play_prod_application_answers_v1.md`) |
| Apple | 라이선스 계약 수락 대기 (Simon) → 그 다음 리뷰 답신 |
| xAI | 계정 개설 완료 실측. `XAI_API_KEY` 발급·입력은 REQ-260821-01 착지 후 |
| DRYRUN | `0` (완료) · 모델 핀 `gpt-5.4` 유지 |

## 2026-08-21 00:55 KST / 벤더 재편(Gemini 폐기·Grok 추가) + XPRIZE 제거 GO + RBAC 발주
