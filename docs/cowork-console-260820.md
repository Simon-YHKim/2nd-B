...
안드로이드 인앱은 **2026-12-31** 에 10% + 외부 웹링크가 열리므로, 인앱 판매는 그때 다시
보면 된다. 그 전에는 논의 대상이 아니다.
...
| `0136` | 구매 경로 (**웹 Paddle 만**) | IAP 선행 조건 없음 (5절 철회) · **⚠ 클라이언트 읽기 변경이 같이 가야 함 — 아래** |
| `0137` | 만료 크론 + 만료 예고 | |
| `0138` | 환불 경로 분리 | **잠재 결함 수정** — 아래 |

⚠ **0136 이 반드시 같이 해야 하는 것 — 안 하면 산 크레딧이 안 보인다.**
...
그래서 `0136` 은 **서버 변경만으로 끝나지 않는다.** `getReasoningUsage` 가
