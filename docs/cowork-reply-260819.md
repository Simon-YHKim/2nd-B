...
(문서 불일치 하나 — `tossbank` 는 현행 API enum 에 있는데 사람이 읽는 22개사 목록에는 없다.
어느 쪽이 맞는지 `UNVERIFIED`.)

### 9.1 "쓰여진 시점에는 거짓이었나" — 아니다, 확인했다

Paddle 이 카카오·네이버 구독을 연 것은 **2025-11-19** 이고, 우리 법률 3종이 확정·게시된 것은
**2026-07-18**(`cddc8e6e feat(legal): finalize the three legal documents with the six business facts`)
이다. **작성 시점에 이미 사실이었다.** 즉 "게시 당시에는 거짓이었다"는 문제는 없다.
남는 것은 위의 "등" 하나와, 아래 대시보드 설정 확인뿐이다.

### 9.2 환불 때 Paddle 이 수수료를 안 돌려준다 — 법적 문제는 아니고 비용이다

Paddle 정산 리포트에 `retained_fee` 항목이 있다 — "Amounts kept by Paddle during refunds to
cover gateway costs".

**이건 우리 환불정책의 "전액 환불" 약속과 충돌하지 않는다.** 구매자는 100% 돌려받는다.
돌려받지 못하는 것은 **판매자 쪽 수수료**다. 즉 환불 1건마다 Simon 은 환불 원금에 더해
**Paddle 수수료 ₩1,195 를 추가로 잃는다.**

우리 정책이 **7일 이내 사유 불문 전액 환불**이므로 이 비용은 정책상 상수다.
`refund-policy.md:16` 기준으로 환불 1건 = 실질 손실 **₩9,000(공급가) + ₩1,195(수수료)**.
법률 문서를 고칠 일은 아니고, **수익 모델에 넣어 둘 숫자**다.

### 9.3 한국 결제수단은 "저장된 수단"으로 표시되지 않는다

한국 결제수단 5종 페이지 전부에 `| Can be presented when saved? | Not supported |` 가 있다.
갱신 자체는 정상 동작한다(Paddle 이 서버 쪽에서 mandate 를 보관). 다만 **사용자에게
"저장된 결제수단" 선택 화면이 안 뜬다.** 데스크톱은 QR 스캔 방식이고, 로컬카드는 은행앱
인증(PIN·생체)을 요구한다. 구독 관리 화면 문구를 짤 때 알아 둘 것.
최소 결제금액 **₩980** (우리 가격대는 무관).
