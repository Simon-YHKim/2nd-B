...
> **⚠ XPRIZE 는 2026-08-15 에 종료됐다** (Simon 결정). **마감은 없다.**
> 이 절은 그때의 정렬 근거를 남긴 **역사 기록**이고 새 판단의 근거가 아니다.
> 마감·심사자·대회 규정·인용 가능성을 스코프 결정에 쓰지 말 것 — 루트 `CLAUDE.md`
> 의 XPRIZE 절이 그 판단을 명시적으로 금지한다. 기준은 실사용자다.
>
> 아래 세 줄의 **제품 논지는 여전히 유효하다** — 대회와 무관하게 이 앱의 근거다.

- "AI 시대 인간의 잠재력" = *자기 자신을 데이터로 가지는 것*
- 누구나 같은 답을 내는 시대의 차별화 = 나의 데이터·취향·패턴
- 이 앱은 그 자산을 *축적* 하고 *개인 비서로 키우는* 도구

원문은 아래에 보존한다. **역사 기록이지 현황이 아니다.**

> ~~Build with Gemini XPRIZE — **Education & Human Potential** 트랙. 마감: 2026-08-17 06:00 KST~~
