...
describe("museum byte-stable boundaries", () => {
  test("preserves the route, canon converter, and existing canon test", () => {
    ...
    ).toBe(// Re-pinned in the integration merge. The old value was the 5b6bbe71
    // fork-base file; main has since landed 458d3a8b (museum pack kept out of
    // the canon index so it stays off the web entry). The merged file is
    // byte-identical to main's, so this still proves the PIXEL-CLAY renderer
    // left the data converter alone -- only the baseline moved.
    "86c7fbdfc0f38ab00b2edf600aca2b2f5458d6718503517836797c05aa5cf31a");
    expect(
      sha256(
        read("src/screens/deepspace/museum/__tests__/museum-canon.test.ts"),
      ),
    ).toBe("594c1d3c6f3b769742e476679606fcada64f9909ee4c96fc516c56ef42ebbbda");
  });
});
