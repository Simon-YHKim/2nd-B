    ...
    <>
      ...
      <View style={styles.modeRow}>
        {["글", "사진", "링크", "음성", "할 일"].map((mode) => (
          ...
          >
            <Text style={[styles.modeChip, captureMode === mode && styles.modeChipActive]}>{mode}</Text>
          </Pressable>
        ))}
      </View>
      ...
      >
        <Text style={styles.primaryButtonText}>{captured ? "담겼어요" : "담기"}</Text>
      </Pressable>
    </>
  );
}
...
function SecondbContent({ chatDraft, setChatDraft, chatSent, setChatSent }: HubState) {
  return (
    <View style={styles.chatStack}>
      ...
      {chatSent ? <Text style={styles.sentNote}>보냈어요. 실제 대화 저장 연결은 다음 단계 TODO입니다.</Text> : null}
...
function TrendContent({ trendAction, setTrendAction }: HubState) {
  return (
    <>
      ...
      {trendAction ? <Text style={styles.sentNote}>{trendAction} 제안을 열 준비가 됐어요.</Text> : null}
...
function ReviewContent({ reviewDecision, setReviewDecision }: HubState) {
  return (
    <>
      ...
      {reviewDecision ? <Text style={styles.sentNote}>{reviewDecision === "approve" ? "승인됨" : "보류됨"}</Text> : null}
      <View style={styles.actionRow}>
        <Pressable accessibilityRole="button" accessibilityLabel="제안 보류" onPress={() => setReviewDecision("hold")} style={({ pressed }) => [styles.secondaryButton, pressed && styles.pressedButton]}><Text style={styles.secondaryButtonText}>보류</Text></Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="제안 승인" onPress={() => setReviewDecision("approve")} style={({ pressed }) => [styles.soulButton, pressed && styles.pressedButton]}><Text style={styles.soulButtonText}>승인</Text></Pressable>
      </View>
    </>
  );
}
...
function TrendCard({ title, delta, body, onPress }: { title: string; delta: string; body: string; onPress: () => void }) {
  return <Pressable accessibilityRole="button" accessibilityLabel={`${title} 제안 열기`} onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressedButton]}><View style={styles.cardHead}><Text style={styles.cardTitle}>{title}</Text><Text style={styles.delta}>{delta}</Text></View><Text style={styles.cardBody}>{body}</Text></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  sentNote: { marginTop: spacing.sm, color: colors.mint, fontFamily: fontFamilies.readable, fontSize: 11, lineHeight: 15, textAlign: "center" },
