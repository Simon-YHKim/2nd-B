...
/** Remove only the terminally deleted owner's device-local account link. */
export async function purgeGithubUsernameForDeletedAccount(userId: string): Promise<boolean> {
  try {
    const key = keyFor(userId);
    const native = nativeStorage();
    if (native) {
