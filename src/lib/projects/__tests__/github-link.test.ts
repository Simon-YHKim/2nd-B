...
import {
  getGithubUsername,
  purgeGithubUsernameForDeletedAccount,
  setGithubUsername,
} from "../github-link";
import {
...
const mockFenceBacking = new Map<string, string>();
...
describe("GitHub username ownership", () => {
  ...
  it("rejects malformed usernames without clearing a valid saved value", async () => {
    ...
    await expect(setGithubUsername("user-a", null as unknown as string)).rejects.toThrow(
      "invalid_github_username",
    );
  ...
  it("terminal deletion purges only the deleted owner's username", async () => {
    await setGithubUsername("user-a", "octocat-a");
    await setGithubUsername("user-b", "octocat-b");

    await expect(purgeGithubUsernameForDeletedAccount("user-a")).resolves.toBe(true);
    expect(await getGithubUsername("user-a")).toBe("");
    expect(await getGithubUsername("user-b")).toBe("octocat-b");
  });

  it("terminal deletion reports a failed local purge", async () => {
    mockEncryptedStorage.removeItem.mockRejectedValueOnce(new Error("disk unavailable"));
    await expect(purgeGithubUsernameForDeletedAccount("user-a")).resolves.toBe(false);
  });

  it("blocks a fenced owner's late save and clear without affecting another owner", async () => {
    await setGithubUsername("user-a", "octocat-a");
    await setGithubUsername("user-b", "octocat-b");
    await expect(installAccountLocalDeletionFence("user-a")).resolves.toBe(true);

    await setGithubUsername("user-a", "late-a");
    await setGithubUsername("user-a", "");
    await setGithubUsername("user-b", "new-b");

    expect(await getGithubUsername("user-a")).toBe("octocat-a");
    expect(await getGithubUsername("user-b")).toBe("new-b");
  });
});
