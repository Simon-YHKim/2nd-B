...
const peer = readMigration("0182_peer_response_atomicity.sql");
const naver = readMigration("0183_oauth_naver_rate_limit.sql");
const purposeQuota = readMigration("0185_llm_proxy_purpose_quota.sql");
...
describe("integrated security migration batch C", () => {
  test("0182 binds every peer child row to the locked invitation and freezes identity", () => {
    expect(peer).toMatch(
  ...
  test("0182 makes invitation identity immutable and only permits forward state transitions", () => {
  ...
  test("0182 forces RLS and leaves only the service RPC executable", () => {
  ...
  test("0183 is re-applicable, atomic, fixed-window, and service-only", () => {
    ...
    expect(naver).toMatch(
      /FOREACH v_table IN ARRAY ARRAY\[[\s\S]*'public\.oauth_preauth_rate_limits'[\s\S]*has_table_privilege\(\s*'anon', v_table/,
    );
  });

  test("0185 recognizes exactly the app purpose vocabulary and validates its proxy seat", () => {
  ...
  test("0185 shares the purpose counter across vendors and derives every cap server-side", () => {
