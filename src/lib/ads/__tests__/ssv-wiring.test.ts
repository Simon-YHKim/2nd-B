...
describe("SSV callback wiring", () => {
  ...
  test("native exchanges the authenticated placement hint for server SSV values", () => {
    ...
    expect(native).not.toMatch(/serverSideVerificationOptions:\s*\{[^}]*userId/);
  ...
  test("provider data is limited to the opaque ticket and the DPIA says so", () => {
    const native = read("src/lib/ads/rewarded.native.ts");
    const types = read("src/lib/ads/types.ts");
    const dpia = read("docs/legal/DPIA-2ndB-minors-draft.md");
    expect(native).not.toMatch(/serverSideVerificationOptions:\s*\{[^}]*userId/);
    expect(types).toContain("local placement hint");
    expect(dpia).toMatch(/opaque[^|]*SSV ticket/i);
    expect(dpia).toMatch(/account UUID is not sent/i);
  });
