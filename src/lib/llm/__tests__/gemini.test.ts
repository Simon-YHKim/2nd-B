...
describe("callGemini", () => {
  ...
  test("multimodal: image is attached as an inlineData part on the direct/Vertex path", async () => {
    await callGemini({
      ...
      image: { mimeType: " IMAGE/X-PNG; charset=binary ", data: `${PNG_IMAGE_BASE64.slice(0, 8)}\n${PNG_IMAGE_BASE64.slice(8)}` },
    });
    expect(mockGenerateContent).toHaveBeenCalledTimes(1);
  ...
  test("multimodal: data URL images are normalized before the direct/Vertex SDK call", async () => {
    await callGemini({
      userId: "u1",
      locale: "en",
      purpose: "capture_ocr",
  ...
  test("multimodal: generic data URL images are sniffed before the direct/Vertex SDK call", async () => {
    await callGemini({
      userId: "u1",
      locale: "en",
      purpose: "capture_ocr",
      user: "Transcribe the text in this image.",
      image: { mimeType: "", data: `data:application/octet-stream;base64,${PNG_IMAGE_BASE64}` },
    });
  ...
  test("multimodal: percent-encoded data URL images are decoded before the direct/Vertex SDK call", async () => {
    await callGemini({
      userId: "u1",
      locale: "en",
      purpose: "capture_ocr",
      user: "Transcribe the text in this image.",
      image: { mimeType: "image/jpeg", data: `data:image/jpeg;base64,${encodeURIComponent(JPEG_PADDED_IMAGE_BASE64)}` },
    });
  ...
  test("multimodal: invalid image data is rejected before the direct/Vertex SDK call", async () => {
    ...
    await expect(
      callGemini({
        ...
        purpose: "capture_ocr",
        user: "Transcribe the text in this image.",
        image: { mimeType: "image/jpeg", data: "data:image/jpeg;base64,%E0%A4%A" },
      }),
    ).rejects.toThrow("llm_image_invalid_data");
