...
type BodyApi = {
  ...
  LLM_PROXY_REQUEST_BODY_TIMEOUT_MS: number;
...
describe("shared bounded JSON/body readers", () => {
  ...
  test("fixes independent finite request and upstream byte limits", () => {
    ...
    expect(api.LLM_PROXY_REQUEST_BODY_TIMEOUT_MS).toBe(15_000);
  ...
  test("cancels a stalled request body at the shared read deadline", async () => {
    jest.useFakeTimers();
    try {
      let cancelled = false;
      ...
      const read = api.readLlmProxyJsonObject(request);
      const outcome = Promise.race([
        read.then(
          () => ({ status: "resolved" as const }),
          (error: unknown) => ({ status: "rejected" as const, error }),
        ),
        new Promise<{ status: "deadline" }>((resolveDeadline) => {
          setTimeout(
            () => resolveDeadline({ status: "deadline" }),
            api.LLM_PROXY_REQUEST_BODY_TIMEOUT_MS,
          );
        }),
      ]);

      await jest.advanceTimersByTimeAsync(api.LLM_PROXY_REQUEST_BODY_TIMEOUT_MS);
  ...
  test("rejects zero-progress and over-fragmented request streams", async () => {
    let zeroPulls = 0;
    let zeroCancelled = false;
    const zeroProgress = new ReadableStream<Uint8Array>({
      pull(controller) {
        zeroPulls += 1;
        if (zeroPulls <= 100) controller.enqueue(new Uint8Array());
    ...
    await expect(api.readLlmProxyJsonObject({
      body: zeroProgress,
      headers: new Headers(),
    } as Request)).rejects.toMatchObject({ code: "invalid_json" });
    expect(zeroPulls).toBeLessThan(100);
    expect(zeroCancelled).toBe(true);

    let fragmentedPulls = 0;
    let fragmentedCancelled = false;
    const fragmented = new ReadableStream<Uint8Array>({
      pull(controller) {
        fragmentedPulls += 1;
        if (fragmentedPulls <= 2_000) controller.enqueue(Uint8Array.of(0x20));
        else if (fragmentedPulls === 2_001) controller.enqueue(new TextEncoder().encode("{}"));
        else controller.close();
      },
      cancel() {
    ...
    expect(fragmentedPulls).toBeLessThan(2_000);
    expect(fragmentedCancelled).toBe(true);
  });
...
describe("Gemini prompt-block response", () => {
  const source = sources["gemini-proxy"];

  test("accepts a validated blockReason without candidates so it reaches the blocked path", () => {
    const responseRead = source.indexOf("data = await readLlmUpstreamJsonObject(upstream)");
    const blockValidation = source.indexOf("promptBlockReason =", responseRead);
    const candidateValidation = source.indexOf("if (!Array.isArray(data.candidates))", responseRead);
    const blockedResponse = source.indexOf("if (blockReason)", candidateValidation);

    expect(responseRead).toBeGreaterThan(-1);
    expect(blockValidation).toBeGreaterThan(responseRead);
    expect(candidateValidation).toBeGreaterThan(blockValidation);
    expect(blockedResponse).toBeGreaterThan(candidateValidation);
    expect(source.slice(candidateValidation, blockedResponse)).toMatch(
      /if \(!Array\.isArray\(data\.candidates\)\) \{[\s\S]*if \(!promptBlockReason\)[\s\S]*upstream_bad_payload/,
    );
  });
});
