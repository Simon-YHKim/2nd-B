...
describe("withDomainTag — typed domainIntent (별 담기 provenance)", () => {
  it("neutral text + growth intent -> exactly domain:growth", () => {
    // 키워드가 하나도 없는 본문이라 detector 는 collect 로 갔을 것 — intent 가
    // 별 담기의 출발 별을 지킨다.
    expect(withDomainTag([], "그냥 떠오른 생각", "growth")).toEqual(["domain:growth"]);
  });

  it("career-like text + growth intent -> intent wins over the detector", () => {
    // detector 단독이면 career (면접·회사). 별 화면에서 시작한 담기는 그 별이 우선.
    expect(withDomainTag(["keep"], "회사 면접 준비", "growth")).toEqual([
      "domain:growth",
      "keep",
    ]);
  });

  it("intent present still strips raw domain:* tags (exactly one domain tag)", () => {
    expect(withDomainTag(["domain:finance", "keep"], "메모", "growth")).toEqual([
      "domain:growth",
      "keep",
    ]);
  });

  it("raw domain tag + no intent -> detector still owns the slug (no hijack)", () => {
    expect(withDomainTag(["domain:finance", "keep"], "면접 회의")).toEqual([
      "domain:career",
      "keep",
    ]);
  ...
  it("runtime-invalid intent falls back to the detector", () => {
    // 타입은 컴파일 타임 방벽일 뿐 — 딥링크 문자열이 근원이므로 런타임 검증
    // (isDomainId)이 최종 게이트다. 무효 값은 없는 것처럼 동작해야 한다.
    expect(withDomainTag([], "회사 면접 준비", "hacker" as never)).toEqual(["domain:career"]);
    expect(withDomainTag([], "메모", "" as never)).toEqual(["domain:collect"]);
  });

  it("no intent argument keeps the pre-intent behavior byte-for-byte", () => {
    expect(withDomainTag(["mine"], "회사 면접")).toEqual(["domain:career", "mine"]);
    expect(withDomainTag(undefined, "음...")).toEqual(["domain:collect"]);
  });
});
