...
// classify whatever the user pasted. DOCX text extraction is web-only and
// loaded on demand. PDF remains a metadata-only attachment because compressed
// streams cannot be given a reliable browser-side heap ceiling.
//
// Native (iOS/Android) DOCX support is deferred. Unsupported binary formats
// leave textContent null so the app can retain filename + MIME metadata.
...
const MAX_DOCX_DEFLATE_CHUNKS = 65_536;
const MAX_DOCX_EMPTY_CHUNKS = 64;
const DOCX_DEFLATE_YIELD_INTERVAL_CHUNKS = 1_024;
const EXTRACTION_CLEANUP_GRACE_MS = 100;
...
function waitForCleanup(result: Promise<unknown> | unknown): Promise<void> {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(finish, EXTRACTION_CLEANUP_GRACE_MS);
...
export async function extractText(
  ...
  // Compressed PDF streams can expand far beyond the selected file size before
  // page/text caps run. Keep the attachment UX, but never fetch or parse it.
  if (PDF_MIMES.has(normalizedMimeType)) return null;
  ...
  try {
    return await runWithExtractionDeadline(async (deadline) => {
      ...
      // DOCX extraction is web-only. Native picks the file but the
...
async function assertSafeDocx(bytes: Uint8Array, deadline: ExtractionDeadline): Promise<void> {
  ...
  for (let index = 0; index < entries; index++) {
    ...
    requireRange(
      localOffset,
      30 + localNameBytes + localExtraBytes + compressedBytes,
      centralOffset,
    );
    if (
      localFlags !== flags ||
...
async function verifyDocxEntrySizes(
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      actualBytes > MAX_DOCX_ENTRY_UNCOMPRESSED_BYTES
    ) {
      throw new BoundedFileReadError("read_failed");
    }
    actualTotal += actualBytes;
    if (actualTotal > MAX_DOCX_TOTAL_UNCOMPRESSED_BYTES)
      throw new BoundedFileReadError("read_failed");
  }
}
...
async function countInflatedBytes(
  ...
  const DecompressionStreamCtor = globalThis.DecompressionStream as unknown as
    ...
    | undefined;
  if (
    typeof DecompressionStreamCtor !== "function" ||
  ...
  try {
    while (true) {
      ...
      chunks += 1;
      if (chunks > MAX_DOCX_DEFLATE_CHUNKS) throw new BoundedFileReadError("read_failed");
      if (chunk.byteLength === 0) {
        emptyChunks += 1;
        if (emptyChunks > MAX_DOCX_EMPTY_CHUNKS) throw new BoundedFileReadError("read_failed");
      } else {
        emptyChunks = 0;
      }
      const cap = Math.min(
