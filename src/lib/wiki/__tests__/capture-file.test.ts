...
describe("extractText", () => {
  ...
  test("never traverses more than the PDF page ceiling", async () => {
    ...
    await expect(
      extractText("file:///many-pages.pdf", "application/pdf", 8),
  ...
  test("does not let a stalled PDF cleanup consume the full extraction deadline", async () => {
    jest.useFakeTimers();
    const pdfjs = require("pdfjs-dist") as { getDocument: jest.Mock };
    const destroy = jest.fn(() => new Promise<never>(() => undefined));
    pdfjs.getDocument.mockReturnValueOnce({
      destroy,
      promise: Promise.resolve({
        numPages: 1,
        getPage: () =>
          Promise.resolve({
            getTextContent: () => Promise.resolve({ items: [{ str: "bounded cleanup" }] }),
          }),
      }),
    });
    mockFetch(new ArrayBuffer(8));

    const result = extractText("file:///cleanup.pdf", "application/pdf", 8);
    for (let turn = 0; turn < 30 && destroy.mock.calls.length === 0; turn++) {
      await Promise.resolve();
    }
    expect(destroy).toHaveBeenCalledTimes(1);
    const settled = expect(result).resolves.toContain("bounded cleanup");
