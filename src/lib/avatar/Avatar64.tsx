 ...
 *
 * 웹 인수 번들을 런타임에 import하거나 SVG 문자열을 실행하지 않는다. 도형은
 * 정수 좌표의 rect 튜플뿐이며, 직업은 프로필 캐릭터 위에 역할 레이어로 합성된다.
 */
export const AVATAR_GRID = 64 as const;
export const AVATAR_HEAD_CROP = { minX: 12, minY: 0, width: 40, height: 40 } as const;

export type AvatarSeed = string | number;
export type AvatarType = "human" | "animal";
export type AvatarRect = [x: number, y: number, width: number, height: number, fill: string];
...
export interface AvatarHair extends AvatarCatalogItem {
  f(color: string): AvatarRect[];
}

export interface AvatarAccessory extends AvatarCatalogItem {
  f(palette: Record<string, string>): AvatarRect[];
}

export interface AvatarFaceDetail extends AvatarCatalogItem {
  f(palette: Record<string, string>): AvatarRect[];
}

export interface AvatarExpression extends AvatarCatalogItem {
  brow: string;
  eye: string;
  mouth: string;
  tear?: boolean;
...
export interface AvatarAnimal extends AvatarCatalogItem {
  fur?: readonly string[];
  pre?: (fur: string) => AvatarRect[];
  ear?: string;
  earColor?: string;
  inner?: string;
  ownEyes?: boolean;
  post?: (fur: string) => AvatarRect[];
}

export type AvatarJobGroup = AvatarCatalogItem;

export interface AvatarJob extends AvatarCatalogItem {
  group: string;
  cloth: string;
  hat: ((hairColor: string) => AvatarRect[]) | null;
  extra: ((cloth: string) => AvatarRect[]) | null;
}

export interface AvatarSpec {
  v: typeof AVATAR_GRID;
  seed: AvatarSeed;
  type: AvatarType;
  skin: string;
  hairColor: string;
...
/** 16-grid 시절의 숫자 인덱스 저장본도 좁혀 받을 수 있는 입력 경계. */
export interface AvatarSpecInput {
  v?: number;
...
// 이 선언 하나를 무시하면 아래 138종 정수 rect 데이터의 원본 줄 구조를 보존할 수 있다.
// 새 제품 코드는 이 블록을 직접 읽지 않고, 아래의 타입 경계를 통해서만 접근한다.
// prettier-ignore
const ENGINE = (function () {
  var K = '#241c18', W = '#f7f2e8';

  function h2(n: any) { return n.toString(16).padStart(2, '0'); }
  function parse(h: any) {
    if (typeof h !== 'string' || h.charAt(0) !== '#') return [128, 128, 128];
    var n = parseInt(h.slice(1), 16); return [n >> 16 & 255, n >> 8 & 255, n & 255];
  }
  function toHex(a: any) { return '#' + a.map(function(v: any) { return h2(Math.max(0, Math.min(255, Math.round(v)))); }).join(''); }
  function shade(h: any, t: any) { var c = parse(h); var f = function(v: any) { return t < 0 ? v * (1 + t) : v + (255 - v) * t; }; return toHex([f(c[0]), f(c[1]), f(c[2])]); }
  function mix(a: any, b: any, t: any) { var x = parse(a), y = parse(b); return toHex([0, 1, 2].map(function(i: any) { return x[i] + (y[i] - x[i]) * t; })); }
  function lum(h: any) { var c = parse(h); return (c[0] * 299 + c[1] * 587 + c[2] * 114) / 1000; }
  function ink(h: any) { return lum(h) > 150 ? shade(h, -0.62) : shade(h, -0.3); }
  ...
  function hash32(s: any) { var h = 2166136261 >>> 0; for (var i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619) >>> 0; } return h >>> 0; }
  function rngOf(seed: any) { var a = hash32(String(seed)) || 1; return function() { a ^= a << 13; a >>>= 0; a ^= a >> 17; a ^= a << 5; a >>>= 0; return a / 4294967296; }; }
  function pick(r: any, arr: any) { return arr[Math.floor(r() * arr.length) % arr.length]; }

  /* ══ 머리통 · 몸통 ═══════════════════════════════════════════════ */
  function headBase(s: any) {
    var sd = shade(s, -0.17), hl = shade(s, 0.15), o: AvatarRect[] = [];
    var R = function(x: any, y: any, w: any, h: any, c: any) { o.push([x, y, w, h, c]); };
  ...
  function torso(s: any, c: any) {
    var cd = shade(c, -0.2), cl = shade(c, 0.14), o: AvatarRect[] = [];
    var R = function(x: any, y: any, w: any, h: any, cc: any) { o.push([x, y, w, h, cc]); };
  ...
  var EXPR_BY_ID: Record<string, any> = {}; EXPR.forEach(function(e: any) { EXPR_BY_ID[e.id] = e; });

  function eyePair(o: any, mode: any, eye: any, s: any) {
    var R = function(x: any, y: any, w: any, h: any, c: any) { o.push([x, y, w, h, c]); };
    function one(x: any, m: any) {
  ...
  function face(s: any, eye: any, hc: any, exprId: any, noEyes?: any) {
    var E = EXPR_BY_ID[exprId] || EXPR[0];
    var sd = shade(s, -0.3), lip = mix(s, '#a8433f', 0.5), o: AvatarRect[] = [];
    var R = function(x: any, y: any, w: any, h: any, c: any) { o.push([x, y, w, h, c]); };
  ...
  function HA(id: any, ko: any, en: any, f: any) { return { id: id, ko: ko, en: en, f: f }; }
  var HAIR = [
    HA('short', '숏컷', 'Short', function(c: any) { return cap(c).concat([[18, 18, 3, 7, c], [43, 18, 3, 7, c], [18, 18, 3, 3, shade(c, 0.15)]]); }),
    HA('long', '롱', 'Long', function(c: any) { return cap(c).concat([[17, 18, 4, 26, c], [43, 18, 4, 26, c], [16, 30, 2, 12, c], [48, 30, 2, 12, c], [17, 40, 4, 4, shade(c, -0.25)], [43, 40, 4, 4, shade(c, -0.25)]]); }),
    HA('bob', '단발', 'Bob', function(c: any) { return cap(c).concat([[17, 18, 4, 15, c], [43, 18, 4, 15, c], [17, 31, 4, 2, shade(c, -0.22)], [43, 31, 4, 2, shade(c, -0.22)]]); }),
    HA('bun', '번', 'Bun', function(c: any) { return cap(c).concat([[27, 2, 10, 7, c], [29, 1, 6, 2, shade(c, 0.24)], [18, 18, 3, 5, c], [43, 18, 3, 5, c]]); }),
    HA('spike', '스파이크', 'Spiky', function(c: any) { var hl = shade(c, 0.22); return [[20, 14, 24, 4, c], [22, 12, 20, 2, c], [21, 6, 4, 8, c], [27, 3, 4, 11, c], [33, 4, 4, 10, c], [39, 7, 4, 7, c], [21, 6, 4, 3, hl], [27, 3, 4, 3, hl], [33, 4, 4, 3, hl]]; }),
    HA('buzz', '버즈컷', 'Buzz', function(c: any) { var d = shade(c, -0.15); return [[24, 11, 16, 2, d], [22, 13, 20, 2, d], [20, 15, 24, 2, d], [24, 11, 7, 2, shade(c, 0.18)]]; }),
    HA('wavy', '웨이브', 'Wavy', function(c: any) { var d = shade(c, -0.2); return cap(c).concat([[17, 18, 4, 6, c], [19, 24, 3, 5, c], [17, 29, 4, 5, d], [43, 18, 4, 6, c], [42, 24, 3, 5, c], [43, 29, 4, 5, d]]); }),
    HA('curly', '곱슬', 'Curly', function(c: any) { var hl = shade(c, 0.2); return [[20, 6, 24, 4, c], [17, 9, 30, 7, c], [15, 14, 5, 8, c], [44, 14, 5, 8, c],
      [21, 4, 6, 4, c], [30, 3, 7, 4, c], [38, 5, 6, 4, c], [21, 5, 5, 3, hl], [30, 4, 5, 3, hl], [17, 10, 6, 4, hl]]; }),
    HA('ponytail', '포니테일', 'Ponytail', function(c: any) { return cap(c).concat([[18, 18, 3, 6, c], [43, 18, 3, 6, c], [46, 20, 6, 5, c], [47, 25, 6, 12, c], [48, 37, 4, 6, shade(c, -0.22)], [44, 19, 4, 4, '#bf4a44']]); }),
    HA('twintail', '트윈테일', 'Twin tails', function(c: any) { return cap(c).concat([[13, 21, 5, 4, c], [12, 25, 5, 13, c], [46, 21, 5, 4, c], [47, 25, 5, 13, c], [12, 36, 5, 3, shade(c, -0.22)], [47, 36, 5, 3, shade(c, -0.22)], [17, 19, 4, 3, '#e278a1'], [43, 19, 4, 3, '#e278a1']]); }),
    HA('mohawk', '모히칸', 'Mohawk', function(c: any) { var hl = shade(c, 0.22), d = shade(c, -0.35); return [[28, 1, 8, 16, c], [30, 0, 4, 2, c], [28, 1, 4, 8, hl], [20, 14, 8, 3, d], [36, 14, 8, 3, d]]; }),
    HA('afro', '아프로', 'Afro', function(c: any) { var hl = shade(c, 0.2), d = shade(c, -0.2); return [[20, 0, 24, 3, c], [16, 2, 32, 4, c], [12, 5, 40, 8, c], [10, 9, 44, 8, c], [12, 16, 8, 8, c], [44, 16, 8, 8, c],
      [20, 1, 12, 3, hl], [16, 4, 8, 4, hl], [12, 8, 6, 5, hl], [44, 12, 8, 6, d], [12, 21, 8, 3, d]]; }),
    HA('sidepart', '사이드파트', 'Side part', function(c: any) { var hl = shade(c, 0.24), d = shade(c, -0.2); return [[24, 8, 16, 2, c], [22, 10, 20, 2, c], [20, 12, 24, 2, c], [18, 14, 28, 4, c],
      [18, 18, 12, 3, c], [18, 18, 3, 6, c], [43, 18, 3, 5, d], [24, 9, 6, 3, hl], [33, 12, 1, 6, d]]; }),
    HA('fringe', '뱅', 'Fringe', function(c: any) { var hl = shade(c, 0.2); return cap(c).concat([[18, 18, 28, 4, c], [18, 22, 5, 4, c], [41, 22, 5, 4, c], [22, 18, 8, 3, hl]]); }),
    HA('braid', '땋은머리', 'Braid', function(c: any) { var d = shade(c, -0.25); return cap(c).concat([[18, 18, 3, 5, c], [43, 18, 3, 5, c],
      [45, 22, 5, 4, c], [45, 26, 5, 4, d], [45, 30, 5, 4, c], [45, 34, 5, 4, d], [46, 38, 3, 3, c]]); }),
    HA('undercut', '언더컷', 'Undercut', function(c: any) { return [[22, 10, 20, 2, c], [20, 12, 24, 2, c], [18, 14, 28, 3, c], [18, 17, 11, 2, c], [22, 10, 7, 2, shade(c, 0.22)], [18, 19, 3, 3, shade(c, -0.2)], [43, 17, 3, 4, shade(c, -0.2)]]; }),
    HA('topknot', '상투', 'Top knot', function(c: any) { return cap(c).concat([[29, 1, 6, 5, c], [28, 5, 8, 3, shade(c, -0.2)], [30, 1, 3, 2, shade(c, 0.24)], [18, 18, 3, 4, c], [43, 18, 3, 4, c]]); }),
    HA('dreads', '드레드', 'Dreads', function(c: any) { var d = shade(c, -0.24); return cap(c).concat([[17, 18, 4, 22, c], [22, 18, 3, 16, d], [39, 18, 3, 16, d], [43, 18, 4, 22, c],
      [17, 26, 4, 3, d], [43, 26, 4, 3, d], [17, 34, 4, 3, d], [43, 34, 4, 3, d]]); }),
    HA('pixie', '픽시', 'Pixie', function(c: any) { var hl = shade(c, 0.22); return [[24, 8, 16, 2, c], [22, 10, 20, 2, c], [20, 12, 24, 3, c], [18, 15, 28, 3, c],
      [17, 18, 4, 6, c], [43, 18, 4, 6, c], [40, 10, 6, 3, c], [24, 9, 7, 2, hl]]; }),
    HA('hime', '히메컷', 'Hime', function(c: any) { var d = shade(c, -0.22); return cap(c).concat([[18, 18, 28, 4, c], [16, 18, 5, 14, c], [43, 18, 5, 14, c],
      [16, 30, 5, 2, d], [43, 30, 5, 2, d], [19, 32, 3, 12, c], [42, 32, 3, 12, c]]); }),
    HA('wavelong', '롱웨이브', 'Long wave', function(c: any) { var d = shade(c, -0.2); return cap(c).concat([[17, 18, 4, 10, c], [15, 28, 4, 8, c], [17, 36, 4, 8, d], [43, 18, 4, 10, c], [45, 28, 4, 8, c], [43, 36, 4, 8, d]]); }),
    HA('halfup', '하프업', 'Half up', function(c: any) { return cap(c).concat([[27, 3, 10, 6, c], [29, 2, 6, 2, shade(c, 0.24)], [17, 18, 4, 16, c], [43, 18, 4, 16, c], [17, 32, 4, 2, shade(c, -0.22)], [43, 32, 4, 2, shade(c, -0.22)]]); }),
    HA('messy', '덥수룩', 'Messy', function(c: any) { var hl = shade(c, 0.2), d = shade(c, -0.2); return cap(c).concat([[16, 12, 4, 8, c], [44, 12, 4, 8, c],
      [21, 5, 5, 5, c], [30, 4, 4, 6, c], [37, 6, 5, 4, c], [18, 18, 4, 6, d], [42, 18, 4, 6, d], [21, 6, 4, 3, hl]]); }),
    HA('bald', '민머리', 'Bald', function() { return []; })
  ];

  /* ══ 액세서리 20종 ═══════════════════════════════════════════════ */
  function AC(id: any, ko: any, en: any, f: any) { return { id: id, ko: ko, en: en, f: f }; }
  var ACC = [
    AC('none', '없음', 'None', function() { return []; }),
    AC('headband', '머리띠', 'Headband', function(p: any) { return [[17, 15, 30, 4, p.cloth], [17, 15, 30, 1, shade(p.cloth, 0.25)]]; }),
    AC('beanie', '비니', 'Beanie', function(p: any) { return [[18, 4, 28, 12, p.cloth], [22, 1, 20, 4, p.cloth], [16, 13, 32, 5, shade(p.cloth, -0.2)], [29, 0, 6, 3, shade(p.cloth, 0.2)]]; }),
    AC('capback', '캡(뒤로)', 'Cap', function(p: any) { return [[19, 6, 26, 9, p.cloth], [17, 13, 30, 3, shade(p.cloth, -0.2)], [45, 13, 8, 4, p.cloth], [19, 6, 12, 3, shade(p.cloth, 0.2)]]; }),
    ...
    AC('bandana', '반다나', 'Bandana', function(p: any) { return [[17, 12, 30, 6, p.cloth], [15, 16, 6, 10, p.cloth], [15, 24, 6, 3, shade(p.cloth, -0.25)], [20, 13, 8, 2, shade(p.cloth, 0.2)]]; }),
    ...
    AC('catears', '고양이 귀', 'Cat ears', function(p: any) { return earTri(18, 10, p.hairColor, '#e0709b').concat(earTri(36, 10, p.hairColor, '#e0709b')); }),
    ...
    AC('scarf', '목도리', 'Scarf', function(p: any) { var c = p.cloth2 || '#bf4a44'; return [[22, 41, 20, 6, c], [20, 44, 24, 4, shade(c, -0.2)], [38, 47, 7, 14, c], [38, 58, 7, 3, shade(c, -0.3)], [24, 42, 8, 2, shade(c, 0.2)]]; }),
    AC('bowtie', '나비넥타이', 'Bow tie', function() { return [[22, 45, 8, 7, '#bf4a44'], [34, 45, 8, 7, '#bf4a44'], [30, 46, 4, 5, '#8f2f2a'], [23, 46, 3, 3, '#d9605a']]; }),
    AC('hoodup', '후드', 'Hood up', function(p: any) { var c = p.cloth; return [[14, 8, 36, 12, shade(c, -0.15)], [12, 16, 8, 30, shade(c, -0.3)], [44, 16, 8, 30, shade(c, -0.3)], [18, 6, 28, 4, c], [14, 8, 12, 4, shade(c, 0.15)]]; }),
    ...
    AC('freckleset', '헤어밴드+핀', 'Band + pin', function(p: any) { return [[17, 15, 30, 3, p.cloth], [38, 11, 8, 3, '#e0a63c'], [21, 30, 4, 2, mix(p.skin, '#a8433f', 0.3)], [39, 30, 4, 2, mix(p.skin, '#a8433f', 0.3)]]; })
  ];
  /* 직업 모자와 같은 머리 위 자리를 쓰는 소품만 양보한다. */
  var HEADTOP: Record<string, number> = { headband:1, beanie:1, capback:1, flowerpin:1, headphone:1,
    bandana:1, hairclip:1, halo:1, catears:1, crownsm:1, hoodup:1,
    antenna:1, horns:1, visor:1, freckleset:1 };

  /* ══ 얼굴 디테일 14종 ════════════════════════════════════════════ */
  function glassesFrame(c: any, round?: any): AvatarRect[] {
  ...
  function FC(id: any, ko: any, en: any, f: any) { return { id: id, ko: ko, en: en, f: f }; }
  var FACE = [
    ...
    FC('beard', '턱수염', 'Beard', function(p: any) { var c = shade(p.hairColor, -0.12); return [[18, 30, 4, 10, c], [42, 30, 4, 10, c], [20, 38, 24, 8, c], [24, 44, 16, 4, c], [26, 32, 12, 2, c], [28, 40, 8, 3, shade(c, -0.2)]]; }),
    FC('mustache', '콧수염', 'Mustache', function(p: any) { var c = shade(p.hairColor, -0.12); return [[26, 31, 12, 3, c], [24, 32, 3, 2, c], [37, 32, 3, 2, c]]; }),
    FC('goatee', '염소수염', 'Goatee', function(p: any) { var c = shade(p.hairColor, -0.12); return [[27, 31, 10, 2, c], [29, 37, 6, 6, c], [30, 43, 4, 2, shade(c, -0.2)]]; }),
    FC('stubble', '무정면도', 'Stubble', function(p: any) { var c = mix(p.skin, shade(p.hairColor, -0.2), 0.45); return [[21, 33, 22, 6, c], [24, 39, 16, 3, c], [26, 30, 12, 2, c]]; }),
    FC('freckles', '주근깨', 'Freckles', function(p: any) { var c = mix(p.skin, '#a8433f', 0.42); return [[22, 30, 2, 2, c], [25, 32, 2, 2, c], [21, 33, 2, 2, c], [38, 30, 2, 2, c], [41, 32, 2, 2, c], [37, 33, 2, 2, c]]; }),
    FC('blush', '볼터치', 'Blush', function(p: any) { var c = mix(p.skin, '#e0709b', 0.55); return [[20, 29, 6, 5, c], [38, 29, 6, 5, c]]; }),
    FC('scar', '흉터', 'Scar', function(p: any) { var c = mix(p.skin, '#a8433f', 0.55); return [[37, 18, 2, 12, c], [35, 20, 2, 2, c], [39, 25, 2, 2, c]]; }),
    FC('mole', '점', 'Mole', function(p: any) { return [[26, 35, 2, 2, shade(p.skin, -0.55)]]; })
  ];

  /* ══ 동물 26종 ═══════════════════════════════════════════════════ */
  function earTri(x: any, w: any, c: any, ic: any) {
    var o: AvatarRect[] = [], cx = x, cw = w, y = 14;
    while (cw > 1 && y > 0) { y -= 3; o.push([cx, y, cw, 3, c]); cx += 1; cw -= 2; }
    if (ic) o.push([x + 3, y + 4, Math.max(1, w - 6), 5, ic]);
    return o;
  }
  function ears(kind: any, f: any, inner: any): AvatarRect[] {
  ...
  function muzzle(f: any, light: any, nose: any) {
    var o: AvatarRect[] = [];
  ...
  function snout(f: any, c: any, nose: any): AvatarRect[] {
    return [[24, 30, 16, 9, c], [24, 30, 16, 2, shade(c, 0.15)], [27, 33, 4, 4, nose], [33, 33, 4, 4, nose]];
  }
  function beak(c: any): AvatarRect[] { return [[27, 28, 10, 5, c], [29, 33, 6, 3, shade(c, -0.2)], [29, 29, 3, 2, shade(c, 0.3)]]; }
  function bigEyes(o: any, wc: any, pc: any) {
    o.push([21, 20, 10, 11, wc]); o.push([33, 20, 10, 11, wc]);
    o.push([24, 23, 5, 5, pc]); o.push([36, 23, 5, 5, pc]);
    o.push([24, 23, 2, 2, W]); o.push([36, 23, 2, 2, W]);
  }
  function AN(id: any, ko: any, en: any, fur: any, o: any) { return Object.assign({ id: id, ko: ko, en: en, fur: fur }, o || {}); }
  var ANIMAL = [
    AN('cat', '고양이', 'Cat', ['#e8a860', '#5c4a3f', '#b0b9cc', '#f2e9d8'], { ear: 'tri', inner: '#e0709b',
      post: function(f: any) { return muzzle(f, shade(f, 0.3), '#e0709b').concat([[12, 31, 6, 1, W], [12, 34, 6, 1, W], [46, 31, 6, 1, W], [46, 34, 6, 1, W]]); } }),
    AN('cat2', '검은고양이', 'Black cat', ['#2b211b'], { ear: 'tri', inner: '#8f4f5a',
      post: function(f: any) { return muzzle(f, shade(f, 0.22), '#e0709b').concat([[12, 31, 6, 1, '#8f98aa'], [12, 34, 6, 1, '#8f98aa'], [46, 31, 6, 1, '#8f98aa'], [46, 34, 6, 1, '#8f98aa']]); } }),
    AN('fox', '여우', 'Fox', ['#e8a860', '#d97757', '#c97a2e'], { ear: 'tri-sharp', inner: '#241c18', earColor: '#c9552a',
      post: function(f: any) { var m = mix(W, f, 0.2); return [[25, 28, 14, 1, shade(f, -0.34)]].concat(muzzle(f, m, '#241c18')).concat([[18, 26, 5, 8, m], [41, 26, 5, 8, m]]); } }),
    AN('dog', '개', 'Dog', ['#c9a877', '#8f4f1a', '#d9c9a8', '#5c4a3f'], { ear: 'floppy',
      post: function(f: any) { return muzzle(f, shade(f, 0.26), '#241c18').concat([[24, 12, 8, 4, shade(f, -0.3)]]); } }),
    AN('wolf', '늑대', 'Wolf', ['#8f98aa', '#5a6070', '#b0b9cc'], { ear: 'tri-sharp',
      post: function(f: any) { return [[25, 28, 14, 1, shade(f, -0.34)]].concat(muzzle(f, mix(W, f, 0.25), '#241c18')).concat([[28, 12, 8, 5, shade(f, -0.25)]]); } }),
    AN('bear', '곰', 'Bear', ['#8f6a4a', '#6b4a2f', '#a8917d'], { ear: 'round',
      post: function(f: any) { return muzzle(f, shade(f, 0.22), '#241c18'); } }),
    AN('panda', '판다', 'Panda', ['#f2e9d8'], { ear: 'round', earColor: '#2b211b', ownEyes: true,
      post: function(f: any) { var o = [[20, 20, 11, 11, '#2b211b'], [33, 20, 11, 11, '#2b211b']];
    ...
    AN('tiger', '호랑이', 'Tiger', ['#e0a63c', '#e8a860'], { ear: 'round', inner: '#241c18',
      post: function(f: any) { var s2 = '#241c18'; return [[21, 14, 3, 7, s2], [28, 12, 3, 6, s2], [35, 12, 3, 6, s2], [42, 14, 3, 7, s2], [17, 22, 3, 6, s2], [45, 22, 3, 6, s2]]
        .concat(muzzle(f, W, '#241c18')); } }),
    AN('lion', '사자', 'Lion', ['#e0a63c', '#c9a877'], { ear: 'round',
      pre: function(f: any) { var m = shade(f, -0.3); return [[10, 8, 44, 30, m], [8, 14, 48, 20, m], [12, 4, 40, 8, m], [14, 34, 36, 10, m], [12, 8, 12, 8, shade(f, -0.18)]]; },
      post: function(f: any) { return muzzle(f, shade(f, 0.28), '#241c18'); } }),
    AN('pig', '돼지', 'Pig', ['#f0b8c4', '#e8a0b0'], { ear: 'tri', inner: '#d98fa0',
      post: function(f: any) { return snout(f, shade(f, -0.1), '#c9758a').concat([[31, 39, 2, 2, ink(f)]]); } }),
    AN('cow', '소', 'Cow', ['#f2e9d8'], { ear: 'side', earColor: '#2b211b',
      pre: function() { return [[18, 4, 5, 7, '#d9c9a8'], [41, 4, 5, 7, '#d9c9a8']]; },
      post: function(f: any) { return [[19, 14, 10, 9, '#2b211b'], [38, 30, 8, 7, '#2b211b'], [24, 45, 12, 8, '#2b211b']]
        .concat(snout(f, '#f0b8c4', '#c9758a')); } }),
    AN('sheep', '양', 'Sheep', ['#f7f2e8', '#e8e2d4'], { ear: 'side', earColor: '#d9c9a8',
      pre: function(f: any) { var c = shade(f, 0.05); return [[16, 2, 32, 8, c], [12, 6, 40, 8, c], [18, 0, 10, 5, c], [32, 0, 10, 5, c], [14, 12, 10, 8, c], [40, 12, 10, 8, c]]; },
      post: function(_f: any) { return muzzle('#d9c9a8', '#e8e2d4', '#8f7a63'); } }),
    AN('mouse', '쥐', 'Mouse', ['#b0b9cc', '#a8917d', '#cbb8d6'], { ear: 'round-big', inner: '#e0709b',
      post: function(f: any) { return muzzle(f, shade(f, 0.26), '#e0709b').concat([[10, 32, 8, 1, W], [46, 32, 8, 1, W]]); } }),
    AN('hamster', '햄스터', 'Hamster', ['#e0a63c', '#d9c9a8', '#c9a877'], { ear: 'round',
      post: function(f: any) { return [[14, 28, 10, 10, shade(f, 0.18)], [40, 28, 10, 10, shade(f, 0.18)]].concat(muzzle(f, W, '#c9758a')); } }),
    AN('rabbit', '토끼', 'Rabbit', ['#f2e9d8', '#d9c9a8', '#cbb8d6', '#a8917d'], { ear: 'tall', inner: '#e0709b',
      post: function(f: any) { return muzzle(f, shade(f, 0.24), '#e0709b').concat([[30, 34, 4, 1, ink(f)], [29, 37, 6, 3, W]]); } }),
    AN('frog', '개구리', 'Frog', ['#5c8a3c', '#7d9463', '#2e7d6e'], { ownEyes: true,
      pre: function(f: any) { return [[16, 8, 12, 10, f], [36, 8, 12, 10, f]]; },
      post: function(f: any) { return [[18, 10, 8, 7, W], [38, 10, 8, 7, W], [20, 12, 4, 4, '#241c18'], [40, 12, 4, 4, '#241c18'], [20, 12, 1, 1, W], [40, 12, 1, 1, W],
        [22, 30, 20, 3, ink(f)], [20, 28, 3, 3, ink(f)], [41, 28, 3, 3, ink(f)], [24, 22, 4, 2, shade(f, 0.2)]]; } }),
    AN('bird', '새', 'Bird', ['#3f8fbf', '#e8c33a', '#e278a1'], { ownEyes: true,
      pre: function(_f: any) { return [[28, 2, 3, 7, '#e0a63c'], [32, 1, 3, 8, '#e0a63c'], [36, 3, 3, 6, '#e0a63c']]; },
      post: function(_f: any) { var o: AvatarRect[] = []; bigEyes(o, W, '#241c18'); return o.concat(beak('#e0a63c')); } }),
    AN('penguin', '펭귄', 'Penguin', ['#2f3b5c'], { ownEyes: true,
      post: function(_f: any) { var o = [[22, 18, 20, 20, '#f7f2e8'], [20, 44, 24, 20, '#f7f2e8']]; bigEyes(o, W, '#241c18'); return o.concat(beak('#e0a63c')); } }),
    AN('owl', '부엉이', 'Owl', ['#8f6a4a', '#a8917d', '#6b4a2f'], { ear: 'tuft', ownEyes: true,
      post: function(f: any) { var o = [[19, 18, 12, 14, shade(f, 0.28)], [33, 18, 12, 14, shade(f, 0.28)]]; bigEyes(o, '#f2c04a', '#241c18'); return o.concat(beak('#c9852a')); } }),
    AN('deer', '사슴', 'Deer', ['#c9a877', '#e0a63c'], { ear: 'side',
      ...
      post: function(f: any) { return [[22, 16, 4, 4, W], [38, 18, 4, 4, W], [26, 12, 4, 4, W]].concat(muzzle(f, shade(f, 0.2), '#241c18')); } }),
    AN('monkey', '원숭이', 'Monkey', ['#8f6a4a', '#6b4a2f'], { ear: 'round-big', inner: '#c9a877',
      post: function(_f: any) { return [[20, 22, 24, 18, '#e0c0a0'], [22, 20, 20, 4, '#e0c0a0']].concat(muzzle('#e0c0a0', '#e8cdb0', '#8f6a4a')); } }),
    AN('koala', '코알라', 'Koala', ['#b0b9cc', '#8f98aa'], { ear: 'side', inner: '#e8e2d4',
      post: function(_f: any) { return [[26, 28, 12, 10, '#3a3f4a'], [27, 26, 10, 3, '#3a3f4a'], [29, 30, 3, 2, '#6d7a90']]; } }),
    AN('raccoon', '너구리', 'Raccoon', ['#8f98aa', '#a8917d'], { ear: 'round', ownEyes: true,
      post: function(f: any) { var o = [[18, 20, 28, 10, '#2b211b'], [22, 18, 20, 3, '#2b211b']];
    ...
    AN('turtle', '거북', 'Turtle', ['#7d9463', '#5c8a3c'], { ear: 'none',
      post: function(f: any) { var sh = '#6b4a2f'; return [[12, 44, 40, 20, sh], [16, 42, 32, 4, sh],
    ...
    AN('dragon', '드래곤', 'Dragon', ['#5c8a3c', '#8f6fb5', '#bf4a44', '#3f8fbf'], { ear: 'none',
      pre: function(_f: any) { var hn = '#e0d0a8'; return [[16, 2, 5, 12, hn], [14, 0, 4, 5, hn], [43, 2, 5, 12, hn], [46, 0, 4, 5, hn]]; },
      post: function(f: any) { var sc = shade(f, 0.2); return [[24, 8, 4, 4, sc], [32, 6, 4, 4, sc], [38, 9, 4, 4, sc]]
    ...
    AN('unicorn', '유니콘', 'Unicorn', ['#f7f2e8', '#cbb8d6', '#f0dced'], { ear: 'tri-sharp',
      ...
      post: function(f: any) { return [[18, 10, 6, 10, '#e278a1'], [40, 10, 6, 10, '#8f6fb5'], [24, 8, 6, 8, '#7fc9f2']]
        .concat(muzzle(f, shade(f, -0.06), '#e0709b')); } })
  ];
  var ANIMAL_BY_ID: Record<string, any> = {}; ANIMAL.forEach(function(a: any) { ANIMAL_BY_ID[a.id] = a; });
  ...
  /* 모자 템플릿 */
  function capHat(c: any, badge?: any): AvatarRect[] { var o: AvatarRect[] = [[19, 6, 26, 8, c], [16, 13, 32, 3, shade(c, -0.25)], [19, 6, 26, 2, shade(c, 0.2)]];
    if (badge) { o.push([29, 8, 6, 5, badge]); o.push([31, 9, 2, 3, shade(badge, 0.3)]); } return o; }
  function helmetHat(c: any, band?: any): AvatarRect[] { var o: AvatarRect[] = [[17, 4, 30, 11, c], [14, 14, 36, 3, shade(c, -0.28)], [17, 4, 30, 2, shade(c, 0.18)]];
    if (band) o.push([28, 6, 8, 6, band]); return o; }
  function brimHat(crown: any, brim: any): AvatarRect[] { return [[20, 2, 24, 8, crown], [11, 9, 42, 4, brim], [20, 2, 12, 2, shade(crown, 0.2)], [20, 8, 24, 2, shade(crown, -0.25)]]; }
  function beretHat(c: any): AvatarRect[] { return [[19, 5, 26, 8, c], [17, 11, 30, 3, shade(c, -0.22)], [41, 2, 4, 4, c], [19, 5, 12, 2, shade(c, 0.2)]]; }
  function bandHat(c: any): AvatarRect[] { return [[17, 14, 30, 5, c], [17, 14, 30, 1, shade(c, 0.25)]]; }
  function tieEx(c: any): AvatarRect[] { return [[30, 44, 4, 4, c], [29, 48, 6, 12, c], [30, 58, 4, 3, shade(c, -0.3)], [24, 44, 16, 2, W]]; }
  function apronEx(c: any): AvatarRect[] { return [[21, 48, 22, 16, c], [21, 48, 22, 2, shade(c, 0.18)], [26, 46, 12, 3, shade(c, -0.2)]]; }
  function coatEx(c: any): AvatarRect[] { return [[24, 44, 3, 20, shade(c, -0.12)], [37, 44, 3, 20, shade(c, -0.12)]]; }
  function vestEx(c: any): AvatarRect[] { return [[19, 50, 26, 4, c], [19, 57, 26, 4, c]]; }
  function JB(id: any, ko: any, en: any, group: any, cloth: any, hat: any, extra: any) { return { id: id, ko: ko, en: en, group: group, cloth: cloth, hat: hat, extra: extra }; }
  var JOB = [
    /* 의료 */
    JB('doctor', '의사', 'Doctor', 'medical', '#f2e9d8', null, function(c: any) { return coatEx(c).concat([[22, 46, 3, 10, '#3a3f4a'], [39, 46, 3, 10, '#3a3f4a'], [24, 55, 16, 2, '#3a3f4a'], [29, 56, 6, 5, '#b0b9cc'], [30, 57, 4, 3, '#8f98aa']]); }),
    ...
    JB('dentist', '치과의사', 'Dentist', 'medical', '#e8f2f7', null, function(c: any) { return coatEx(c).concat([[22, 30, 20, 12, '#e8f2f7'], [22, 30, 20, 2, '#c9d6de'], [18, 26, 5, 6, '#c9d6de'], [41, 26, 5, 6, '#c9d6de']]); }),
    /* IT */
    JB('dev', '개발자', 'Developer', 'it', '#4a4a52', null, function(c: any) { return [[14, 46, 36, 5, shade(c, -0.15)], [20, 44, 24, 2, shade(c, -0.15)], [14, 46, 6, 18, shade(c, -0.3)], [44, 46, 6, 18, shade(c, -0.3)],
    ...
    JB('scientist', '과학자', 'Scientist', 'education', '#f2e9d8', null, function(c: any) { return coatEx(c).concat([[20, 16, 24, 6, '#46b6ff'], [18, 18, 3, 4, '#3a3f4a'], [43, 18, 3, 4, '#3a3f4a'], [22, 17, 8, 2, '#ccfaff']]); }),
    /* 요리 */
    ...
    JB('soldier', '군인', 'Soldier', 'safety', '#5c6b47', function() { return helmetHat('#4a5a38'); }, function(c: any) { return [[16, 50, 8, 6, shade(c, -0.2)], [26, 52, 10, 5, shade(c, 0.15)], [38, 48, 8, 7, shade(c, -0.2)]]; }),
    /* 비즈니스 */
    ...
    JB('miner', '광부', 'Miner', 'agriculture', '#5c4a3f', function() { return helmetHat('#e0a63c', '#f2c04a').concat([[28, 4, 8, 5, '#ccfaff']]); },
      function(c: any) { return [[18, 52, 28, 3, shade(c, -0.3)]]; }),
    /* 서비스 */
    ...
    JB('viking', '바이킹', 'Viking', 'fantasy', '#6b4a2f', function() { return [[18, 6, 28, 9, '#a8917d'], [15, 13, 34, 3, '#8f7a63'], [12, 0, 6, 12, '#e0d0a8'], [46, 0, 6, 12, '#e0d0a8'], [10, 0, 4, 5, '#e0d0a8'], [50, 0, 4, 5, '#e0d0a8']]; },
      function(c: any) { return [[18, 48, 28, 4, '#a8917d'], [24, 44, 16, 3, shade(c, -0.3)]]; }),
  ...
  var JOB_BY_ID: Record<string, any> = {}; JOB.forEach(function(j: any) { JOB_BY_ID[j.id] = j; });

  /* ══ spec · 조립 ═════════════════════════════════════════════════ */
  /* 16 그리드 저장본·스튜디오 패치는 색·부품을 인덱스로 담는다 — 여기서 흡수 */
  function pxc(v: any, arr: any, d2: any) {
    if (typeof v === 'number') return arr[Math.abs(v) % arr.length];
    if (typeof v === 'string' && v.charAt(0) === '#') return v;
    return d2;
  }
  function pid(v: any, arr: any) {
  ...
  function ops(sp0: any) {
    ...
    if (sp.type === 'animal') {
      ...
      var AC2 = ACC.filter(function(a: any) { return a.id === sp.acc; })[0];
    ...
    P.cloth = cloth;
    var H = HAIR.filter(function(h: any) { return h.id === sp.hair; })[0] || HAIR[0];
    o = o.concat(headBase(sp.skin));
    var hairOps = H.f(sp.hairColor);
    if (J && J.hat) hairOps = hairOps.filter(function(c: any) { return c[1] > 15; });
    o = o.concat(hairOps);
    ...
    var F = FACE.filter(function(x: any) { return x.id === sp.face; })[0];
    if (F && sp.face !== 'none') o = o.concat(F.f(P));
    var A3 = ACC.filter(function(a: any) { return a.id === sp.acc; })[0];
...
export const AVATAR_HAIR = ENGINE.HAIR as readonly AvatarHair[];
export const AVATAR_ACCESSORIES = ENGINE.ACC as readonly AvatarAccessory[];
export const AVATAR_FACE_DETAILS = ENGINE.FACE as readonly AvatarFaceDetail[];
export const AVATAR_EXPRESSIONS = ENGINE.EXPR as readonly AvatarExpression[];
export const AVATAR_ANIMALS = ENGINE.ANIMAL as readonly AvatarAnimal[];
...
export const AVATAR_EYE_COLORS = ENGINE.EYEC as readonly string[];
export const AVATAR_CLOTH_COLORS = ENGINE.CLOTH as readonly string[];
export const AVATAR_FUR_COLORS = ENGINE.FUR as readonly string[];

export function createAvatarSpec(
  seed: AvatarSeed | null = "nova",
  overrides: AvatarSpecInput = {},
): AvatarSpec {
  return ENGINE.spec(seed, overrides) as AvatarSpec;
}

export function avatarOps(input: AvatarSpecInput | AvatarSpec): AvatarRect[] {
  return ENGINE.ops(input) as AvatarRect[];
}

export interface Avatar64Props extends Omit<SvgProps, "children" | "height" | "viewBox" | "width"> {
...
/**
 * 정수 rect만 네이티브 SVG 노드로 렌더한다.
 *
 * 픽셀 경계를 보존하려면 full은 64의 정수배/약수, head는 40의 정수배/약수 크기로
 * 사용한다. 프로필·직업 화면의 대량 선택지는 FlatList로 가상화해야 한다.
 */
export const Avatar64 = memo(function Avatar64({
  spec: input,
  size = AVATAR_GRID,
  crop = "full",
  nativeComposition,
  ...
  const rects = useMemo(() => avatarOps(input), [input]);
  ...
  return (
    ...
    >
      {rects.map(([x, y, width, height, fill], index) => (
        <SvgRect key={index} x={x} y={y} width={width} height={height} fill={fill} />
      ))}
    </Svg>
  );
});
