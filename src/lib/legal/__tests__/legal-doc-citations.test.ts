...
//
// ⚠⚠ 회차 58 의 첫 판은 **자기 주장보다 좁았다.** 백틱 바로 뒤의 숫자만 봐서
// `` `0031:26-27` `` 은 잡고 `` `db/migrations/0031:26-27` `` 은 놓쳤다 -
// 디렉터리는 있는데 **파일 이름이 없는** 형태다. 인용 정규식은 확장자를
// 요구하고 이 가드는 백틱을 요구하니, 그 9건은 바로 같은 이유로 검사 밖에
// 그대로 남아 있었다. 회차 60 이 그중 하나(Q-G4 의 `0031:11-13`)를 읽으려다
// 발견했다.
//
// 회차 55 가 두 번 만난 "검사기가 자기 주장보다 넓다"의 거울상이다. 넓으면
// 멀쩡한 문장이 위반이 되고, **좁으면 위반이 조용히 통과한다.**
const ABBREVIATED_MIGRATION = /`(?:db\/migrations\/)?(\d{4})(?::[0-9][0-9,\-\s]*)`/g;
