...
export const LEXICON_SCAN_ALLOWLIST: readonly string[] = [
  ...
  "docs/CONSTRAINTS.md",
  // flow-map.json is the machine-readable structure map a fresh session looks up
  // (docs/FLOW-HANDOFF.md is what it reads). It is a developer artifact — never
  // bundled, never rendered — and its Korean strings are the flow-debugger's own
  // descriptions of what a screen does ("이건 정상이에요" = "that is expected
  // behaviour"), not copy shown to a user. Generated by flow-debugger's
  // make-handoff.js; regenerate rather than hand-edit.
  "docs/flow-map.json",
  ...
  // Static UI/function audit findings (legacy/docs/ui-audit/**) quote the exact strings
...
export const LEXICON_NON_CLINICAL_CONTEXTS: Record<Locale, readonly RegExp[]> = {
  ...
  ko: [
    /(?:서버|네트워크|연결|저장|동작|응답|로그인|동기화|실행|설치|빌드|테스트|배포|업로드|다운로드|데이터|호출|쿼리|요청|앱|웹|순서|경로)(?: 상태)?(?:가|이|는|은)?\s+정상이/g,
