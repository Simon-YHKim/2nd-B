...
const regression = readFileSync(
  resolve(root, "db/tests/public_data_quota_regression.sql"),
  "utf8",
);
const workflow = readFileSync(
  resolve(root, ".github/workflows/supabase-dry-run.yml"),
  "utf8",
);
...
describe("0171 public-data quota boundary", () => {
  ...
  test("executes the behavioral quota contract in the pinned Postgres workflow", () => {
    expect(workflow).toMatch(/- "db\/tests\/\*\*"/);
    expect(workflow).toContain(
      "-f db/tests/public_data_quota_regression.sql",
    );
    expect(regression).toContain("has_function_privilege(");
    expect(regression).toContain("relforcerowsecurity");
    expect(regression).toContain("cross-user call unexpectedly succeeded");
    expect(regression).toContain("EXIM call 900 was denied");
    expect(regression).toContain("denied user call changed the global count");
  });
});
