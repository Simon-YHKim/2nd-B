...
describe("capture-full life-area screen wiring", () => {
  ...
  test("clears local area state at sign-out and direct account changes", () => {
    // main's hydration effect opens with a generation fence (#1551), so the
    // anchor is that line rather than the bare `if (!userId)`.
    const boundaryStart = captureSource.indexOf(
      "const hydrationGeneration = ++draftHydrationGenerationRef.current;",
    );
    const boundaryEnd = captureSource.indexOf("loadCaptureDraftState(userId)", boundaryStart);
  ...
  test("keeps domain queries hidden, fails closed, and preserves ordinary tags", () => {
    // This PR originally parsed `mode`/`tag` itself. main (#1551) had meanwhile
    // put route consumption behind `planCaptureParamConsumption`, which also
    // gates on focus, owner, share priority and durable ack. Two consumption
    // paths would let a stale closure overwrite that verdict, so the selector
    // now MIRRORS the planner's intent instead of deciding for itself.
    expect(captureSource).toContain(
      'const routedArea = intent.kind === "set" ? lifeAreaFromTag(chip) : null;',
    );
    // Fails closed: no valid life area, or a record instrument that owns its own
    // classification, leaves the visible chip off.
    expect(captureSource).toContain("!isRecordCaptureMode(plan.targetMode ?? mode)");
    expect(captureSource).toContain("setSelectedLifeArea(showsArea ? routedArea : null);");
    // Ordinary user tags survive a domain chip application, still capped at ten.
    expect(captureSource).toContain(
      "const base = prev.filter((x) => !isDomainTag(x)).slice(0, 10);",
    );
    // Manual hashtag entry can never inject a reserved domain tag. main made
    // that guard unconditional, so it no longer hangs off enableLifeAreaIntents
    // -- strictly stronger than the flag-scoped check this PR shipped.
    const addTag = captureSource.slice(
      captureSource.indexOf("function addTagFromInput(input: string)"),
      captureSource.indexOf("function updateOcrBody("),
    );
    expect(addTag).toContain("isDomainTag(norm) ||");
    expect(addTag).not.toContain("enableLifeAreaIntents");
  });
  ...
  test("adds the selected tag after classification and only on the source path", () => {
    ...
    // main (#1551) nested the post-save cleanup inside the revision fence, so
    // `reset()` is indented deeper than when this PR was written.
    const reset = captureSource.indexOf("reset();", save);
