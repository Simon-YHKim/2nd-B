...
import {
  addImportHistory,
  getImportHistory,
  purgeImportHistoryForDeletedAccount,
  removeImportHistory,
...
describe("native import history uses encrypted storage", () => {
  ...
  test("terminal deletion purges only the deleted owner's history", async () => {
    await withNativeRuntime(async () => {
      await addImportHistory("user-A", entry("a1"));
      await addImportHistory("user-B", entry("b1"));

      await expect(purgeImportHistoryForDeletedAccount("user-A")).resolves.toBe(true);
      expect(await getImportHistory("user-A")).toEqual([]);
      expect((await getImportHistory("user-B")).map((item) => item.id)).toEqual(["b1"]);
    });
  });

  test("terminal deletion reports an unobserved local purge", async () => {
    await withNativeRuntime(async () => {
      mockEncryptedStorage.removeItem.mockImplementation(async (key: string) => {
        if (key === "import.history:user-A") throw new Error("disk unavailable");
        nativeStore.delete(key);
      });
      await expect(purgeImportHistoryForDeletedAccount("user-A")).resolves.toBe(false);
    });
  });

  test("a durable deletion fence blocks late add/remove while preserving another owner", async () => {
    await withNativeRuntime(async () => {
      await addImportHistory("user-A", entry("a1"));
      await addImportHistory("user-B", entry("b1"));
      ...
      await addImportHistory("user-A", entry("a2"));
      await removeImportHistory("user-A", "a1");
      await addImportHistory("user-B", entry("b2"));

      expect((await getImportHistory("user-A")).map((item) => item.id)).toEqual(["a1"]);
      expect((await getImportHistory("user-B")).map((item) => item.id)).toEqual(["b2", "b1"]);
    });
  });
});
