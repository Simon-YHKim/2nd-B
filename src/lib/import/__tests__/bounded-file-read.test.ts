...
const mockReadOnlyMode = "r";

jest.mock("expo-file-system", () => ({
  File: mockNativeFile,
  FileMode: { ReadOnly: mockReadOnlyMode },
}));
...
describe("local URI fetch boundary", () => {
  ...
  test.each(["file:///cache/local.bin", "content://picker/document/42"])(
    "reads native local source %s only through a bounded FileHandle",
    async (uri) => {
      ...
      const close = jest.fn();
      const open = jest.fn(() => ({ size: 3, readBytes, close }));
      mockNativeFile.mockImplementation(() => ({ open }));
      const fetchSpy = jest.fn();
      globalThis.fetch = fetchSpy as unknown as typeof fetch;

      await expect(fetchBoundedLocalBytes(uri, { declaredBytes: 3 })).resolves.toEqual(
        new Uint8Array([1, 2, 3]),
      );

      expect(mockNativeFile).toHaveBeenCalledWith(uri);
      expect(open).toHaveBeenCalledWith(mockReadOnlyMode);
  ...
  test("closes a native handle when its snapshot size disagrees with picker metadata", async () => {
    (Platform as { OS: string }).OS = "ios";
    const readBytes = jest.fn();
    const close = jest.fn();
    mockNativeFile.mockImplementation(() => ({
      open: () => ({ size: 4, readBytes, close }),
    }));

    await expect(
      fetchBoundedLocalBytes("file:///changed.bin", { declaredBytes: 3 }),
    ).rejects.toMatchObject({ code: "size_mismatch" });
    expect(readBytes).not.toHaveBeenCalled();
    expect(close).toHaveBeenCalledTimes(1);
  });

  test("closes a native handle after a read failure without exposing provider details", async () => {
    (Platform as { OS: string }).OS = "android";
    const close = jest.fn();
    mockNativeFile.mockImplementation(() => ({
      open: () => ({
        size: null,
        readBytes: () => {
          throw new Error("content://private/provider/path");
        },
        close,
      }),
    }));

    const read = fetchBoundedLocalBytes("content://picker/private", { maxBytes: 4 });
    await expect(read).rejects.toMatchObject({ code: "read_failed" });
    await expect(read).rejects.not.toThrow("content://private/provider/path");
    expect(close).toHaveBeenCalledTimes(1);
  });

  test("closes an active native handle when the caller aborts", async () => {
    (Platform as { OS: string }).OS = "android";
    const controller = new AbortController();
    const close = jest.fn();
    const readBytes = jest.fn(() => {
    ...
    await expect(
      fetchBoundedLocalBytes("file:///abort.bin", {
        declaredBytes: 1,
  ...
  test("closes an active native handle when its read deadline expires", async () => {
    (Platform as { OS: string }).OS = "android";
    let now = 10_000;
    ...
    mockNativeFile.mockImplementation(() => ({
      open: () => ({ size: null, readBytes, close }),
    }));

    try {
      await expect(
        fetchBoundedLocalBytes("file:///deadline.bin", { maxBytes: 100, timeoutMs: 5 }),
  ...
  test.each([
    [
      ...
      {
        ...responseWith(streamFrom([]), String(MAX_BOUNDED_FILE_BYTES + 1)),
        redirected: false,
        url: "file:///cache/local.bin",
      } as Response,
      "too_large",
    ],
    [
      "picker and Content-Length mismatch",
      {
        ...responseWith(streamFrom([]), "2"),
        redirected: false,
        url: "file:///cache/local.bin",
      } as Response,
      "size_mismatch",
    ...
    [
      "changed final URI",
      {
        ...
        redirected: false,
        url: "file:///cache/other.bin",
      } as Response,
      "unsafe_source",
    ],
  ])("aborts fetch before rejecting %s", async (_caseName, response, code) => {
    let requestSignal: AbortSignal | undefined;
    globalThis.fetch = jest.fn((_uri: string | URL | Request, init?: RequestInit) => {
      requestSignal = init?.signal as AbortSignal;
      return Promise.resolve(response);
    }) as unknown as typeof fetch;

    await expect(
      fetchBoundedLocalBytes("file:///cache/local.bin", { declaredBytes: 1 }),
