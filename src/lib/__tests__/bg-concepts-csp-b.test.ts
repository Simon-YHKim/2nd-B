...
function contentSecurityPolicy(html: string): Map<string, string[]> {
  const matches = [...html.matchAll(
    /<meta\s+http-equiv=(["'])Content-Security-Policy\1\s+content=(["'])([\s\S]*?)\2\s*\/?>/gi,
  )];
  if (matches.length !== 1) {
    throw new Error(`Expected one Content-Security-Policy meta, got ${matches.length}`);
  }
...
describe("background concept CSP group B", () => {
  test.each(PAGE_PATHS)("%s has a default-deny, hash-only policy", (path) => {
    ...
    expect(html.match(/<meta\s+name=["']referrer["']\s+content=["']no-referrer["']\s*\/?>/gi)).toHaveLength(1);
  });
