...
describe("standalone proto supply chain", () => {
  ...
  test("chains the deterministic proto compiler into the existing static build", () => {
    ...
    expect(buildScript).toContain('legalComments: "eof"');
    expect(buildScript).toContain('public/THIRD_PARTY_NOTICES.txt');
    expect(buildScript).toContain('distRoot, "THIRD_PARTY_NOTICES.txt"');
  ...
  test("ships complete notices for every bundled runtime dependency", () => {
    for (const component of [
      ...
      "Scheduler 0.27.0",
      "three.js 0.160.0",
    ]) expect(thirdPartyNotices).toContain(component);
    expect(thirdPartyNotices.match(/Permission is hereby granted/g)).toHaveLength(2);
    expect(thirdPartyNotices.match(/THE SOFTWARE IS PROVIDED "AS IS"/g)).toHaveLength(2);
    expect(thirdPartyNotices).toContain("Copyright (c) Meta Platforms, Inc. and affiliates.");
    expect(thirdPartyNotices).toMatch(/Copyright .* three\.js authors/);
  });

  test("binds the edit protocol to the exact trusted parent origin", () => {
    const harness = loadTweaksProtocol({
      ancestorOrigins: ["https://proto.example"],
      referrer: "https://proto.example/session/42",
    });

    expect(harness.outbound).toEqual([
      {
        message: { type: "__edit_mode_available" },
        targetOrigin: "https://proto.example",
      },
    ]);
  ...
  test("accepts only exact control messages from the bound parent", () => {
    ...
    harness.dispatch('{type:"__activate_edit_mode"}', "https://proto.example", false);
    ...
    harness.dispatch('{type:"__activate_edit_mode_extra"}', "https://proto.example");
    expect(harness.stateTransitions).toEqual([]);

    harness.dispatch('{type:"__activate_edit_mode"}', "https://proto.example");
    harness.dispatch('{type:"__deactivate_edit_mode"}', "https://proto.example");
  ...
  test.each([
    ...
    [
      "arbitrary secure cross-origin parent",
      {
        ancestorOrigins: ["https://editor.example"],
        referrer: "https://editor.example/embed",
      },
    ],
  ...
  test.each([
    [{ ancestorOrigins: ["https://proto.example"] }, "https://proto.example"],
    [{ referrer: "https://proto.example/session/42" }, "https://proto.example"],
