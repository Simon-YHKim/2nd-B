...
// 쪽으로 틀렸을 행이다.
//
// ── 판독 불가 구멍은 등록 경로에서 닫혔다 (2026-09-22) ────────────────
// deviceRegionCode() 가 null 이면 이메일 가입과 OAuth 프로필 완료 화면이 그때만
// 거주 국가를 묻는다. `resolveRegistrationConsentFloor()` 가 조사표에 있는 선택에는
// 그 나라 행을 적용하고, "목록에 없음"과 임의 값은 폴백 18 로 보낸다. 읽힌 기기
// 지역은 자기신고 값으로 덮어쓸 수 없다.
//
// 이 선택은 거주 증명이 아니며 저장하지도 않는다. 서버의 전역 < 14 하한 역시
// 그대로라서 14보다 높은 나라별 층은 직접 RPC 호출에 대해 여전히 advisory 다.
// 세 `FloorSource` 를 갈라 둔 덕분에 등록 경로가 `region-unreadable` 만 복구하고
// `country-no-row` 를 낮추지 않는다.
// 화면은 선택 전 제출을 닫고 선택 뒤 생년월일 문구와 판정을 같은 하한으로 바꾼다.
// 낮은 수준의 이메일 가입과 OAuth 프로필 생성도 auth/DB 호출 전에 다시 판정하므로
// 비활성 버튼만 믿는 흐름은 아니다.
//
