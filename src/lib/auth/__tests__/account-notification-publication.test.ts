...
describe("account notification publication gate", () => {
  test("AuthProvider awaits the gate before its first owner publication", () => {
    ...
    expect(source).toContain("noteExternalAuthSessionMutation();");
  });

  test("an external B owner event invalidates an in-flight destructive lease", async () => {
    const paused = deferred();
    const destructive = runAuthSessionMutation(async (lease) => {
