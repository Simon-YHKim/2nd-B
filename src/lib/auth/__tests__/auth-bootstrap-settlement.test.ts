...
const LEASE_B: TestLease = { revision: 2, token: "attempt-b" };
...
function makeDeps(
  overrides: Partial<AuthBootstrapSettlement<TestSession, unknown, TestLease>> = {},
) {
  const calls = {
...
describe("recovery cases stay locked", () => {
  ...
  test("a late A NULL answer cannot release readiness or resolve after B takes ownership", async () => {
    let answer: (v: SessionLoadOutcome<TestSession>) => void = () => {};
    ...
    const { deps, calls } = makeDeps({
      ...
      releaseRecoveryPending: async (lease) => {
        calls.clearedPending.push(lease);
        // A's exact clear is stale once B owns both memory and durable state.
        return false;
      },
    });
    settleAuthBootstrap(deps);
...
describe("pending-owner release runtime", () => {
  test("claims single-flight synchronously and stale A cannot release B", async () => {
    ...
    const coalesced = runtime.release({ ...LEASE_A });
    expect(coalesced).toBe(first);

    currentOwner = LEASE_B;
    pending = true;
    finishClear(currentOwner === LEASE_A ? "cleared" : "stale");

    await expect(first).resolves.toBe(false);
    await expect(coalesced).resolves.toBe(false);
    expect(clearCalls).toEqual([LEASE_A]);
    expect(currentOwner).toBe(LEASE_B);
    expect(pending).toBe(true);
  });

  test("releases only when the exact clear leaves no pending owner", async () => {
    let pending = true;
    const runtime = createRecoveryPendingReleaseRuntime<TestLease>({
      sameLease: (left, right) => left.revision === right.revision && left.token === right.token,
      clear: async () => {
    ...
    await expect(runtime.release(LEASE_A)).resolves.toBe(true);
...
describe("provider and screen wiring", () => {
  ...
  test("the bootstrap outer catch classifies storage before local sign-out", () => {
    ...
    const failClosed = block.indexOf("void failClosedRecovery(");
  ...
  test("all auth-storage entry points share the exact recovery classifier", () => {
    ...
    expect(effect).toContain("return failClosedRecovery(stored, error, null);");
