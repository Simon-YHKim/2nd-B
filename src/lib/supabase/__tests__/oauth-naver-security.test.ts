...
import { completeNaverOAuth, isNativeNaverCallbackState, signInWithNaver } from "../auth";
...
const authPath = join(process.cwd(), "src/lib/supabase/auth.ts");
...
function edgeRequest(
  ...
  return new Request("https://project.supabase.co/functions/v1/oauth-naver", {
    method: "POST",
    headers: {
...
describe("oauth-naver durable server boundary", () => {
  ...
  test("kill switch, missing pepper, redirect tricks, and caller policy fields fail closed", async () => {
    ...
    const invalidBodies = [
      { action: "start", redirect_uri: `${PRODUCTION_REDIRECT}?next=https://evil.invalid`, native: false },
      { action: "start", redirect_uri: PRODUCTION_REDIRECT, native: false, provider: "naver" },
      { action: "start", redirect_uri: PRODUCTION_REDIRECT, native: false, limit: 1 },
    ];
  ...
  test("rejects untrusted origins, absent gateway identity, oversized bodies, and bad provider JSON", async () => {
    ...
    const huge = await edgeHandler(edgeRequest({
      action: "start",
      redirect_uri: PRODUCTION_REDIRECT,
      native: false,
  ...
  test("source has hard egress/body limits and no scan, raw PII response, detail, or logs", () => {
    expect(edgeSource).toContain("AbortSignal.timeout(5000)");
    expect(edgeSource).toContain("redirect: 'error'");
    expect(edgeSource).toMatch(/REQUEST_BODY_LIMIT_BYTES\s*=\s*16 \* 1024/);
...
describe("Naver client state capability", () => {
  const originalClientId = process.env.EXPO_PUBLIC_NAVER_CLIENT_ID;
  const originalEnabled = process.env.EXPO_PUBLIC_ENABLE_NAVER;

  beforeEach(() => {
    process.env.EXPO_PUBLIC_NAVER_CLIENT_ID = "public-client-id";
    process.env.EXPO_PUBLIC_ENABLE_NAVER = "true";
  ...
  afterAll(() => {
    if (originalClientId === undefined) delete process.env.EXPO_PUBLIC_NAVER_CLIENT_ID;
    else process.env.EXPO_PUBLIC_NAVER_CLIENT_ID = originalClientId;
    if (originalEnabled === undefined) delete process.env.EXPO_PUBLIC_ENABLE_NAVER;
    else process.env.EXPO_PUBLIC_ENABLE_NAVER = originalEnabled;
  });

  test("gets state from the server, preserves redirect binding, and consumes it locally once", async () => {
    ...
    expect(invoke).toHaveBeenNthCalledWith(1, "oauth-naver", {
      body: { action: "start", redirect_uri: PRODUCTION_REDIRECT, native: false },
    });
    const entries = [...storage.values.entries()];
    expect(entries).toHaveLength(1);
    const [storageKey, rawTransaction] = entries[0]!;
    const transaction = JSON.parse(rawTransaction) as { state: string; redirectUri: string };
    expect(transaction.state).toBe(state);
    expect(transaction.redirectUri).toBe(PRODUCTION_REDIRECT);
    ...
    await expect(completeNaverOAuth({ code: "opaque-code", state })).rejects.toThrow(/state|transaction/i);
    expect(invoke).toHaveBeenCalledTimes(2);
  });

  test("accepts only exact server-shaped native states and has no weak random fallback", () => {
    const authSource = readFileSync(authPath, "utf8");
    expect(isNativeNaverCallbackState(`native.${"c".repeat(64)}`)).toBe(true);
    expect(isNativeNaverCallbackState("native.abc123")).toBe(false);
    expect(isNativeNaverCallbackState(`${"c".repeat(64)}`)).toBe(false);
    expect(authSource).not.toMatch(/Math\.random\s*\(/);
    expect(authSource).not.toMatch(/code_challenge|code_verifier/);
  });
});
