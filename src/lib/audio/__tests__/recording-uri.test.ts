...
describe("recorder caller lifecycle contract", () => {
  ...
  const callers = {
    callReflection: readFileSync(resolve(appRoot, "call-reflection.tsx"), "utf8"),
  ...
  test("all stop-to-transcribe paths claim before reading and release in finally", () => {
    for (const source of Object.values(callers)) {
  ...
  test("cancel, back, recorder replacement, mode exit, and unmount use bounded cleanup", () => {
    expect(callers.callReflection).toContain("stopAndDiscardRecording(audioRecorder)");
    expect(callers.callReflection).toContain("phaseRef.current");
    expect(callers.secondb).toContain("stopAndDiscardRecording(audioRecorder)");
    expect(callers.secondb).toContain("voicePhaseRef.current");
    expect(callers.capture).toContain("stopAndDiscardRecording(audioRecorder)");
    expect(callers.capture).toContain("stopVoiceCaptureForModeExit");
    for (const source of Object.values(callers)) {
      expect(source).toContain("waitForRecordingCleanup(audioRecorder)");
    }
    expect(callers.callReflection).toContain("recorderLifecycleRef.current");
    expect(callers.secondb).toContain("recorderLifecycleRef.current");

    for (const source of Object.values(callers)) {
      expect(source).not.toMatch(/discardRecording\((?:audioRecorder\.uri|recordingUri)\)/);
    }
    expect(callers.callReflection).not.toContain(".catch(() => {})");
    ...
    const callUnmountCleanup = lifecycleEffect(callers.callReflection);
    const chatUnmountCleanup = lifecycleEffect(callers.secondb);
    expect(callUnmountCleanup).not.toContain("setPhase(");
    expect(chatUnmountCleanup).not.toContain("setVoicePhase(");

    const startFlow = callers.callReflection.slice(
      callers.callReflection.indexOf("async function startRecording"),
      callers.callReflection.indexOf("async function stopAndTranscribe"),
    );
    const transcriptionFlow = callers.callReflection.slice(
      callers.callReflection.indexOf("async function stopAndTranscribe"),
      callers.callReflection.indexOf("async function cancelRecording"),
    );
    expect(startFlow).not.toContain("(e as Error).message");
    expect(transcriptionFlow).not.toContain("(e as Error).message");
  });
