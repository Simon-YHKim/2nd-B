...
function PeopleMapBody({ userId }: { userId: string }) {
  ...
  // Saves do NOT use a latest-wins guard. A timed-out create may still land in
  // Postgres, so the write carries a stable id+revision and `createPerson`
  // upserts on it — a retry converges onto the same row instead of adding a
  // duplicate person. `attemptGenRef` invalidates only the presentation.
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Hide the unmounted screen's presentation. The in-flight request keeps
      // its id+revision lock so a late success still converges server-side.
  ...
  // `closeAddForm` is a hoisted declaration below with no reactive closure —
  // it only calls state setters and touches `attemptGenRef`. Leaving it out of
  // the dependency list keeps this callback stable for the BackHandler effect.
  ...
  async function handleAdd() {
    if (!name.trim()) return;
    // The in-flight lock — not `saving` — is the real double-press gate: it is
    // set synchronously, so two taps in one frame cannot both open a write.
    ...
    try {
      ...
      // The write is confirmed, so it is shown immediately and kept through a
      // later failing reconciliation read. A successful save must never look
      // lost — that is what invites the user to enter the person twice.
      ...
      setSaveFailed(false);
      if (isCurrentUi) {
        // A hidden form skips its stale presentation reset, but the confirmed
        // row above still appears and the draft above stays consumed.
        setKind("friend");
        setCloseness(3);
        setPage(0);
        setAdding(false);
      ...
      // The draft and its id survive so reopening retries the same row at
      // rev+1 rather than writing a second person.
      if (isCurrentUi) setSaveFailed(true);
    }
  }

  function closeAddForm() {
    // Presentation only. A request already in flight keeps its id+revision
    // lock, so closing mid-save cannot strand or duplicate the write.
    invalidatePersonSaveAttemptUi(attemptGenRef);
    setAdding(false);
    setSaveFailed(false);
  }
