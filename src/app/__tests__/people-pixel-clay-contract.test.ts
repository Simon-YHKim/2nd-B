...
describe("/people PIXEL-CLAY contract", () => {
  test("keeps the actual owner-scoped read and create path without sample or contacts state", () => {
    ...
    expect(SOURCE).toContain("<PeopleMapBody key={userId} userId={userId} />");
  ...
  test("drops stale owner reads, and pins writes to one id+revision", () => {
    ...
    // This PR originally guarded saves with a second latest-wins token. main
    // had meanwhile shipped a strictly stronger guarantee (#1581): the write
    // carries a stable id+revision and `createPerson` upserts on it, so a
    // timed-out save that lands late converges onto the same row. A latest-wins
    // token only hides a stale result — it cannot stop the duplicate row. The
    // stronger machinery is kept; the save token is intentionally gone.
    expect(SOURCE).not.toContain("saveGuardRef");
    expect(SOURCE).toContain("const attemptGenRef = useRef(0)");
    expect(SOURCE).toContain("const saveIdRef = useRef<PersonSaveIdentity | null>(null)");
    expect(SOURCE).toContain("const inFlightRef = useRef<PersonSaveIdentity | null>(null)");
    expect(SOURCE).toContain("}, attempt.id, attempt.rev);");
    expect(SOURCE).toMatch(
      /return \(\) => \{\s+loadGuardRef\.current\.begin\(\);[\s\S]{0,200}?invalidatePersonSaveAttemptUi\(attemptGenRef\);/,
    );
  ...
  test("uses the same form, selection, route fallback order for hardware and top back", () => {
    ...
    // The close path is `closeAddForm` (main's name, kept so its own contract
    // test still binds). It no longer refuses to close mid-save: the in-flight
    // id+revision lock makes closing safe, so the user is not trapped behind a
    // slow write. The "don't start a second write" guard moved to the toggle.
    expect(SOURCE).toMatch(/function closeAddForm\(\) \{[\s\S]*?invalidatePersonSaveAttemptUi\(attemptGenRef\);/);
    expect(SOURCE).toMatch(/function toggleAddForm\(\) \{\s+if \(adding\) \{\s+closeAddForm\(\);\s+\} else if \(!saving\) \{/);
  });
