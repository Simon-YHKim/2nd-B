...
// 크래시 리포팅 재도입은 별도 결정 사항이다.
//
// ⚠ 2026-09-08 정정. 여기 이렇게 적혀 있었다: *"`d0e88e64` 가 **그 벤더 의존성까지
// 지웠으므로** 되살리면 선언되지 않은 패키지를 require 하게 된다."* **그 근거는
// 이 경고가 다루는 경우에 대해 틀렸다.** `d0e88e64`(2026-09-06)가 지운 것은 **웹**
// 쪽 패키지이고, 이 주석이 경고하는 **네이티브** SDK 는 `package.json` 에 그대로
// 선언돼 있다(#619, 2026-07-03 이후 제거된 적 없음). 즉 네이티브에서는 "패키지가
// 없어서 못 켠다" 가 성립하지 않는다.
//
// 되살리기를 실제로 막는 것은 **검사**다 — `analytics.test.ts` 의
// "runtime source and native build config contain no ... SDK entry point" 가
// 런타임 소스와 네이티브 빌드 설정을 읽어 진입점이 다시 생기면 빌드를 깨뜨린다.
// 결론(되살리지 않는다)은 그대로지만 **근거가 바뀌었다**: 없는 패키지가 아니라
// 있는 검사가 막는다. 결론이 맞아도 근거가 틀린 주석은 다른 곳에서 재적용된다.
