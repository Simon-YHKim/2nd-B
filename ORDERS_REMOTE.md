...
### [시뮬 P1 사이클 47 / 2026-06-12] — ✅ "편하게 보기" 1회 제안 + 시뮬 오탐 정정 (PR — CI 확인 중)
[2026-06-12 / 05:09:43 KST] 클라우드 세션 — 시뮬 v2 신규 P1 첫 배치(PR #366 머지 확인, main `5097c3d`).
- **P1-1 가입 직후 외형 제안**: 그래프 홈 하단 슬롯 **최우선 1회 카드** — "글자가 작거나 흐리게 보이면 읽기 쉬운 글꼴로 바꿔드릴게요" + [읽기 쉽게 보기](탭 즉시 글꼴 전환 = 변화 자체가 피드백) / [지금이 좋아요] + "설정 > 테마에서 언제든" 복귀 안내 1줄. 카드 본문·버튼은 **자기 자신부터 읽기 쉬운 글꼴로 렌더**(타깃 사용자가 픽셀 폰트로 된 제안문을 못 읽는 모순 차단). 조건: 미응답 + 픽셀 기본값일 때만, 응답 시 영구 해제(`appearance.comfortOffer.dismissedAt`, core-hint 스토어 미러 — 3번째 사본, 4번째 생기면 팩토리화 주석). 빈 그래프 카드·코치 힌트·스포트라이트는 전부 이 카드 뒤로(정보밀도 1메시지). 44px+ 버튼. 테스트 2.
- **P1-4 그래프 노드 토크백 라벨 = 시뮬 오탐 정정**: NavGraph는 이미 전 노드에 `accessibilityLabel`(코어 `n.label[locale]`·센터·데이터 노드) 구현돼 있음 — 시뮬 에이전트가 코드 레벨 a11y를 볼 수 없는 한계. 실수정은 데이터 노드 무제목 폴백 `"piece"` 영문 고정 → 로케일 정합("조각") 닛 1건만.
- 잔여 신규 P1: P1-2 음성 입력(네이티브 STT — EAS 트랙 설계 필요) · P1-3 보관소 검색(다음 사이클) · P1-5 생년월일 입력 마찰(DateField UX). verify 129 suites/1125 tests green.

### [전연령 시뮬 v2 / 2026-06-12] — ✅ 30인 페르소나 × 전 기능 워크스루 + 근거 검증 HTML 보고서 (PR — CI 확인 중)
[2026-06-12 / 00:34:04 KST] 클라우드 세션 — Simon /deep-research 지시(전연령·직업·소득 페르소나 전 기능 테스트, 화면별 행위·혼잣말·생각, 도달 그래프, 조치 내용 포함 HTML). 산출물 = **`docs/persona-allage-simulation-v2.html`** (186KB self-contained, 실브라우저 렌더 검수 0 에러).
- **방법**: ① deep-research 5각 병렬 검색→61클레임→적대 검증(43 확정/13 강등/5 킬 — D7 90% 오인용·가짜 벤치마크 등 교정) ② 페르소나 30인(영유아 5세~93세, **전문직 7인**: UX 디자이너·백엔드 개발자·AI 리서처·PM·변호사·CTO·은퇴 교수, 소득 하~상, 기기 보급형~플래그십) ③ **2026-06-11 라이브 QA 실측 화면 카피**를 대본으로 4클러스터 병렬 워크스루 — 여정 204건 전량 기록(행위/혼잣말/생각/마찰 0-3).
- **결과**: S7 완주 18/30, 평균 잔존 3.1/5, 최다 이탈 S5→S6. 연령 절벽은 70+에서만 급락(P27 뒤로가기 미로·P29 음성 부재·P30 사용 불가 정직 판정) — 14~68세는 전 구간 생존. 어제까지의 조치들이 실전 검증됨: 드래프트 영속화(P27 "있다! 안 날아갔어!"), 읽기쉬운 글꼴(P21 발견 성공), 토글 행 터치·리본(P25 합격), 클립보드 제안·OCR(P28 "젊은 앱들도 잘 안 해주는 건데").
- **조치 표**: A 완료 18건(시뮬에서 효과 입증 페르소나 매핑) / B 게이트 4건 / **C 신규 17건(P1 5)**: ① 가입 직후 글꼴/밝기 1회 제안(60+ 전원 — 옵션은 있는데 "픽셀 어둠을 뚫고" 설정까지 가야 함) ② 음성 입력("말로 담기" — P29 세대 입장권) ③ 보관소 검색 부재(P28 "메모장의 반쪽") ④ 그래프 노드 토크백 라벨 공백(P25 "별들은 이름이 없어") ⑤ 생년월일 텍스트 입력(P27 "줄표가 어딨냐") 등.
- 근거 20클레임 각주(출처 URL) + 한계 고지 포함. verify 1123 green.

### [라이브 QA 사이클 46 / 2026-06-11] — ✅ 전 기능 도달성·직관성 실주행 감사 + P1 픽스 배치 (PR — CI 확인 중)
[2026-06-11 / 22:49:31 KST] 클라우드 세션 — Simon 지시("사용자 입장에서 모든 기능 사용 가능 + 진입 직관성 확인"). **방법: 정적 도달성 스캔(전 라우트 × 진입 참조) + 라이브 사이트 실브라우저 E2E**(QA 계정 `qa.bot.85081488@example.com` 생성 — 가입→온보딩→저장→그래프→루틴→설정 전 여정, 스크린샷 25장).
- **여정 검증 통과**: 가입(마스터 동의 1탭) → 온보딩 1장 → journal 저장("Lumen이 새 조각을 저장했어요"+CTA) → records 반영 → insights 주간 리포트 즉시 반영 → settings 2그룹+언어(5종)/크루/위험구역 → privacy(외부 분석·광고·**루틴 푸시** 토글 ✓) → theme(글꼴·**가벼운 모드** ✓) → plans 4티어 → secondb(0/2 캡 표시) → esm → wiki(빈 상태 안내) → core-brain → **/ops 풀 플로우**(몸→운동 루틴→**라이브 개인화 추천 생성 성공**("점심시간 15분 활력 산책")→푸시 3버튼) — LLM live Variables 작동 확인.
- **P1 픽스 3건 (동-PR)**: ① **Apple/Kakao 죽은 버튼** — Supabase 콘솔 미활성인데 기본 노출, 클릭 시 원시 JSON 에러로 이탈(웹은 전면 리다이렉트라 세션 자동숨김 불가) → 기본 OFF(나버 패턴)+web-deploy Variables 배선(Google은 실동작 확인, 기본 ON 유지). **켜려면: Supabase 콘솔 provider 활성 + Variable true — Simon 게이트.** ② **/inbox 도달 불가**(전진 진입 0 — 클립 수신함 전체가 고아) → 프로필 허브 "받은편지함" 행 신설(2탭). ③ **/mbti 도달 불가**(검사 시작 경로 없음 — 결과 카드는 데이터 있어야만 노출) → persona 도구 카드에 MBTI 추가.
- **env 토글 전면 경화**: 어제 장애 클래스 일반화 — 모든 optional 토글(LLM_MODE/FORCE_TIER/ENABLE_* 등)이 워크플로의 ""-주입을 zod 도달 전에 흡수(presentOrUndefined). 회귀 테스트 2건 추가.
- **잔여 등록 (P2~)**: React #418 hydration mismatch가 전 화면 콘솔에 반복(프로덕션 자동복구로 화면 무영향 — 별도 진단 사이클) · ops 첫 추천 간헐 실패(재시도 성공, 정직 카피 동작 — 관찰) · 그래프 데이터 fetch 일시 실패 리본(헤드리스 환경 요인 추정 — 재시도 탭 동작 ✓) · 소셜 가입 후 복귀 여정은 Apple/Kakao 활성화 시 재검증.
- verify 128 suites/1123 tests green. QA 계정은 테스트 데이터 1건 보유 — 위험구역에서 삭제 가능.

### [🚨 장애 복구 / 2026-06-11] — ✅ 라이브 블랙 스크린 (~03:17부터 ~18시간) 원인 제거 + 복구 검증 (PR #363 머지)
[2026-06-11 / 21:45:12 KST] 클라우드 세션 — Simon 폰 스크린샷 제보(21:24) → 헤드리스 크로미엄 재현 → 픽스 → 재배포 → **실브라우저 복구 확인**(React 마운트 + 로그인 화면 렌더).
- **원인**: web-deploy가 미설정 repo Variable을 **빈 문자열로 주입** + `env.ts`의 `EXPO_PUBLIC_POSTHOG_HOST: z.string().url()`이 ""를 거부 → 모듈 초기화 ZodError → React 무마운트 → 전 방문자 검은 화면. **#331(분석 배선) 배포 03:17 KST부터 지속** — 빌드·CI·HTTP 200은 전부 green이라 안 보였음.
- **픽스 (PR #363, main `702aeb0`)**: 원칙 = 텔레메트리 설정의 유일한 허용 실패 = "분석 꺼짐". ① `normalizeAnalyticsUrl()` — 빈 값→off, 스킴 없는 호스트→https:// 자동 보정(동작하게), 쓰레기→경고+off ② readRaw가 분석 7필드의 ""를 zod 전에 undefined로 ③ Supabase URL은 의도적 strict 유지. 회귀 가드 5케이스(장애 모양 그대로 포함). verify 128/1121.
- **프로세스 교훈 (재발 방지 규칙)**: "Pages green"은 빌드 신호일 뿐 — **머지 후 라이브 검증은 반드시 브라우저 프로브**(React 마운트 + pageerror 0 확인). 이 컨테이너에 playwright+chromium 설치 완료, 프로브 스크립트 = `/tmp/probe.cjs` 패턴(향후 사이클은 배포 후 이 프로브를 표준 단계로).
- 영향 범위 참고: 오늘 머지된 기능들(사이클 31~45)은 전부 무결 — 죽어 있던 건 분석 env 검증 한 줄이었고, 복구 후 전 기능 정상.
