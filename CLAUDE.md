...
- ~~`src/lib/auth/consent-age.ts:55 resolveJurisdiction()` 이 **항상 "KR" 을 반환**해
  ...
  **폴백 18** 이다(`src/lib/auth/consent-age.ts:97`). **KR 고정 경로는 런타임에 없다** —
  ...
  ⚠ **"resolver 가 항상 KR 을 반환한다" 를 새 판단의 근거로 인용하지 말 것.** 지금의 사실은
  그 자리에 **남은 것 둘**이다:
  ① 표의 최댓값은 **태국 20** 인데 폴백은 **18** 이라, **지역이 안 읽히는 태국 18~19세**는
  아직 과소보호다. 이건 실수가 아니라 **의도된 절충**이고 `FALLBACK_SHORTFALL`
  (`src/lib/auth/consent-age.ts:111-116`)에 이름과 이유가 적혀 있다 — 폴백을 20 으로 올리면
  지역을 못 읽었다는 이유만으로 **거의 모든 나라의 성인(18~19세)을 막게 된다.**
  ② **서버 하한은 여전히 나라를 안 보는 `< 14`**
  (`db/migrations/0050_health_consent_default.sql:38`)라, 14 를 넘는 클라이언트 층은
  직접 RPC 호출에 대해 **advisory** 다. 이 둘을 닫는 것은 각각 거주국 자기신고와
  서버 마이그레이션이고, **둘 다 Simon 결정 사항이다.**
